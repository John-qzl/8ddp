...
public class CodeUtil {
	 ...
	 * @return
	 */
	public static String getFreeMarkerTemplate(String html,IFormTable bpmFormTable,Boolean isEdit){
	...
	public static String getDialogTags(String html, IFormField field) {
			Document doc= Jsoup.parseBodyFragment(html);
	 ...
	 * @param isEdit
	 */
	private static void parseSubTable(Document doc,IFormTable bpmFormTable,boolean isEdit){
		Elements list=doc.select("div[type=subtable]");
		List<?extends IFormTable> subTables=bpmFormTable.getSubTableList();
			...
			for(IFormTable subTable:subTables){
				if(subTable.getTableName().equalsIgnoreCase(tableName)){
					...
					if(subEle.select("[formtype]").size()!=0){
						...
						//行内编辑模式
						if(IFormDef.EditInline.equals(mode)){
	 ...
	 * @param isEdit
	 */
	private static void parseWindowSubTableField(IFormTable subTable,
	...
	private static void parseEditSubTableField(IFormTable table,Element e,boolean isEdit){
	...
	private static void parseField(Element e,String classVar, boolean isEdit) {
		...
		if(controltype.equals("office")){
			if(isEdit){
				e.attr("right", "w");
				if(e.parent().select("div[class=office-div]").size()>0){
	 ...
	 * @param isEdit
	 */
	private static void parseMainField(Document doc,IFormTable bpmFormTable,Boolean isEdit) {
