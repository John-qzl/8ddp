 ...
 * 2.实现IScript。
 *  在spring注册类。
 *  &lt;bean id="scriptImpl" class="com.ibms.oa.service.flow.ScriptImpl">&lt;/bean>
 *  使用类的方法如下：
 *  获取当前用户：
...
public class GroovyScriptEngine implements BeanPostProcessor {
	...
	/**
	 * 工程启动时候，通过该方法将配置在spring中的所有bean都加载进来，在groovy脚本中直接可以引用这些bean。
	 * 如：groovy脚本中写：jdbcDao
	 * .upd("UPDATE  CWM_SYS_USER SET PHOTO =15061542312 WHERE USERID = -1"
	 * )。能够成功运行。 因为jdbcDao在app-beans.xml文件中注册过。 给groovy脚本引擎注入
	 * service对象引用和实现IScript对象的引用。
	 */
	public Object postProcessAfterInitialization(Object bean, String beanName)
			throws BeansException {
		boolean rtn = true;// BeanUtils.isInherit(bean.getClass(),
							// BaseService.class);
