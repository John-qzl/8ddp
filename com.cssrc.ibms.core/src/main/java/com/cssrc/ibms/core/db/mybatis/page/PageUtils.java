...
public class PageUtils {
	...
	public static int getLastNumber(int currentPage, int pageSize,
			int totalCount) {
	 ...
	 * @return
	 */
	public static List<Integer> getPageNumbers(int currentPage, int totalPage,
			int count) {
		int avg = count / 2;
	 ...
	 * @return
	 */
	public static int getPageNumber(int currentPage, int pageSize,
			int totalCount) {
		if (currentPage <= 1) 
			return 1;	
		if (Integer.MAX_VALUE == currentPage
				|| currentPage > getTotalPage(totalCount, pageSize)) { // last page
	 ...
	 * @throws Exception
	 */
	public static String getPageHtml(PagingBean pageBean, String url,
		...
		return freemarkEngine.mergeTemplateIntoString(templetName, model);
	}
}
