...
public class TaskReminder extends BaseModel implements ITaskReminder
{
    // 时间设置ID
    @XmlAttribute
    ...
    {
        if (this.taskWarningSetList != null)
        {
            return this.taskWarningSetList;
        }
        
        if (StringUtil.isEmpty(this.warningSetJson))
        {
            return Collections.emptyList();
        }
        try
        {
            List<TaskWarningSet> list= JSONArray.parseArray(this.warningSetJson, TaskWarningSet.class);
            for(TaskWarningSet wset:list) {
                String json=PropertyUtil.getByAlias("task.WarnLevel");
                JSONArray jsons=JSONArray.parseArray(json);
                String name=getStringFromJSON(jsons,wset.getLevel());
                wset.setName(name);
            }
            return list;
    ...
    public String getStringFromJSON(JSONArray jsons,int level) {
        for(Object json:jsons) {
            JSONObject _jsonObject=(JSONObject)json;
            if(_jsonObject.getInteger("level")==level) {
    ...
    {
        return getStingFromJson("mailText", mailContent);
    }
    
    public String getCopyTo()
    {
        return getStingFromJson("copyTo", mailContent);
        
    }
    
    public String getCopyToID()
    {
        return getStingFromJson("copyToID", mailContent);
        
    }
    
    public String getFormVarCopyTo()
    {
        return getStingFromJson("formVarCopyTo", mailContent);
        
    }
    
    public String getMailSubject()
    {
        return getStingFromJson("mailSubject", mailContent);
    }
    ...
    public int hashCode()
    {
        return new HashCodeBuilder(-82280557, -700257973).append(this.taskDueId)
    ...
    public String getQuarzCron()
    {
        return quarzCron;
    }
    
    public void setQuarzCron(String quarzCron)
    {
        this.quarzCron = quarzCron;
    }
