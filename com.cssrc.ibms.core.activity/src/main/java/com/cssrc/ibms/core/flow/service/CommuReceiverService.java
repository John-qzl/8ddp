...
public class CommuReceiverService extends BaseService<CommuReceiver>
{
	 ...
	 * @param usrIdTaskIds
	 */
	public void saveReceiver(Long opinionId, Map<Long, Long> usrIdTaskIds,ISysUser curUser){
		...
		while (it.hasNext()) {
			...
			ISysUser sysUser= (ISysUser)sysUserService.getById(receiverId);
	...
	public void setCommuReceiverStatus(Long taskId, Integer status,
			Long recevierId) {
		CommuReceiver commuReceiver = getByTaskReceiver(taskId, recevierId);
		if (BeanUtils.isEmpty(commuReceiver))
			return;
		commuReceiver.setStatus(status);
		commuReceiver.setFeedbacktime(new Date());
		this.dao.update(commuReceiver);
	}
	 ...
	 * @param sysUser
	 */
	public void setCommuReceiverStatus(TaskEntity taskEntity,ISysUser sysUser){
	 ...
	 * @param sysUser
	 */
	public void setCommuReceiverStatusToFeedBack(TaskEntity taskEntity,ISysUser sysUser){
		Long userId=sysUser.getUserId();
		String taskId=taskEntity.getId();
		//沟通意见任务、流转任务
		if(!TaskOpinion.STATUS_COMMUNICATION.toString().equals(taskEntity.getDescription()) &&
	 ...
	 * @param sysUser
	 */
	public void setRestartCommuReceiverStatus(TaskEntity taskEntity,ISysUser sysUser){
		Long userId=sysUser.getUserId();
		String taskId=taskEntity.getId();
		//沟通意见任务
