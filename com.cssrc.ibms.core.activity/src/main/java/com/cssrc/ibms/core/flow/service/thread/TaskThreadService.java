 ...
 */
@Service
public class TaskThreadService implements ITaskThreadService{
	...
	//外部子流程调用,存放当前外部子流程taskid 列表。
    private static ThreadLocal<List<String>> extSubTask=new ThreadLocal<List<String>>();
	 ...
	 * @return
	 */
	public ProcessCmd getProcessCmdP(){
    ...
    public static List<String> getExtSubTask() {
        return extSubTask.get();
    }

    public static void setExtSubTask(List<String> extSubTaskList) {
        extSubTask.set(extSubTaskList);
    }
     ...
     * @param value
     */
    public static void addExtSubTask(String taskId) {
        List<String> list=null;
        if(extSubTask.get()==null){
            list=new ArrayList<String>();
            list.add(taskId);
            extSubTask.set(list);
        }
        else{
            extSubTask.get().add(taskId);
        }
    }
    
    public static void cleanExtSubTask() {
    	if(extSubTask.get()!=null){
        	extSubTask.get().clear();
            extSubTask.remove();
    	}
