...
public class JumpRule {
	...
	public String evaluate(ExecutionEntity execution,Short isJumpForDef){
		...
		//获取规则列表
		List<NodeRule> ruleList=nodeRuleService.getByDefIdNodeId(actDefId, activityId);
		...
		for(NodeRule nodeRule:ruleList){
			try{
		...
		if(ruleList.size()>0 && NodeSet.RULE_INVALID_NO_NORMAL.equals(isJumpForDef)) return RULE_INVALID;
		return "";
	}
