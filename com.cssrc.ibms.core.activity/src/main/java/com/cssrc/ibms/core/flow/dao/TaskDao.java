...
public class TaskDao extends BaseDao<TaskEntity>
{
	...
	{
		return "com.cssrc.ibms.core.activity.model.ProcessTask";
	}
	...
	public List<ProcessTask> getTasks(Long userId,String taskName,String subject,String processName,String orderField,String orderSeq,PagingBean pb){
		...
		List list = null;
		//用于获取所有的数据，不进行条数的限制，使用时需要将pagesize设置为-1 ----by liubo
		if(pb.getPageSize()==-1){
			list=getBySqlKey("getAllMyTask",params);
		}else{
			list=getBySqlKey("getAllMyTask",params,pb);
		}
		return list;
	}
...
//		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	 ...
	 * @return
	 */
	public List<ProcessTask> getByTaskNameOrTaskIds(String userId,String taskName, String taskIds,PagingBean pb){
		Map<String,Object> params=new HashMap<String,Object>();
		params.put("userId", userId);
	...
	//通过父任务id获取流程流转任务
	public List<TaskEntity> getTransToTaskByParentTaskId(String parentTaskId) {
		return getBySqlKey("getTransToTaskByParentTaskId", parentTaskId);
	}
	
	//通过父任务id获取流程沟通意见任务
	public List<ProcessTask> getCommunicationTaskByParentTaskId(String parentTaskId) {
		List list= getBySqlKey("getCommunicationTaskByTaskId", parentTaskId);
		return list;
	}

	public ProcessTask getTaskByParentIdAndUser(String parentTaskId,
			String userId) {
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("parentTaskId", parentTaskId);
		params.put("userId", userId);
	...
	public void updateTaskPriority(String taskId, int level) {
		...
		update("updateTaskPriority", params);
	}
