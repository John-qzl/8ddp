...
//		}
	...
	public FormTableIndex getIndex(String tableName, String indexName) {
		...
		List<FormTableIndex> indexes = jdbcTemplate.query(sql, indexRowMapper);
		List<FormTableIndex> indexList = mergeIndex(indexes);
		if(BeanUtils.isEmpty(indexList)){
			return null;
		}else{
			FormTableIndex index = indexList.get(0);
	...
	public List<FormTableIndex> getIndexesByTable(String tableName) {
		...
		List<FormTableIndex> indexList = mergeIndex(indexes);
		for(FormTableIndex index:indexList){
			index.setIndexDdl(generateIndexDDL(index));
		}
		return indexList;
	}
	...
	@Override
	public List<FormTableIndex> getIndexesByFuzzyMatching(String tableName, String indexName,Boolean getDDL) {
		return getIndexesByFuzzyMatching(tableName, indexName, getDDL, null);
	}

	@Override
	public List<FormTableIndex> getIndexesByFuzzyMatching(String tableName, String indexName, Boolean getDDL, PagingBean pageBean) {
		String sql = SQL_GET_ALL_INDEX;
	 ...
	 * @see com.ibms.core.customertable.AbstractTableOperator#rebuildIndex(java.lang.String, java.lang.String)
	 */
	@Override
	...
	private List<FormTableIndex> mergeIndex(List<FormTableIndex> indexes){
		List<FormTableIndex> indexList = new ArrayList<FormTableIndex>();
		for (FormTableIndex index : indexes) {
			boolean found=false;
			for (FormTableIndex index1 : indexList) {
	...
	/**
	 * 生成FormTableIndex对象对应的DDL语句
	 * @param index
	 * @return
	 */
	private String generateIndexDDL(FormTableIndex index){
		...
		if(!StringUtil.isEmpty(index.getIndexType())){
			if(FormTableIndex.INDEX_TYPE_CLUSTERED.equalsIgnoreCase(index.getIndexType())){
				sql.append(" CLUSTER ");
			}
		}
	...
	private RowMapper<FormTableIndex> indexRowMapper=new RowMapper<FormTableIndex>() {
		@Override
		public FormTableIndex mapRow(ResultSet rs, int rowNum) throws SQLException {
			FormTableIndex index=new FormTableIndex();
			index.setIndexTable(rs.getString("TABNAME"));
			index.setTableType(FormTableIndex.TABLE_TYPE_TABLE);
			...
			if("CLUS".equalsIgnoreCase(indexType)){
				index.setIndexType(FormTableIndex.INDEX_TYPE_CLUSTERED);
			}else if("REG".equalsIgnoreCase(indexType)){
				index.setIndexType(FormTableIndex.INDEX_TYPE_REG);
			}else if("DIM".equalsIgnoreCase(indexType)){
				index.setIndexType(FormTableIndex.INDEX_TYPE_DIM);
			}else if("BLOK".equalsIgnoreCase(indexType)){
				index.setIndexType(FormTableIndex.INDEX_TYPE_BLOK);
			}
