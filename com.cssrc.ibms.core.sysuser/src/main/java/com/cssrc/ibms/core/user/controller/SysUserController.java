...
public class SysUserController extends BaseController {
	...
	public ModelAndView list(HttpServletRequest request,
		...
		boolean isSupportWeixin = PropertyUtil.getBooleanByAlias(
				"supportWeiXin", false);
		List<SysUser> list = this.sysUserService
				.getUsersByQuery(new QueryFilter(request, "sysUserItem"));
	...
	public ModelAndView edit(HttpServletRequest request) throws Exception {
		...
		// 旧签章模型(弃用 by zmz )
		//List<?extends ISignModel> signModelList = this.signModelService.getByUserId(userId);
		//返回当前签章
	...
	public ModelAndView getEditMv(HttpServletRequest request, ModelAndView mv) {
		...
		if (userId.longValue() != 0L) {
			...
			List<UserPosition> userPosList = this.userPositionService
					.getByUserId(userId);
			...
			/* sysUser.setPassword(this.pwdStrategyService.getUsingInitPwd()); */
			/*
			 * ======= //added by liqing 2015-10-26 电子签章 if(user.getSign_pic()
			 * != null) { SysFile file =
			 * sysFileService.getById(Long.valueOf(user.getSign_pic()));
			 * if(file!=null){
			 * userE.setSign_picpath(file.getFilename()+"."+file.getExt()); } }
			 * if(user.getOriginalsign_pic() != null) { SysFile file =
			 * sysFileService.getById(Long.valueOf(user.getOriginalsign_pic()));
			 * if(file!=null){
			 * userE.setOriginalsign_picpath(file.getFilename()+"."
			 * +file.getExt()); } } if(user.getPhoto() != null) { SysFile file =
			 * sysFileService.getById(Long.valueOf(user.getPhoto()));
			 * if(file!=null){
	...
	public void modifyPwd(HttpServletRequest request,
		...
		if ((StringUtil.isEmpty(newPassword))
			...
			resultMsg = "输入的密码不能为空";
		else if(isChange&&accountStrategyMin.getIs_enable().equals("1")&&Integer.valueOf(accountStrategyMin.getStrategy_value().toString())*24*3600>between)
	...
	public void resetPwd(HttpServletRequest request,
		...
		if (StringUtil.isEmpty(password))
			...
			resultMsg = "输入的密码不符合密码复杂性要求:"+accountStrategyService.meetPasswordComplexity(sysUser.getUsername(),password);
		else {
			try {
				/*JSONObject json = this.pwdStrategyService.checkUser(sysUser,
						password);
				...
				String str = MessageUtil.getMessage();
				if (StringUtil.isNotEmpty(str)) {
					ResultMessage resultMessage = new ResultMessage(0, "重置密码失败:"
							+ str);
					response.getWriter().print(resultMessage);
				} else {
					String message = ExceptionUtil.getExceptionMessage(ex);
	...
	public ModelAndView editSecurityView(HttpServletRequest request)
			throws Exception {
		String returnUrl = RequestUtil.getPrePage(request);
		Long userId = Long.valueOf(RequestUtil.getLong(request, "userId"));
		SysUser sysUser = (SysUser) this.sysUserService.getById(userId);
	...
	public void editStatus(HttpServletRequest request,
		...
		Short result = 0;
		try {
			this.sysUserService.updStatus(userId,currentUserId,
					Short.valueOf((short) (short) status));
			result = 1;
			...
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				ResultMessage resultMessage = new ResultMessage(0, "修改用户状态失败:"
						+ str);
				response.getWriter().print(resultMessage);
			} else {
	...
	@DataNote(beanName = { SysUser.class }, pkName = "userId")
	public void editSecurity(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		Long userId = Long.valueOf(RequestUtil.getLong(request, "userId"));
		Long currentUserId = UserContextUtil.getCurrentUserId();
		...
		try {
			...
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				ResultMessage resultMessage = new ResultMessage(0, "修改用户密级失败:"
						+ str);
				response.getWriter().print(resultMessage);
			} else {
	...
	public ModelAndView getInfo(HttpServletRequest request,HttpServletResponse response) throws Exception{
		ModelAndView mv = getAutoView();
		//ISysUser user = sysUserService.findByUserId(RequestUtil.getLong(request,"userId"));
		boolean isRight = UserContextUtil.getCurrentUserId().equals(ISysUser.RIGHT_USER)?true:false;
	...
	public ModelAndView getView(HttpServletRequest request,
		...
		List posList = this.userPositionService.getByUserId(Long
				.valueOf(userId));

		List userPosList = this.userPositionService.getOrgListByUserId(Long
				.valueOf(userId));
		...
		return mv.addObject("sysUser", sysUser).addObject("roleList", roleList)
	...
	private void handelUserSoruce(ModelAndView mv) {
		boolean isShowPos = PropertyUtil.getBooleanByAlias(
				"userDialog.showPos", true);
		boolean isShowRole = PropertyUtil.getBooleanByAlias(
				"userDialog.showRole", true);
		boolean isShowOnlineUser = PropertyUtil.getBooleanByAlias(
				"userDialog.showOnlineUser", true);
	...
	public ModelAndView GradeDialog(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView mv = getAutoView();
	...
	public ModelAndView flowDialog(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ModelAndView mv = getAutoView();
		List demensionList = this.demensionService.getAll();
		mv.addObject("demensionList", demensionList);
	...
	public ModelAndView selector(HttpServletRequest request,
		...
		if ("4".equals(searchBy)) {
			...
			if (demId.equals("-1")) {
				...
				list = this.sysUserService
						.getDistinctUserByOrgPath(queryFilter);
			}
			...
			if (includSub == 0)
				...
				list = this.sysUserService
						.getDistinctUserByPosPath(queryFilter);
			}
			...
			if(orgId==0){
				...
				if ((StringUtil.isNotEmpty(type)) && (!"all".equals(typeVal))
					...
					String path = this.orgServiceImpl.getSysOrgByScope(type,
							typeVal).getPath();
		...
		for (SysUser user : list) {
			orgNames = this.userPositionService.getOrgnamesByUserId(user
					.getUserId());
	...
	public List<SysUser> getUserListByJobId(HttpServletRequest request)
			throws Exception {
	...
	public List<SysUser> getUserListByPosId(HttpServletRequest request)
			throws Exception {
	...
	public void save(HttpServletRequest request, HttpServletResponse response,
			SysUser sysUser,String signModelNewPath) throws Exception {
		String resultMsg = null;
		//设置操作结果，默认为操作失败
		Short result = 0;
		Long selectedOrgId = Long.valueOf(RequestUtil.getLong(request,
				"selectedOrgId"));
		...
		;
		Long posIdPrimary = Long.valueOf(RequestUtil.getLong(request,
				"posIdPrimary"));
		Long userId = Long.valueOf(RequestUtil.getLong(request,"userId"));
		Long[] roleIds = RequestUtil.getLongAry(request, "roleId");
		Long[] aryOrgId = RequestUtil.getLongAry(request, "orgId");
		Long[] arySuperiorId = RequestUtil.getLongAryByStr(request,
				"superiorId");
		boolean isadd = true;
		// 第一次新增用户需明确主组织主岗位
		if ((BeanUtils.isNotEmpty(aryOrgId))
				&& (posIdPrimary.longValue() == 0L)) {
			resultMsg = "请选择主岗位！";
			isadd = userId == null;
			writeResultMessage(response.getWriter(), resultMsg, 0);
		}else{
			if (sysUser.getUserId() == null) {
				boolean isExist = this.sysUserService.isUsernameExist(sysUser
						.getUsername());
				if (isExist) {
					resultMsg = "该账号已经存在！";
					writeResultMessage(response.getWriter(), resultMsg, 0);
					...
					if(StringUtil.isNotEmpty(resultMsg)){
						...
						try {
							this.sysUserService.saveUser(bySelf, sysUser, posIdCharge,
									posIds, posIdPrimary, roleIds, aryOrgId);
							
							/*-----------------------------------更新签章(ie8样式不兼容,弃用)------------------------------------*/
							//取刚刚保存的用户id
/*							Long savedUserId=sysUserService.getByUsernames(sysUser.getUsername()).get(0).getUserId();	
				...
				if ((UserContextUtil.isSuperAdmin(sysUser))
						&& (!UserContextUtil.isSuperAdmin())) {
					resultMsg = "使用限制，不能编辑超级管理员！";
					writeResultMessage(response.getWriter(), resultMsg, 0);
				}else{
					boolean isExist = this.sysUserService.isUsernameExistForUpd(
	...
	public void getFuzzySysUser(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			JSONArray userArr = this.sysUserService.getFuzzySysUserList(userFuzzyName, fieldName, relvalue, type, typeVal);
	...
	public void dataSynchronization(HttpServletRequest request, HttpServletResponse response) throws Exception {
		String syncUserUrl="http://staffer.no8.sast.casc:8080/StaffService.asmx/WsGetStaff";  //3
	/*	String syncCheck="http://staffer.no8.sast.casc:8080/StaffService.asmx/WsCheckUser";   //
