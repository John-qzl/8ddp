...
CREATE OR REPLACE PROCEDURE synData_dwj (owneruser IN VARCHAR DEFAULT 'ibms')
AS
countt number;
sqlll varchar2(1000);
BEGIN
--------------------------1.CWM_SYS_RES添加数据------2017-4-16----------------------------------------------- 
BEGIN
	...
	IF(countt=0) THEN  
		...
		values('5001','项目权限管理','xmqxgl',1,'/styles/default/images/resicon/add.png',3,'',0,1,1,0,null);
    	insert into CWM_SYS_ROLE_RES(ROLERESID,ROLEID,RESID) select MAX(ROLERESID)+1,-1,5001 FROM CWM_SYS_ROLE_RES;
    	commit;
    END IF;
	...
	IF(countt=0) THEN  
		insert into CWM_SYS_RES(RESID,RESNAME, ALIAS,SN,ICON, PARENTID, DEFAULTURL, ISFOLDER,ISDISPLAYINMENU,ISOPEN,ISNEWOPEN,PATH)
		values('5002','项目功能点类别','xmgndlb',1,'/images/function/mid/gtk-dialog-question.png',5001,'/oa/system/recType/list.do',0,1,1,0,null);
    	insert into CWM_SYS_ROLE_RES(ROLERESID,ROLEID,RESID) select MAX(ROLERESID)+1,-1,5002 FROM CWM_SYS_ROLE_RES;
    	commit;
    END IF;
	...
	IF(countt=0) THEN  
		insert into CWM_SYS_RES(RESID,RESNAME, ALIAS,SN,ICON, PARENTID, DEFAULTURL, ISFOLDER,ISDISPLAYINMENU,ISOPEN,ISNEWOPEN,PATH)
		values('5003','项目角色类别','xmjslb',2,'/images/function/mid/guayadeque.png',5001,'/oa/system/recRole/manage.do',0,1,1,0,null);
    	insert into CWM_SYS_ROLE_RES(ROLERESID,ROLEID,RESID) select MAX(ROLERESID)+1,-1,5003 FROM CWM_SYS_ROLE_RES;
    	commit;
    END IF;
   ...
   -- 2017年7月4日：数据分析
	...
	IF(countt=0) THEN  
		insert into CWM_SYS_RES(RESID,RESNAME, ALIAS,SN,ICON, PARENTID, DEFAULTURL, ISFOLDER,ISDISPLAYINMENU,ISOPEN,ISNEWOPEN,PATH)
		values('5005','工具集成','statistics',15,'form',2,'',0,1,1,0,null);
    	insert into CWM_SYS_ROLE_RES(ROLERESID,ROLEID,RESID) select MAX(ROLERESID)+1,-1,5005 FROM CWM_SYS_ROLE_RES;
    	commit;
    END IF;
    -- 修改图标
	...
	IF(countt=0) THEN  
    	...
    	insert into CWM_SYS_ROLE_RES(ROLERESID,ROLEID,RESID) select MAX(ROLERESID)+1,-1,5006 FROM CWM_SYS_ROLE_RES;
    	commit;
    END IF;
	...
	IF(countt=0) THEN  
    	...
    	insert into CWM_SYS_ROLE_RES(ROLERESID,ROLEID,RESID) select MAX(ROLERESID)+1,-1,5007 FROM CWM_SYS_ROLE_RES;
    	commit;
    END IF;
    -- 2017-7-18：数据迁移
	...
	IF(countt=0) THEN  
    	...
    	insert into CWM_SYS_ROLE_RES(ROLERESID,ROLEID,RESID) select MAX(ROLERESID)+1,-1,5008 FROM CWM_SYS_ROLE_RES;
    	commit;
    END IF;
	...
	END IF;	
    -- 2017-7-18：数据迁移-业务表结构
    SELECT COUNT(*) into countt FROM CWM_SYS_RES WHERE ALIAS = 'businessTableStruct';
	IF(countt=0) THEN  
		insert into CWM_SYS_RES(RESID,RESNAME, ALIAS,SN,ICON, PARENTID, DEFAULTURL, ISFOLDER,ISDISPLAYINMENU,ISOPEN,ISNEWOPEN,PATH)
		values('5009','业务表结构','businessTableStruct',2,'form',5008,'/oa/migration/config.do',0,1,1,0,null);
    	insert into CWM_SYS_ROLE_RES(ROLERESID,ROLEID,RESID) select MAX(ROLERESID)+1,-1,5009 FROM CWM_SYS_ROLE_RES;
    	commit;
    END IF;
    -- 2017-7-18：数据迁移-表单xml生成
    SELECT COUNT(*) into countt FROM CWM_SYS_RES WHERE ALIAS = 'crateFormXml';
	IF(countt=0) THEN  
		insert into CWM_SYS_RES(RESID,RESNAME, ALIAS,SN,ICON, PARENTID, DEFAULTURL, ISFOLDER,ISDISPLAYINMENU,ISOPEN,ISNEWOPEN,PATH)
		values('5010','表单xml生成','crateFormXml',2,'form',5008,'/oa/migration/xml.do',0,1,1,0,null);
    	insert into CWM_SYS_ROLE_RES(ROLERESID,ROLEID,RESID) select MAX(ROLERESID)+1,-1,5010 FROM CWM_SYS_ROLE_RES;
    	commit;
    END IF;
	...
	IF(countt=0) THEN  
		INSERT INTO ins_col_type VALUES (5011, '列表数据预览', 'listingPreview', '/oa/portal/insPortal/getDataListHtml.do', '无', 'URL', '', '', '', '', null, '-4', '', '-4', '0');
    	commit;
    END IF;
    SELECT COUNT(*) into countt FROM ins_column WHERE KEY_ like 'listingPreview';
	IF(countt=0) THEN  
		INSERT INTO ins_column VALUES (5012, 5011, '列表数据预览', 'listingPreview', null, 'ENABLED', '20', 'YES', '列表数据预览', '0', '-4', null, '-4', '', '500');
    	commit;
    END IF;
	...
	END IF;
END;
--------------------------2.CWM_SYS_PARAMETER添加数据------2017-6-12----------------------------------------------- 
-- 1.新增参数配置，是否将用户个性化设置放入到redis
BEGIN
	SELECT COUNT(*) into countt FROM CWM_SYS_PARAMETER WHERE ID = '5004';
	IF(countt=0)THEN
		insert into CWM_SYS_PARAMETER(ID,NAME,DATATYPE,VALUE,DESCRIPTION)values(5004,'user.custom.infoset',0,1,'用户个性化设置是否放入到redis中参数配置');
	END IF;
END;
COMMIT;
END synData_dwj;
