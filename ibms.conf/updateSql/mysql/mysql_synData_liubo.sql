DROP PROCEDURE IF EXISTS prod_data_liubo;
CREATE PROCEDURE prod_data_liubo(spacename varchar(100))
BEGIN
...
-- -----------添加判断是本地存储还是分布式文件存储的系统参数-------2016-11-27 by liubo------------------------- --
 SELECT COUNT(*) into countt FROM CWM_SYS_PARAMETER WHERE NAME = 'IS_FILE_FASTDFS';
	IF(countt=0) THEN  
		...
		values('4001','IS_FILE_FASTDFS','0','0','采用分布式文件存储方式FASTDFS进行文件上传，下载，删除操作。1表示是，0表示否');
 END IF;
 ...
 -- -----------通过该参数来设置沟通是是否产生代表任务，"1"表示产生，"2"表示不产生。-------2017-02-13 by liubo---------------------------
  SELECT COUNT(*) into countt FROM CWM_SYS_PARAMETER WHERE NAME = 'IS_PRODUCE_TASK';
	IF(countt=0) THEN  
		insert into CWM_SYS_PARAMETER(ID,NAME,DATATYPE,VALUE,DESCRIPTION)
		values('4003','IS_PRODUCE_TASK','0','1','通过该参数来设置沟通是是否产生代表任务，"1"表示产生，"2"表示不产生。');
 END IF;
...
SELECT count(*) into countt from CWM_SYS_PARAMETER WHERE NAME = 'IS_COMMUNICATE_EMAIL';
 	IF(countt=0) THEN
	  	insert into CWM_SYS_PARAMETER(ID,NAME,DATATYPE,VALUE,DESCRIPTION)
	  	values('4002','IS_COMMUNICATE_EMAIL','0','1#1#1','流程中沟通采用形式。以#号区分，分别代表邮件、站内消息、RTX消息，1代表可以选择，0代表不可以选择。必须保证有一种沟通形式为1');
END IF;
...
SELECT COUNT(*) into countt FROM CWM_SYS_PARAMETER WHERE NAME = 'IS_DISPLAY_SECURITY';
	IF(countt=0) THEN
	  	insert into CWM_SYS_PARAMETER(ID,NAME,DATATYPE,VALUE,DESCRIPTION)
	  	values('4004','IS_DISPLAY_SECURITY','0','0','文件上传组件中是否显示密级选择权限和附件列表中是否按照密级筛选，默认为0。0：不显示；1：显示。');
END IF;
...
-- -----------账号策略表新增基本信息-------2017-03-27 by liubo------------------------- --
...
IF(countt=0) THEN
	...
	-- 给系统管理员分配权限----
	SELECT RESID into @res_id FROM CWM_SYS_RES WHERE ALIAS = 'zhcl';
...
IF(countt=0) THEN
	...
	-- 给系统管理员分配权限---
	SELECT RESID into @res_id FROM CWM_SYS_RES WHERE ALIAS = 'sjypzgl';
	insert into CWM_SYS_ROLE_RES(ROLERESID,ROLEID,RESID) select MAX(ROLERESID)+1,-1,@res_id FROM CWM_SYS_ROLE_RES;
END IF;


SELECT COUNT(*) into countt FROM CWM_SYS_RES WHERE ALIAS = 'sjymb';
IF(countt=0) THEN
	insert into CWM_SYS_RES(RESID,RESNAME, ALIAS, SN,ICON, PARENTID, DEFAULTURL, ISFOLDER,ISDISPLAYINMENU,ISOPEN,ISNEWOPEN,PATH)
	...
	-- 给系统管理员分配权限---
	SELECT RESID into @res_id FROM CWM_SYS_RES WHERE ALIAS = 'sjymb';
	insert into CWM_SYS_ROLE_RES(ROLERESID,ROLEID,RESID) select MAX(ROLERESID)+1,-1,@res_id FROM CWM_SYS_ROLE_RES;
END IF;

SELECT COUNT(*) into countt FROM CWM_SYS_RES WHERE ALIAS = 'sjy';
IF(countt=0) THEN
	insert into CWM_SYS_RES(RESID,RESNAME, ALIAS, SN,ICON, PARENTID, DEFAULTURL, ISFOLDER,ISDISPLAYINMENU,ISOPEN,ISNEWOPEN,PATH)
	values('4008','数据源','sjy',2,'/styles/default/images/resicon/icon_tab3.png',4006,'/oa/system/sysDataSourceDef/list.do',1,1,1,0,null);
	-- 给系统管理员分配权限----
	SELECT RESID into @res_id FROM CWM_SYS_RES WHERE ALIAS = 'sjy';
	insert into CWM_SYS_ROLE_RES(ROLERESID,ROLEID,RESID) select MAX(ROLERESID)+1,-1,@res_id FROM CWM_SYS_ROLE_RES;
END IF;
...
-- -----------判断是否是门户系统-------2017-4-20 by liubo------------------------- --
 SELECT COUNT(*) into countt FROM CWM_SYS_PARAMETER WHERE NAME = 'IS_SHOW_DOOR';
	IF(countt=0) THEN  
		insert into CWM_SYS_PARAMETER(ID,NAME,DATATYPE,VALUE,DESCRIPTION)
		values('4009','IS_SHOW_DOOR','0','0','是否为门户系统，1表示是，2表示否');
 END IF;
 ...
 SELECT COUNT(*) into countt FROM CWM_SYS_PARAMETER WHERE NAME = 'FILE_SECURITY_MAP';
	IF(countt=0) THEN  
		insert into CWM_SYS_PARAMETER(ID,NAME,DATATYPE,VALUE,DESCRIPTION)
		values('4010','FILE_SECURITY_MAP','0','3:公开-6:秘密-9:机密-12:绝密','文件密级属性键值对的配置，需要与业务表定义中的“密级管理”控制保持一致，格式为“3:公开-6:秘密-9:机密-12:绝密”');
 END IF;
...
UPDATE CWM_SYS_RES SET DEFAULTURL='/oa/system/sysParameter/manage.do'  WHERE RESID='953';
...
IF(countt=0) THEN
	...
	-- 给系统管理员分配权限----
	SELECT RESID into @res_id FROM CWM_SYS_RES WHERE ALIAS = 'xtjk';
	insert into CWM_SYS_ROLE_RES(ROLERESID,ROLEID,RESID) select MAX(ROLERESID)+1,-1,@res_id FROM CWM_SYS_ROLE_RES;
END IF;
...
IF(countt=0) THEN
	...
	-- 给系统管理员分配权限----
	SELECT RESID into @res_id FROM CWM_SYS_RES WHERE ALIAS = 'sjyxxjk';
	insert into CWM_SYS_ROLE_RES(ROLERESID,ROLEID,RESID) select MAX(ROLERESID)+1,-1,@res_id FROM CWM_SYS_ROLE_RES;
END IF;

-- ------------更改JMS消息监控节点位置------2017-8-11 by liubo------------------------ --
UPDATE CWM_SYS_RES SET PARENTID='4011',SN=1  WHERE ALIAS='JMSM';

-- -----------系统中若无三员默认新增三员(system、right、check)------2017-10-10  by liubo-------------------------------
 SELECT COUNT(*) into countt FROM cwm_sys_user WHERE USERID = '-1';
	IF(countt=0) THEN  
		insert into cwm_sys_user(USERNAME,PASSWORD,FULLNAME,SEX,EMAIL,STATUS,DELFLAG,USERID,SECURITY)
		values('system','54B53072540EEEB8F8E9343E71F28176','系统管理员',1,'system@ibms.com',1,0,-1,'12');
 END IF;
 SELECT COUNT(*) into countt FROM cwm_sys_user WHERE USERID = '-2';
	IF(countt=0) THEN  
		insert into cwm_sys_user(USERNAME,PASSWORD,FULLNAME,SEX,EMAIL,STATUS,DELFLAG,USERID,SECURITY)
		values('right','7C4F29407893C334A6CB7A87BF045C0D','安全保密员',1,'right@cssrc.com.cn',1,0,-2,'12');
 END IF;
 SELECT COUNT(*) into countt FROM cwm_sys_user WHERE USERID = '-3';
	IF(countt=0) THEN  
		insert into cwm_sys_user(USERNAME,PASSWORD,FULLNAME,SEX,EMAIL,STATUS,DELFLAG,USERID,SECURITY)
		values('check','0BA4439EE9A46D9D9F14C60F88F45F87','安全审计员',1,'check@cssrc.com.cn',1,0,-3,'12');
 END IF;

-- ----------系统新增实施人员-------2017-8-15  by liubo------------------------- --
 SELECT COUNT(*) into countt FROM cwm_sys_user WHERE USERID = '-4';
	IF(countt=0) THEN  
		insert into cwm_sys_user(USERNAME,PASSWORD,FULLNAME,SEX,STATUS,DELFLAG,USERID,SECURITY)
		values('implement','4B9E4636494461CF31094E9A16F456FE','系统实施人员',1,1,0,-4,'12');
 END IF;
 
-- ----------系统新增实施人员角色-------2017-8-15  by liubo------------------------- --
 SELECT COUNT(*) into countt FROM cwm_sys_role WHERE ROLEID = '-4';
	IF(countt=0) THEN  
		insert into cwm_sys_role(ROLEID,ROLENAME,ROLEDESC,STATUS,ALIAS)
		values(-4,'系统实施员','系统实施员',1,'系统实施员');
 END IF;
 
-- ----------将系统实施人员加入系统实施员角色-------2017-8-15  by liubo------------------------- --
 SELECT COUNT(*) into countt FROM cwm_sys_role_user WHERE ROLEID = '-4';
	IF(countt=0) THEN  
		insert into cwm_sys_role_user(ROLEID,USERID,USERROLEID)
		values(-4,-4,-4);
 END IF;

-- ----------新增用户角色分配功能点--2017-8-16  by liubo---- --
SELECT COUNT(*) into countt FROM CWM_SYS_RES WHERE RESID = '4014';
IF(countt=0) THEN
	insert into CWM_SYS_RES(RESID,RESNAME, ALIAS, SN,ICON, PARENTID, DEFAULTURL, ISFOLDER,ISDISPLAYINMENU,ISOPEN,ISNEWOPEN,PATH)
...
SELECT COUNT(*) into countt FROM CWM_SYS_RES WHERE RESID = '4015';
IF(countt=0) THEN
	insert into CWM_SYS_RES(RESID,RESNAME, ALIAS, SN,ICON, PARENTID, DEFAULTURL, ISFOLDER,ISDISPLAYINMENU,ISOPEN,ISNEWOPEN,PATH)
...
SELECT COUNT(*) into countt FROM CWM_SYS_RES WHERE RESID = '4016';
IF(countt=0) THEN
	insert into CWM_SYS_RES(RESID,RESNAME, ALIAS, SN,ICON, PARENTID, DEFAULTURL, ISFOLDER,ISDISPLAYINMENU,ISOPEN,ISNEWOPEN,PATH)
...
UPDATE CWM_SYS_ORG SET PATH=concat('.',PATH)  WHERE PATH is not null and PATH not like '.%';
-- ------------系统数据字典path数据格式修改------2017-9-13 by liubo------------------------ --
UPDATE CWM_SYS_DIC SET NODEPATH=concat('.',NODEPATH)  WHERE NODEPATH is not null and NODEPATH not like '.%';
UPDATE CWM_SYS_GLTYPE SET NODEPATH=concat('.',NODEPATH)  WHERE NODEPATH is not null and NODEPATH not like '.%';

-- --------------do my sql end--------------
COMMIT;
END;
CALL prod_data_liubo(@spacename);