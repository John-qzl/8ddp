DROP PROCEDURE IF EXISTS prod_data_songchen;
CREATE PROCEDURE prod_data_songchen(spacename VARCHAR(100))
BEGIN
DECLARE resnum NUMERIC;
DECLARE theme_val VARCHAR(1000);
-- ---定义异常处理HANDLER----
...
IF(resnum=0)THEN
	INSERT into CWM_SYS_PARAMETER(ID,NAME,DATATYPE,`VALUE`,DESCRIPTION,`TYPE`)value(6002,'SOLRSERVER',1,'0#0#[doc,docx,pdf,ppt,pptx,xls,xlsx,txt]','是否开启文件搜索#是否开启数据库搜索#[可搜索的文件类型]--0不开启，1开启，不需要的文件类型请删除','辅助参数');
END IF;
IF(resnum=1)THEN
