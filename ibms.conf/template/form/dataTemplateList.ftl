...
<#macro dicQuerySelect field formatData>
  <#if formatData?if_exists >
      <input lablename="Q_${field.na}_${field.qt}" class="dicComboTree" nodeKey="${formatData}" validate="{empty:false,required:true}" name="Q_${field.na}_${field.qt}" height="200" width="170"/>
  </#if>
</#macro>
<#-- 外键控件宏 -->
...
<#macro genManage mindex manage managePermission actionUrl pk>
  ...
  <#elseif manage.name == 'detail' >
    <#if managePermission['${mindex}_detail']>
      <a class="link detail" action="${actionUrl.detail}&__pk__=${pk}" onclick="openLinkDialog({keyName:'${mindex}_${manage.name}',scope:this,width:1000,height:800,isFull:false})" href="####">${manage.desc}</a>
    </#if>
  <#--复杂表单明细-->
    <#elseif manage.name == 'complexDetail' >
      <#if managePermission['${mindex}_complexDetail']>
        <a class="link complexDetail" action="${actionUrl.complexDetail}&__pk__=${pk}" onclick="multiTabView({keyName:'${mindex}_${manage.name}'scope:this,isFull:false,openType:'frist',formKey:'${bpmDataTemplate.formKey}'})" href="####">${manage.desc}</a>
      </#if>
   <#--附件-->
  ...
  <#elseif manage.name == 'start' >
    <#if managePermission['${mindex}_start']>
      ...
      <a class="link run" action="${actionUrl.start}&businessKey=${pk}"  onclick="openLinkDialog({keyName:'${mindex}_${manage.name}',scope:this,width:1000,height:800,isFull:false,isStart:${isStart}})" href="####">${manage.desc}</a>
    </#if>
  </#if>
</#macro>
<#--顶部按钮-->
  ...
  <#elseif manage.name == 'start' >
    ...
    <#elseif manage.name == 'feedBack' >
      <#if managePermission.feedBack>
        <div class="group"><a class="link feedBack" action="${actionUrl.print}"  onclick="feedBack()"  href="####">${manage.desc}</a></div>
