			...
			<table cellpadding="1" cellspacing="1" class="table-grid table-list">
				...
				<tr class="<#noparse><#if data_index % 2 == 0>odd</#if><#if data_index % 2 == 1>even</#if></#noparse>">
					...
					<td>
						...
						<a href="detail.do?formKey=${formKey}&flowRunId=<#noparse>${data.flowrunid_}</#noparse>&pkValue=<#noparse>${data.</#noparse>${pkField}}" class="link detail">明细</a>
						<#noparse><#if data.flowrunid_?? ></#noparse> 
							<a href="../../bpm/processRun/get.do?runId=<#noparse>${data.flowrunid_}</#noparse>" target="_blank" class="link dataList">查看流程数据</a>
						<#noparse></#if></#noparse>
						<#noparse><#if data.flowrunid_?? ></#noparse>
						<#noparse><#else></#noparse> 
							<#noparse><#if data.defid_?? && data.defid_!='0' ></#noparse>
								<a href="####" onclick="javascript:jQuery.openFullWindow('../../bpm/task/startFlowForm.do?defId=<#noparse>${data.defid_}</#noparse>&businessKey=<#noparse>${data.</#noparse>${pkField}}')"  class="link dataList">启动流程</a>
							<#noparse></#if></#noparse>
						<#noparse></#if></#noparse>
					</td>
				</tr>
