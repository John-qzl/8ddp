...
<xsl:stylesheet version="1.0"
	...
	xmlns:bg="bpm.graphic" xmlns:fg="bpm.graphic" xmlns:activiti="http://activiti.org/bpmn"
	xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:omgdc="http://www.omg.org/spec/DD/20100524/DC"
	xmlns:omgdi="http://www.omg.org/spec/DD/20100524/DI" xmlns:ciied="com.ibm.ilog.elixir.diagram"
	xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:calc="xalan://com.cssrc.ibms.core.activity.graph.TransformUtil"
	xmlns:ht="http://www.cssrc.com.cn/BPMN20EXT" extension-element-prefixes="calc">
	<xsl:param name="id" />
	<xsl:param name="name" />
	...
	<xsl:template match="/">
		<definitions xmlns="http://www.omg.org/spec/BPMN/20100524/MODEL"
			...
			xmlns:ht="http://www.cssrc.com.cn/BPMN20EXT" xsi:schemaLocation="http://www.cssrc.com.cn/BPMN20EXT BPMN20EXT.xsd"
			...
			<!-- 生成图形的坐标的描述 -->
			<bpmndi:BPMNDiagram>
				<xsl:attribute name="id">BPMNDiagram_<xsl:value-of
					select="$id"></xsl:value-of>
				</xsl:attribute>
				<bpmndi:BPMNPlane>
					<xsl:attribute name="bpmnElement"><xsl:value-of
						select="$id"></xsl:value-of>
					</xsl:attribute>
					<xsl:attribute name="id">BPMNPlane_<xsl:value-of
						select="$id"></xsl:value-of>
					</xsl:attribute>
	...
	<xsl:template match="/diagram">
		...
		<!-- 生成不在泳道池内的组件 -->
		<xsl:if
			test="contains($comp,'Event') or contains($comp,'Task') or contains($comp,'SequenceFlow') or contains($comp,'SubProcess') or contains($comp,'Gateway')">
			<process>
	...
	<xsl:template match="bg:HPool">
		<process>
			...
			<laneSet>
				<xsl:attribute name="id">laneSet_process_<xsl:value-of
					select="@id"></xsl:value-of>
   				</xsl:attribute>
				<xsl:for-each select="bg:HLane">
					<lane>
						...
						<xsl:for-each
							select="child::*[starts-with(name(.),'bg:') and name(.)!='bg:SequenceFlow']">
							<flowNodeRef>
								<xsl:value-of select="@id"></xsl:value-of>
							</flowNodeRef>
						</xsl:for-each>
		...
		<xsl:for-each select=".">
			...
			<sequenceFlow>
				<xsl:attribute name="sourceRef">
					<xsl:value-of
					select="//ciied:Port[@id=$sourceRef]/parent::*/parent::*/@id"></xsl:value-of>
				</xsl:attribute>
				<xsl:attribute name="targetRef">
					<xsl:value-of
					select="//ciied:Port[@id=$targetRef]/parent::*/parent::*/@id"></xsl:value-of>
				</xsl:attribute>
				<xsl:call-template name="setAttrubute">
					<xsl:with-param name="obj" select="."></xsl:with-param>
				</xsl:call-template>
			...
			<collaboration id="Collaboration">
				<xsl:for-each select="child::*[contains(name(.),'Pool')]">
					<participant>
						...
						<xsl:attribute name="processRef"><xsl:value-of
							select="$id"></xsl:value-of>
		   				</xsl:attribute>
					</participant>
				</xsl:for-each>
	...
	<xsl:template name="transition">
		<xsl:for-each select="//bg:SequenceFlow">
			<bpmndi:BPMNEdge>
				...
				<xsl:attribute name="id">BPMNEdge_<xsl:value-of
				...
				<xsl:choose>
					...
					<xsl:when test="$parentName='bg:SubProcess'">
						...
						<xsl:variable name="fx1"
							select="calc:add(string($fx),string($fParent/@x))"></xsl:variable>
						<xsl:variable name="fy1"
							select="calc:add(string($fy),string($fParent/@y))"></xsl:variable>

						<xsl:variable name="tx1"
							select="calc:add(string($tx),string($tParent/@x))"></xsl:variable>
						<xsl:variable name="ty1"
							select="calc:add(string($ty),string($tParent/@y))"></xsl:variable>
						...
						<xsl:variable name="intermediatePoints">
							<xsl:for-each select="./intermediatePoints/child::*[name(.)='fg:Point']">
								<xsl:value-of
									select="concat(calc:add(string($fParent/@x),calc:add('10',@x)),':',calc:add(string($fParent/@y),calc:add('28',@y)),',')"></xsl:value-of>
							</xsl:for-each>
						</xsl:variable>
						<!-- 通过迭代 处理多层内嵌子流程的情况 -->
					...
					<xsl:when test="$parentName='bg:HLane'">
						...
						<xsl:variable name="fParentYY">
							<xsl:choose>
								<!-- 用于计算在两个泳道之间连线的y算法 -->
								<xsl:when
									test="string($fParent/@y)!='' and string($fParent/@y)=string($tParent/@y) ">
									<xsl:value-of select="string($fParent/@height)"></xsl:value-of>
								</xsl:when>
								<xsl:when
									test="(string($fParent/@y)!='' or string($tParent/@y)!='') and string($fParent/@y)!=string($tParent/@y) ">
						...
						<xsl:variable name="intermediatePoints">
							<xsl:for-each select="./intermediatePoints/child::*[name(.)='fg:Point']">
								<xsl:value-of
									select="concat(calc:add(calc:add(string($fGrandparent/@graphMarginLeft),'0'),calc:add('0',calc:add(calc:add($fParentXX,calc:add('0',string($fGrandparent/@left))),calc:add('0',@x)))),':',calc:add(string($fGrandparent/@top),calc:add('0',calc:add($fParentYY,@y))),',')"></xsl:value-of>
							</xsl:for-each>
						</xsl:variable>
						<xsl:variable name="fx1"
							select="calc:add(string($fx),string($fGrandparent/@x))"></xsl:variable>
						...
						<xsl:variable name="fy">
							<xsl:value-of
								select="calc:add(calc:accumulate($preSibsHeight,$fGrandparentY),string($fromFlowEl/@y))"></xsl:value-of>
						</xsl:variable>
						<xsl:variable name="tx1"
							select="calc:add(string($tx),string($tGrandparent/@x))"></xsl:variable>
						...
						<xsl:variable name="ty">
							<xsl:value-of
								select="calc:add(calc:accumulate($tPreSibsHeight,$tGrandparentY),string($toFlowEl/@y))"></xsl:value-of>
						</xsl:variable>
					...
					<xsl:when test="$parentName='bg:VLane'">
						...
						<xsl:variable name="fParentXX">
							<xsl:choose>
								...
								<xsl:when
									test="(string($fParent/@x)!='' or string($tParent/@x)!='') and string($tParent/@x)!=string($tParent/@x) ">
									<xsl:value-of select="'2'"></xsl:value-of>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="'0'"></xsl:value-of>
						...
						<xsl:variable name="intermediatePoints">
							<xsl:for-each select="./intermediatePoints/child::*[name(.)='fg:Point']">
								<xsl:value-of
									select="concat(calc:add(string($fGrandparent/@left),calc:add('0',calc:add($fParentXX,@x))),':',calc:add(calc:add(string($fGrandparent/@graphMarginTop),'0'),calc:add('0',calc:add(calc:add($fParentYY,calc:add('0',string($fGrandparent/@top))),calc:add('0',@y)))),',')"></xsl:value-of>
							</xsl:for-each>
						</xsl:variable>
						<xsl:variable name="fy1"
							select="calc:add(string($fy),string($fGrandparent/@y))"></xsl:variable>
						...
						<xsl:variable name="fx">
							<xsl:value-of
								select="calc:add(calc:accumulate($preSibsWidth,$fGrandparentX),string($fromFlowEl/@x))"></xsl:value-of>
						</xsl:variable>
						<xsl:variable name="ty1"
							select="calc:add(string($ty),string($tGrandparent/@y))"></xsl:variable>
						...
						<xsl:variable name="tx">
							<xsl:value-of
								select="calc:add(calc:accumulate($tPreSibsWidth,$tGrandparentX),string($toFlowEl/@x))"></xsl:value-of>
						</xsl:variable>
						<!-- <xsl:value-of select="calc:calc($fName,$fx,$fy,$fW,$fH,$fHOffset,$fVOffset,$fDirX,$fDirY,$tName,$tx,$ty,$tW,$tH,$tHOffset,$tVOffset,$tDirX,$tDirY 
							)" /> -->
						<xsl:value-of
	...
	<xsl:template name="diagram_block">
		...
		<xsl:for-each select="$elName">
			<bpmndi:BPMNShape>
				...
				<xsl:attribute name="id">BPMNShape_<xsl:value-of
					select="@id"></xsl:value-of>
				</xsl:attribute>
					...
					<xsl:choose>
						...
						<xsl:when test="$name='bg:HPool'">
							...
							<xsl:attribute name="height">
								...
								<xsl:value-of
								select="calc:add(string(@height),string(-@gap*(calc:splitLength($lanes,',') -1)))"></xsl:value-of>
							</xsl:attribute>
						</xsl:when>
						...
						<xsl:when test="$name='bg:VPool'">
							<xsl:attribute name="width">
								...
								<xsl:value-of
								select="calc:add(string(@width),string(-@gap*(calc:splitLength($lanes,',') -1)))"></xsl:value-of>
							</xsl:attribute>
							<xsl:attribute name="height">
								<xsl:value-of select="@height"></xsl:value-of>
							</xsl:attribute>
					...
					<xsl:choose>
						...
						<xsl:when test="$parentName='bg:HLane'">
							...
							<xsl:variable name="x1"
								select="calc:add(string($x),string($grandparent/@x))"></xsl:variable>
							<xsl:variable name="x" select="calc:add(string($x1),'40')"></xsl:variable>
							<xsl:attribute name="x"><xsl:value-of
								select="$x"></xsl:value-of></xsl:attribute>
							<xsl:attribute name="y">
								<xsl:variable name="grandparentY" select="$grandparent/@y"></xsl:variable>
								<xsl:variable name="preSibsHeight">
									<xsl:for-each
								select="parent::*/preceding-sibling::*[name(.)='bg:HLane']">
										<xsl:value-of select="@height"></xsl:value-of>,</xsl:for-each>
								</xsl:variable>
								<xsl:value-of
								select="calc:add(calc:accumulate($preSibsHeight,$grandparentY),@y)"></xsl:value-of>
							</xsl:attribute>
						</xsl:when>
						<xsl:when test="$parentName='bg:VLane'">
							<xsl:variable name="grandparent" select="./parent::*/parent::*"></xsl:variable>
							<xsl:variable name="y1"
								select="calc:add(string($y),string($grandparent/@y))"></xsl:variable>
							...
							<xsl:attribute name="x">
								...
								<xsl:variable name="preSibsWidth">
									<xsl:for-each
								select="parent::*/preceding-sibling::*[name(.)='bg:VLane']">
										<xsl:value-of select="concat(@width,',')"></xsl:value-of>
									</xsl:for-each>
								</xsl:variable>
								<xsl:value-of
								select="calc:add(calc:accumulate($preSibsWidth,$grandparentX),@x)"></xsl:value-of>
							</xsl:attribute>
						</xsl:when>
						<xsl:when test="$parentName='bg:SubProcess'">
						...
						<xsl:otherwise>
							...
							<xsl:attribute name="y"><xsl:value-of
								select="$y"></xsl:value-of></xsl:attribute>
						</xsl:otherwise>
					</xsl:choose>
		...
		<xsl:choose>
			...
			<xsl:when test="name($fParent/parent::*)='bg:HLane'">
				...
				<xsl:variable name="getAllPreVlans">
					<xsl:for-each
						select="$fParent/parent::*/preceding-sibling::*[name(.)='bg:HLane']">
						<xsl:choose>
							<xsl:when test="@y!='' ">
				...
				<xsl:variable name="getAllAfterVlans">
					<xsl:for-each
						select="$fParent/parent::*/following-sibling::*[name(.)='bg:HLane']">
						<xsl:choose>
							<xsl:when test="@y!='' ">
				...
				<xsl:variable name="fParentXX">
					<xsl:choose>
						<xsl:when
							test="string($fGrandparent/@y)!='' and string($tGrandparent/@y)!='' and name(./parent::*/parent::*)!='bg:SubProcess'">
							<xsl:value-of select="'0'"></xsl:value-of>
						</xsl:when>
						<xsl:when
							test="(string($fGrandparent/@y)!='' and string($tGrandparent/@y)!='' and name(./parent::*/parent::*)='bg:SubProcess') or (string($fGrandparent/@y)='' and string($tGrandparent/@y)='' and name(./parent::*/parent::*)='bg:SubProcess')">
							<xsl:value-of
								select="calc:add(string($fGrandparent/@graphMarginLeft),calc:add(-string($fGrandparent/@poolMarginLeft),calc:add(string(./parent::*/@x),'-2')))"></xsl:value-of>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="'0'"></xsl:value-of>
						</xsl:otherwise>
				...
				<xsl:variable name="fParentYY">
					<xsl:choose>
						<xsl:when
							test="string($fGrandparent/@y)!='' and string($tGrandparent/@y)!='' and name(./parent::*/parent::*)!='bg:SubProcess'"><!-- string($fGrandparent/@graphMarginLeft) -->
							<xsl:value-of select="calc:add(string($fGrandparent/@y),'2')"></xsl:value-of>
						</xsl:when>
						<xsl:when
							test="string($fGrandparent/@y)!='' and string($tGrandparent/@y)!='' and name(./parent::*/parent::*)='bg:SubProcess'">
							<xsl:value-of
								select="calc:add(string(./parent::*/@y),calc:add(string(./parent::*/parent::*/@y),calc:add(string($fParent/parent::*/@poolMarginTop),calc:add(calc:min(string($fGrandparent/@y),string($fGrandparent/@height)),calc:add($yFloat,'-2')))))"></xsl:value-of>
						</xsl:when>
						<xsl:when
							test="string($fGrandparent/@y)='' and string($tGrandparent/@y)=''   and name(./parent::*/parent::*)='bg:SubProcess'">
							<xsl:value-of
				...
				<!-- <xsl:variable name="concatYY" select="calc:add(calc:add(string($fGreatGrandparent/@top),'0'),calc:add(string($fParent/parent::*/@poolMarginTop),calc:add(string($fParentYY),calc:add('0','0'))))"></xsl:variable> 
					...
					<xsl:variable name="intermediatePoints"> <xsl:for-each select="./intermediatePoints/child::*[name(.)='fg:Point']">string($fGreatGrandparent/@left) 
					string($fGreatGrandparent/@graphMarginTop) <xsl:value-of select="concat(calc:add($concatXX,calc:add(string($fParent/@x),@x)),':',calc:add($concatYY,calc:add(string($fParent/@y),@y)),',')"></xsl:value-of> 
					</xsl:for-each> </xsl:variable> -->

				<xsl:variable name="concatXX"
					select="calc:add(calc:add(string($fGreatGrandparent/@graphMarginLeft),string($fGreatGrandparent/@poolMarginLeft)),calc:add('0',calc:add(calc:add('0',calc:add(string($fParent/parent::*/@poolMarginLeft),string($fGreatGrandparent/@left))),calc:add('8',$fParentXX))))"></xsl:variable>
				<xsl:variable name="concatYY"
					select="calc:add(calc:add(string($fGreatGrandparent/@top),string($fGreatGrandparent/@graphMarginTop)),calc:add(string($fParent/parent::*/@poolMarginTop),calc:add(string($fParentYY),calc:add(string($fParent/@y),'-2'))))"></xsl:variable>
				<xsl:variable name="intermediatePoints">
					<xsl:for-each select="./intermediatePoints/child::*[name(.)='fg:Point']"><!-- string($fParent/@y) -->
						<xsl:value-of
							select="concat(calc:add($concatXX,calc:add(string($fParent/@x),@x)),':',calc:add($concatYY,calc:add('0',@y)),',')"></xsl:value-of>
					</xsl:for-each>
				</xsl:variable>

				<xsl:variable name="fx1"
					select="calc:add(string($fx),string($fGreatGrandparent/@x))"></xsl:variable>
				<xsl:variable name="fx" select="calc:add(string($fx1),'40')"></xsl:variable>
				...
				<xsl:variable name="preSibsHeight">
					<xsl:for-each
						select="$fGrandparent/preceding-sibling::*[name(.)='bg:HLane']">
						<xsl:value-of select="concat(@height,',')"></xsl:value-of>
					</xsl:for-each>
				</xsl:variable>
				<xsl:variable name="fy">
					<xsl:value-of
						select="calc:add(calc:accumulate($preSibsHeight,$fGreatGrandparentY),$fy)"></xsl:value-of>
				</xsl:variable>

				<xsl:variable name="tx1"
					select="calc:add(string($tx),string($tGreatGrandparent/@x))"></xsl:variable>
				<xsl:variable name="tx" select="calc:add(string($tx1),'40')"></xsl:variable>
				...
				<xsl:variable name="tPreSibsHeight">
					<xsl:for-each
						select="$tGrandparent/preceding-sibling::*[name(.)='bg:HLane']">
						<xsl:value-of select="concat(@height,',')"></xsl:value-of>
					</xsl:for-each>
				</xsl:variable>
				...
				<xsl:variable name="ty">
					<xsl:value-of
						select="calc:add(calc:accumulate($tPreSibsHeight,$tGreatGrandparentY),string($ty))"></xsl:value-of>
				</xsl:variable>
			...
			<xsl:when test="name($fParent/parent::*)='bg:VLane'">
				...
				<xsl:variable name="fy1"
					select="calc:add(string($fy),string($fGreatGrandparent/@y))"></xsl:variable>
				<xsl:variable name="fy" select="calc:add(string($fy1),'40')"></xsl:variable>
				<!-- 用于获取当前泳道前面的泳道的x坐标 以便于计算连线的x轴偏移值 -->
				<xsl:variable name="getAllPreVlans">
					<xsl:for-each
						select="$fParent/parent::*/preceding-sibling::*[name(.)='bg:VLane']">
						<xsl:value-of select="concat(@x,',')"></xsl:value-of>
					</xsl:for-each>
				</xsl:variable>
				<xsl:variable name="getAllAfterVlans">
					<xsl:for-each
						select="$fParent/parent::*/following-sibling::*[name(.)='bg:VLane']">
						<xsl:choose>
							<xsl:when test="@x!=''">
								<xsl:value-of select="concat(@x,',')"></xsl:value-of>
							</xsl:when>
				...
				<xsl:variable name="fParentXX">
					<xsl:choose>
						<xsl:when
							test="string($fGrandparent/@x)!='' and string($tGrandparent/@x)!='' and name(./parent::*/parent::*)!='bg:SubProcess'"><!-- string($fGrandparent/@graphMarginLeft) -->
							<xsl:value-of select="calc:add(string($fGrandparent/@x),'2')"></xsl:value-of>
						</xsl:when>
						<xsl:when
							test="string($fGrandparent/@x)='' and string($tGrandparent/@x)='' and name(./parent::*/parent::*)='bg:SubProcess'">
							<xsl:value-of
								select="calc:add(string(./parent::*/@x),calc:add(string(./parent::*/parent::*/@x),calc:add(-string($fParent/parent::*/@poolMarginLeft),calc:add(calc:min(string($fGrandparent/@x),string($fGrandparent/@width)),$xFloat))))"></xsl:value-of>
						</xsl:when>
						<xsl:when
							test="string($fGrandparent/@x)!='' and string($tGrandparent/@x)!=''and name(./parent::*/parent::*)='bg:SubProcess' and $getAllAfterVlans!=''">
							<xsl:value-of
								select="calc:add(string(./parent::*/@x),calc:add(string(./parent::*/parent::*/@x),calc:add(string($fParent/parent::*/@poolMarginLeft),calc:add(calc:min(string($fGrandparent/@x),string($fGrandparent/@width)),$xFloat))))"></xsl:value-of>
						</xsl:when>
						<xsl:when
				...
				<xsl:variable name="fParentYY">
					<xsl:choose>
						...
						<xsl:when
							test="(string($fGrandparent/@x)!='' and string($tGrandparent/@x)!='' and name(./parent::*/parent::*)='bg:SubProcess') or (string($fGrandparent/@x)='' and string($tGrandparent/@x)='' and name(./parent::*/parent::*)='bg:SubProcess')">
							<xsl:value-of
								select="calc:add(string($fGrandparent/@graphMarginTop),calc:add(string($fGrandparent/@poolMarginTop),calc:add(string(./parent::*/@y),'-2')))"></xsl:value-of>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="'0'"></xsl:value-of>
						</xsl:otherwise>
				...
				<xsl:variable name="concatXX"
					select="calc:add(calc:add(string($fGreatGrandparent/@left),'0'),calc:add(string($fParent/parent::*/@poolMarginLeft),calc:add(string($fParentXX),calc:add('0','0'))))"></xsl:variable>
				<xsl:variable name="concatYY"
					select="calc:add(calc:add(string($fGreatGrandparent/@graphMarginTop),string($fGreatGrandparent/@poolMarginTop)),calc:add('0',calc:add(calc:add(string($fParent/parent::*/@graphMarginTop),calc:add(string($fParent/parent::*/@poolMarginTop),string($fGreatGrandparent/@top))),calc:add('8',$fParentYY))))"></xsl:variable>
				<xsl:variable name="intermediatePoints">
					<xsl:for-each select="./intermediatePoints/child::*[name(.)='fg:Point']"><!-- string($fGreatGrandparent/@left) 
							string($fGreatGrandparent/@graphMarginTop) -->
						<xsl:value-of
							select="concat(calc:add($concatXX,calc:add(string($fParent/@x),@x)),':',calc:add($concatYY,calc:add(string($fParent/@y),@y)),',')"></xsl:value-of>
					</xsl:for-each>
				</xsl:variable>
				<!-- 计算连线前端组件的X坐标 -->
				...
				<xsl:variable name="preSibsWidth">
					<xsl:for-each
						select="$fGrandparent/preceding-sibling::*[name(.)='bg:VLane']">
						<xsl:value-of select="concat(@width,',')"></xsl:value-of>
					</xsl:for-each>
				</xsl:variable>
				<xsl:variable name="fx">
					<xsl:value-of
						select="calc:add(calc:accumulate($preSibsWidth,$fGreatGrandparentX),string($fx))"></xsl:value-of>
				</xsl:variable>
				<xsl:variable name="ty1"
					select="calc:add(string($ty),string($tGreatGrandparent/@y))"></xsl:variable>
				<xsl:variable name="ty" select="calc:add(string($ty1),'40')"></xsl:variable>
				...
				<xsl:variable name="tPreSibsWidth">
					<xsl:for-each
						select="$tGrandparent/preceding-sibling::*[name(.)='bg:VLane']">
						<xsl:value-of select="concat(@width,',')"></xsl:value-of>
					</xsl:for-each>
				</xsl:variable>
				...
				<xsl:variable name="tx">
					<xsl:value-of
						select="calc:add(calc:accumulate($tPreSibsWidth,$tGreatGrandparentX),$tx)"></xsl:value-of>
				</xsl:variable>
				<!-- <xsl:value-of select="calc:calc($fName,$fx,$fy,$fW,$fH,$fHOffset,$fVOffset,$fDirX,$fDirY,$tName,$tx,$ty,$tW,$tH,$tHOffset,$tVOffset,$tDirX,$tDirY 
					)" /> -->
				<xsl:value-of
			...
			<xsl:when test="name($fParent/parent::*)='bg:SubProcess'">
				...
				<xsl:variable name="intermediatePoints">
					<xsl:for-each select="./intermediatePoints/child::*[name(.)='fg:Point']">
						<xsl:value-of
							select="concat(calc:add($parentFloatX,calc:add('160',@x)),':',calc:add($parentFloatY,calc:add('56',@y)),',')"></xsl:value-of>
					</xsl:for-each>
				</xsl:variable>
	...
	<xsl:template name="setSubProcessParam">
		...
		<xsl:variable name="x1"
			select="calc:add(string($x),string($parent/@x))"></xsl:variable>
		<xsl:variable name="y1"
			select="calc:add(string($y),string($parent/@y))"></xsl:variable>
		...
		<xsl:choose>
			<xsl:when test="$grandParentName='bg:HLane'">
				...
				<xsl:variable name="x1"
					select="calc:add(string($x),string($greatGrandparent/@x))"></xsl:variable>
				<xsl:variable name="x" select="calc:add(string($x1),'40')"></xsl:variable>
				...
				<xsl:variable name="preSibsHeight">
					<xsl:for-each
					select="$grandparent/preceding-sibling::*[name(.)='bg:HLane']">
						<xsl:value-of select="concat(@height,',')"></xsl:value-of>
					</xsl:for-each>
				</xsl:variable>
				<xsl:value-of
			...
			<xsl:when test="$grandParentName='bg:VLane'">
				...
				<xsl:variable name="greatGrandparent" select="$grandparent/parent::*"></xsl:variable>
				<xsl:variable name="y1"
					select="calc:add(string($y),string($greatGrandparent/@y))"></xsl:variable>
				<xsl:variable name="y" select="calc:add(string($y1),'40')"></xsl:variable>
				...
				<xsl:variable name="preSibsWidth">
					<xsl:for-each
					select="$grandparent/preceding-sibling::*[name(.)='bg:VLane']">
						<xsl:value-of select="concat(@width,',')"></xsl:value-of>
					</xsl:for-each>
				</xsl:variable>
				<xsl:value-of
					select="calc:add(calc:accumulate($preSibsWidth,$grandparentX),calc:add('0',$x))"></xsl:value-of>
			</xsl:attribute>
			</xsl:when>
			...
			<xsl:when test="$grandParentName='bg:SubProcess'">
				...
				<xsl:variable name="fParentXX">
					<xsl:choose>
						...
						<xsl:when test="name($grandGreatParent)='bg:VLane'">
							<xsl:value-of
								select="calc:add(string($grandGreatParent/@x),string($grandGreatParent/parent::*/@left))"></xsl:value-of>
						</xsl:when>
						<xsl:when test="name($grandGreatParent)='bg:HLane'">
							<xsl:value-of
								select="calc:add(string($grandGreatParent/parent::*/@graphMarginLeft),calc:add(string($grandGreatParent/parent::*/@left),string($grandGreatParent/@graphMarginLeft)))"></xsl:value-of>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="'0'"></xsl:value-of>
						</xsl:otherwise>
				...
				<xsl:variable name="fParentYY">
					<xsl:choose>
						...
						<xsl:when test="name($grandGreatParent)='bg:VLane'">
							<xsl:value-of
								select="string($grandGreatParent/parent::*/@graphMarginTop)"></xsl:value-of>
						</xsl:when>
						<xsl:when test="name($grandGreatParent)='bg:HLane'">
							<xsl:value-of
								select="calc:add(string($grandGreatParent/@y),string($grandGreatParent/parent::*/@top))"></xsl:value-of>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="'0'"></xsl:value-of>
						</xsl:otherwise>
				...
				<xsl:call-template name="setSubProcessParam">
					<xsl:with-param name="x"
						select="calc:add($fParentXX,calc:add('0',$x))" />
			...
			<xsl:when test="name($grandParentName)='bg:SubProcess'">
				...
				<xsl:variable name="fParentXX">
					<xsl:choose>
						...
						<xsl:when test="name($grandGreatParent/parent::*)='bg:VLane'">
							<xsl:value-of
								select="calc:add(string($grandGreatParent/parent::*/@x),string($grandGreatParent/parent::*/parent::*/@left))"></xsl:value-of>
						</xsl:when>
						<xsl:when test="name($grandGreatParent/parent::*)='bg:HLane'">
							<xsl:value-of
								select="calc:add(string($grandGreatParent/parent::*/parent::*/@graphMarginLeft),calc:add(string($grandGreatParent/parent::*/parent::*/@left),string($grandGreatParent/parent::*/@graphMarginLeft)))"></xsl:value-of>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="'0'"></xsl:value-of>
						</xsl:otherwise>
				...
				<xsl:variable name="fParentYY">
					<xsl:choose>
						...
						<xsl:when test="name($grandGreatParent/parent::*)='bg:VLane'">
							<xsl:value-of
								select="string($grandGreatParent/parent::*/parent::*/@graphMarginTop)"></xsl:value-of>
						</xsl:when>
						<xsl:when test="name($grandGreatParent/parent::*)='bg:HLane'">
							<xsl:value-of
								select="calc:add(string($grandGreatParent/parent::*/@y),string($grandGreatParent/parent::*/parent::*/@top))"></xsl:value-of>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="'0'"></xsl:value-of>
						</xsl:otherwise>
		...
		<xsl:choose>
			<xsl:when test="$XorY='x'">
				<xsl:variable name="number"
					select="calc:add(calc:add($number,'10'),string($parent/@x))"></xsl:variable>
			...
			<xsl:otherwise>
				<xsl:variable name="number"
					select="calc:add(calc:add($number,'28'),string($parent/@y))"></xsl:variable>
