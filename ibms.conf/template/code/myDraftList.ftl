...
<script type="text/javascript">
	function startFlow(obj,id,runId){
		...
		if(!linkObj.hasClass('disabled')) {
			...
			$.post("run.do?isList=1&<#if table.isExternal==0>id<#else>${table.pkField}</#if>="+id+"&runId="+runId,function(responseText){
				
				var obj = new com.ibms.form.ResultMessage(responseText);
				if (obj.isSuccess()) {
					$.ligerDialog.success("启动流程成功！", "成功", function(rtn) {
						this.close();
						window.location.href = "<#noparse>${ctx}</#noparse>/${system}/${package}/${classVar}/getMyDraft.do";
					});
				} else {
...
<body>
	<div class="panel">
		...
		<div class="panel-body">
		    ...
		    <display:table name="${classVar}List" id="${classVar}Item" requestURI="getMyDraftJson.do" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
