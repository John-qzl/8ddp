...
public class DateUtil {
	...
	public static SimpleDateFormat fileDateFormat = new SimpleDateFormat("yyyy-MM-dd-HHmmss");
	public static SimpleDateFormat normalDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	public static SimpleDateFormat yearDateFormat = new SimpleDateFormat("yyyy-MM-dd");
	/**
	...
	public static Date parseDate(String dateString) {
		Date date = null;
		try {
			date = DateUtils.parseDate(dateString, new String[] { "yyyy","yyyy-MM",
	...
	public static String getCurMonth() {
		...
		return now.get(Calendar.YEAR) + "" + longMonth;
	}
	...
	public static Date getCurrentDateDate(String format) throws ParseException {
		if (null == format) {
			return new Date();
		}
		Calendar calendar = Calendar.getInstance(TimeZone.getDefault());
	...
	public static Date parseDate(Date date, String format)
		...
		return getDate(getDateString(date, format));
	}
	...
	public static Date getDate(String str) throws ParseException {
		if (str == null || "".equals(str))
			return null;
		if (str.length() <= 4) {
			return getDate(str, "yyyy");
		}else if (str.length() <= 7) {
			return getDate(str, "yyyy-MM");
		} else if(str.length() <= 10) {
			return getDate(str, "yyyy-MM-dd");
		} else if (str.length() <= 14) {
			return getDate(str, "yyyy-MM-dd HH");
	...
	public static String getDateStr(String str,String pattern) {
		...
		try {
			...
			datestr = getDateString(date, pattern);
		}catch(Exception e) {
			e.printStackTrace();
		}
		return datestr;
	...
	/**
	 * @author Yangbo 2016.7.14 日期添加天数 返回Date类型(用于LoginLog登陆结束时间)
	 * @param date
	 * @param day
	 * @return
	 */
	...
	public static boolean isBefore(Date dateA,Date dateB){
		boolean flag = false;
		int aYear = dateA.getYear();
		int bYear = dateB.getYear();
		if(aYear < bYear){
			flag = true;
		}else if(aYear == bYear){
			int aMoth = dateA.getMonth();
			int bMoth = dateB.getMonth();
			if(aMoth < bMoth){
				flag = true;
			}else if(aMoth == bMoth){
				int aDate = dateA.getDate();
				int bDate = dateB.getDate();
				if(aDate <= bDate){
					flag = true;
				}
			}
	...
	/**
	 * 返回基础时间格式：yyyy-MM-dd HH:mm:ss
	 */
	public static String dateFormat(Date date){
		return normalDateFormat.format(date);
	}
	/**
	 * 返回文件后缀时间日期  格式 ： yyyy-MM-dd-HHmmss
	 * @param date
	 * @return
	 */
