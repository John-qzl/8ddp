		   ...
		   var $button=	$('.drop',$(box));
			...
			$button.live('click',function(){
				 if( $content.is(':hidden')){
					 ...
					 $content.show();
					 //展开时panelbody高度减小
					 var panelBody = $(box).parent().next(".panel-body");
					 var curHeight = panelBody.height() - $content.height();
					 panelBody.height(curHeight);
...
$(function(){
	$.extend({initFoldBox:function(){
		...
		try{
			...
			//高级搜索
			if($('.panel-search').find('.panel-complex-search').html()){
				$('.panel-search').css({
					'border':'none',
					'margin-top':'0px',
					'padding-top':'0px'
				});
				$('.panel-toolbar').css('padding-left','14px');
				$('#searchForm').css('padding-top','5px');
				$('.searchShift label').click(function(){
					var search_box = $(this).parent().next('.search_box');
					if(search_box.is(':hidden')){
						$(this).text('收缩');
						var search_box = $(this).parent().next('.search_box');
						search_box.show();
						var panelBodyHeight = $(this).parents('.panel-top').next('.panel-body').height();
						var search_box_height = search_box.height();
						$('.panel-body').height(panelBodyHeight-search_box_height);
					}else{
						$(this).text('展开');
						var search_box = $(this).parent().next('.search_box');
						search_box.hide();
