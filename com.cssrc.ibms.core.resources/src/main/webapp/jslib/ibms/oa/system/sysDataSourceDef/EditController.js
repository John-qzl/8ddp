var app = angular.module('app',['baseServices','SysDataSourceTemplateService','SysDataSourceDefService','commonListService','arrayToolService']);
app.controller("EditController",['$scope','BaseService','SysDataSourceTemplate','SysDataSourceDef','CommonListService','ArrayToolService',function($scope,BaseService,SysDataSourceTemplate,SysDataSourceDef,CommonListService,ArrayToolService){
	...
	//如果id为空，获取数据源模板
	if(id==""){
		SysDataSourceTemplate.getAll(function(data){
			$scope.sysDataSourceTemplateList=data;
		});
	}else{
	...
	$scope.save=function(prop){
		...
		SysDataSourceDef.save(prop,function(data){
			if(data.result==1){
				...
				//title,msg,detail
				$.ligerDialog.err("提示信息","保存数据源出错了",data.message);
			}
		});
	}
	
	//监听这个数据源对应的数据源模板，改变了就设置
	$scope.sysDataSourceTemplateChange=function(newVal){
	...
	$scope.checkConnection=function(prop){
		SysDataSourceDef.checkConnection(prop,function(data){
			alert(data.message);
		});	
	}
