/**
 * 这个js是sysQueryFieldAndSetting设置页面的显示字段和查询字段等js
 */
/**
 ...
 * 数据模板构造方法
 */
var SysQuerySettingEdit = function() {

}
// 属性及函数
SysQuerySettingEdit.prototype = {
	/**
	 * 初始化
	 */
	init : function() {
	...
	initDisplaySetting : function() {
		...
		for (var i = 0, c; c = displayField[i++];) {
			...
			$("input[var='id']", tr).attr("value", c.id);
	...
	getHtmlTd : function(v, rightId, rightName, rightScript) {
		var aryTd = [
				'<select name="displayFieldRight" class="change_right plat-select-type" var="right"  >',
				'<option value="none" '
				...
				'<a  href="####" name="btnScript" class="link var" title="常用脚本" onclick="__SysQuerySetting__.selectScript(this,false)">常用脚本</a>',
				'</span>'];
		return aryTd.join('');
	},
	/**
	...
	initExportField:function(){	
		...
		var tr = $($("#exportFieldTemplate .table-detail tr")[0]).clone(true,
				true);
		var exportFieldVal = $("#exportField").val();
		if ($.isEmpty(exportFieldVal))
			return;
		var exportField = $.parseJSON(exportFieldVal);
		var sb = new StringBuffer();
		for (var i = 0, c; c = exportField[i++];) {
			$("[var='index']", tr).html(i);
			$("[var='name']", tr).html(c.name);
			$("input[var='id']", tr).attr("value", c.id);
			$("input[var='desc']", tr).attr("value", c.desc);
		...
		$("#condition-columnsTbl input:[name='select']:checked")
				.each(function() {
					var me = $(this), na = me.attr("fieldname"), ty = me
							...
							.attr("controltype"),ftm = me.attr("fieldfmt"),obj= $("#conditionTbl"),
							id = me.attr("fieldId");
					var ppt;
					if(ftm!=undefined&&ftm!=null&&ftm!=""){
								ppt = ftm;
	...
	constructConditionTr : function(condition) {
		...
		var ty = condition.ty,id=condition.id, na = condition.na, cm = condition.cm, op = condition.op, va = condition.va, vf = condition.vf;
		// 控件类型
		ct = condition.ct;
		// 查询类型
		qt = condition.qt;
		...
		// ID
		var idTd = this.constructTag("td", {"var":"id"}, id);
		var idInput = this.constructTag("input", {
			type : "hidden",
			value : id
		});
		idTd.append(idInput);
		...
		ctTd=this.constructTag("td", {"var":"ct"}, ctSelect.find('option:selected').text());
		ctTd.append(ctSelect);
		...
		var vfSelect = this.constructTag("select", {
			name : "vf",
			onchange : "__SysQuerySetting__.selectValueFrom(this)"
		});
		...
		var a = this.constructTag("a", {
			...
			'class' : 'hide link var',
			title : '常用脚本',
			onclick : "__SysQuerySetting__.selectScript(this,true)"
		}, "常用脚本");
		var vaInput = {};
		if (vf == 1) {
		...
		var moveupA = this.constructTag("a", {
			href : "####",
			'class' : "link moveup",
			onclick : '__SysQuerySetting__.moveTr(this,true)'
		}, "");

		var movedownA = this.constructTag("a", {
			href : "####",
			'class' : "link movedown",
			onclick : '__SysQuerySetting__.moveTr(this,false)'
		}, "");
		var deleteA = this.constructTag("a", {
			href : "####",
			'class' : "link del",
			onclick : '__SysQuerySetting__.delTr(this)'
		}, "");
	...
	initManageField : function() {
		...
		for (var i = 0, c; c = manageField[i++];) {
			var tr = $($("#manageTemplate .table-detail tr")[0]).clone(true,
					true);
			$("[var='index']", tr).html(i);
			$("[var='desc']", tr).html(c.desc);
			$("[var='className']", tr).html(c.className);
			$("[var='clickName']", tr).html(c.clickName);
			$("[var='urlPath']", tr).html(c.urlPath);
			$("input[var='urlParams']", tr).attr("value", JSON2.stringify(c.urlParams));
			$("input[var='name']", tr).attr("value", c.name);
			for (var j = 0, k; k = c.right[j++];) {
				var rightHtml = this.getHtmlTd(k.type, k.id, k.name, k.script);
	...
	addFilter : function(conf) {
		var _self = this, sqlId = $('#sqlId').val(), source = $('#source').val(), 
	...
	filterDialog : function(conf) {
		...
		var url = __ctx
				+ "/oa/form/querySetting/filterDialog.do?sqlId="
				+ conf.sqlId;
		url = url.getNewUrl();
...
var __SysQuerySetting__ = new SysQuerySettingEdit();// 默认生成一个对象
