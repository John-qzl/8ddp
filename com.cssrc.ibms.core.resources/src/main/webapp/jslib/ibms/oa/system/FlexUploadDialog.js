...
function FlexUploadDialog(conf) {
	...
	var url=__ctx + "/oa/system/sysFile/dialog.do?isSingle="+isSingle;
...
function FormUploadDialog(conf) {
	if(!conf) conf={};
	var filenum = conf.filenum||0;
	var type = conf.type||"";
	var isSingle=conf.isSingle?1:0;
	var url="";
	if(conf.url!=null){
		...
		url=__ctx + "/upload/formFileUpload.do";
	}
	url=url+"?filenum="+filenum+"&type="+type+"&isSingle="+isSingle
	url=url.getNewUrl();
...
function DirectUploadDialog(conf) {
	...
	if(conf.url!=null){
		...
		url=__ctx + "/oa/system/sysFile/uploadDialog.do";
	}
	url=url.getNewUrl();
	var param = window;
...
function UploadDialog(conf) {
	if(!conf) conf={};
	var url=__ctx + "/oa/system/sysFile/uploadDialog.do";
	url=url.getNewUrl();
	DialogUtil.open({
		height:300,
		width: 450,
 ...
 * 直接文件上传文本框。
 * 使用方法。
 * AjaxUploadDialog({callback:function(json){
 *			for(var i=0;i<json.length;i++){
 *				var obj=json[i];
 *				alert(obj.fileId +"," + obj.fileName);
 *			}
 ...
 * @param conf
 */
function AjaxUploadDialog(conf) {
	if(!conf) conf={};
	...
	var param = window;
 	...
 	DialogUtil.open({
		...
		isResize: true,
		//自定义参数
		sucCall:function(returnArray){
				if(conf.callback){
					conf.callback.call(this,window.curobj,returnArray);
				}
		}
	});
 	//openwindow (url, '附件上传', '700', '350');
}
function UploadMergeDialog(conf){
	if(!conf) conf={};
	var url=__ctx + "/oa/system/sysFile/uploadMerge.do";
	url=url.getNewUrl();
	DialogUtil.open({
		height:500,
		width: 800,
		...
		sucCall:function(rtn){
				if(conf.callback){
					var result = {};
					if(rtn.fileIds){
						var files = {fileIds:rtn.fileIds,fileNames:rtn.fileNames,extPath:rtn.extPath};
						result = {type:"sys",files:files}
					}else{
						result = {type:"local",files:rtn}
					}
