...
function selectTable(){
	...
	var url=__ctx +"/oa/system/sysTableManage/selectTable.do?dsName="+dsName;
	url=url.getNewUrl();
...
function getTableColumns(dsName,tableName,tableType){
	...
	var columns;
	var url=__ctx +"/oa/system/sysTableManage/getTableColumns.do";
	$.ajax({
		url:url,
		data:params,
		async:false
	...
	if (obj.isSuccess()) {
		$.ligerDialog.confirm( obj.getMessage()+",是否继续操作","提示信息", function(rtn) {
			if(rtn){
				...
				window.location.href =  __ctx+"/oa/system/sysTableManage/list.do";
			}
		});
	} else {
...
function validateConditionScript(obj){
	var url=__ctx+"/oa/system/sysTableManage/parseConditionScript.do";
	var parameters=getParameters();
	var inputs=[];
	var inputnas=[];
	for(var i=0;i<parameters.length;i++){
