...
function cascadeSetCallback(t){
	var guid = t.guid;	
	var	span = $("span[name='change-type-span'][key='"+guid+"']");
	delete t.guid;
	span.data("cascade",t);
};
...
$(function(){
	...
	$("a.clean").click(function(){
		...
		$.ligerDialog.confirm('确定要删除吗?','提示',function(r){
			if(r){
				pks.each(function(){
					$(this).parents("tr").remove();
				});
				if($("tr",table).length<2){
					table.append('<tr class="empty"><td colspan="6">当前没有记录。</td></tr>');
				}
			}
			else{
				return;
...
function parse2json(m){
	...
	var url = __ctx + "/oa/flow/gangedSet/save.do";
	$.post(url,{json:json,defid:defId},function(t){
		t = eval("("+t+")");
		if(t.result)
			$.ligerDialog.success(t.message,'提示');
		else
			$.ligerDialog.error(t.message,'提示');
	});
};
...
function getChooseFieldHtml(callback,filter,setId){
	...
	var url = __ctx+"/oa/flow/gangedSet/getFields.do";
...
function addChooseItem(t,m){
	...
	var cSelect = $('<select class="fieldsSelect" name="choose-field"></select>').html(choseFieldsHtml),
		cSpan = $('<span class="change-panel"></span>').attr("key",guid).append(cSelect).append('<a href="javascript:;" class="remove-field">删除</a>');
...
function bindEvent(){		
	...
	$("select[name='choose-node']").live("change",function(){
		...
		if(!nodeFormKey&&!globalFormkey){
			...
			choseFieldsHtml = '';
			$.ligerDialog.warn('该节点未绑定表单，而且未设置全局表单，不能进行联动设置！','提示',function(){
				// 移除选择框中的所选值
...
function getNewTr(id,data){
	...
	var td1Input = $('<input type="checkbox" class="pk" name="id" />').val(id),
		...
		vSpan = $('<span class="change-panel"></span>').attr("key",guid2).append(vSelect).append('<a href="javascript:;" class="add-change-field">添加</a>'),
	...
	if(data||id=='0'){
		...
		var cSelect = $('<select class="fieldsSelect" name="choose-field"></select>').html(choseFieldsHtml),
			cSpan = $('<span class="change-panel"></span>').attr("key",guid1).append(cSelect).append('<a href="javascript:;" class="add-chose-field">添加</a><a href="javascript:;" class="remove-field">删除</a>');
		cSelect.live("change",updateChooseVal);
		td3.append(cSpan);
