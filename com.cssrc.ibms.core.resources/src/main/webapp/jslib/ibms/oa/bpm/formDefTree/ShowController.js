...
app.controller("ShowController",['$scope','BaseService','FormDefTree','CommonListService','ArrayToolService',function($scope,BaseService,FormDefTree,CommonListService,ArrayToolService){
	...
	$scope.param={formKey:formKey,__displayId__:__displayId__};
	
	//初始化对象
	FormDefTree.getObject({action:"formDefTree",formKey:formKey},function(data){
		if(data){
			$scope.prop=data;
			formKey=formKey;//初始化全局变量  formDefId
			
			if(data.loadType==0){//同步加载
				ztreeCreator = new ZtreeCreator('tree',"getObject.do?action=json&formKey="+formKey);
			}else{//异步加载
				var conf={
					enable: true,
					url: "getObject.do?action=json&formKey="+formKey,
					autoParam: ["id"]
				}; 
	...
	function zTreeOnClick(event, treeId, treeNode) {
		var url=__ctx +"/oa/form/dataTemplate/detailData.do?__pk__="+treeNode.id;
		url += "&__displayId__="+$scope.param.__displayId__;
	...
	function addNode(){
		...
		//var url=__ctx +"/oa/form/dataTemplate/editData_"+$scope.param.formKey+".do";
		var url=__ctx +"/oa/form/dataTemplate/editData.do";
		url += "?__formKey__="+$scope.param.formKey;
		...
		$("#centerFrame").on("load",function(){
			$(this.contentWindow.document).find("[name$=':"+$scope.prop.parentId+"']").val(pid);
	...
	//编辑
	function editNode(){
		//var url=__ctx +"/oa/form/dataTemplate/editData_"+$scope.param.formKey+".do?__pk__="+selectedTreeNode.id;
		var url = __ctx +"/oa/form/dataTemplate/editData.do?__pk__="+selectedTreeNode.id;
			url += "&__formKey__="+$scope.param.formKey;
		$("#centerFrame").attr("src",url);
	}
	//删除
