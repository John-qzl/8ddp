		...
		init:function(){
			...
			for ( var i = 0; i < this.fileObjs.length; i++) {
				...
				if(right=="no"){
					...
					//加载office控件。
					officeObj.renderTo(divId,{fileId:fileId,doctype:doctype,myNum:myNum,right:right,user:this.user});
					//是否有office控件。
...
function sysFileType(fileId){
	var doctype = "doc";
	var path = __ctx +'/oa/system/sysFile/getFileType.do';
...
function getDocUserData(){
	...
	var path = __ctx +'/oa/system/sysUser/getUserData.do';
    $.ajaxSetup({async:false});  //同步
