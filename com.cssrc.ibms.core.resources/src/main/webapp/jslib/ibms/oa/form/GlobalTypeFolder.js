 ...
 * @returns {GlobalType}
 */
GlobalTypeFolder=function(catKey,divId,conf){
	{
	...
	this.loadGlobalTree=function(){
		...
		var url=__ctx + "/oa/system/globalType/getByCatKey.do";
		if(conf.url){
			url=conf.url;
		}
        ...
        $.post(url,params,function(result){
            ...
            for(var i=0;i<result.length;i++){
                var node=result[i];
                if(node.parentId==-1){
                   /* node.icon=__ctx +"/styles/green/images/icon/root.png";*/
                    node.isRoot=1;
                }
            }
	...
	this.openGlobalTypeDlg=function(isAdd,isPrivate){
		...
		var url=__ctx +'/oa/system/globalType/dialog.do';
		if(isAdd){
			if(isRoot){
