...
OfficeControl=function(){
	...
	this.getTemplate=function(callback){
		var url=__ctx +"/oa/system/sysOfficeTemplate/dialog.do?type="+this.templatetype;
		url=url.getNewUrl();
	...
	this.insertContentTemplate=function(){
		try{
			...
			var templateUrl = __ctx + "/oa/system/sysOfficeTemplate/getTemplateById.do?templateId=" + rtn.templateId;
	...
	this.insertTemplate=function(){
		try{
			...
			var headFileURL = __ctx + "/oa/system/sysOfficeTemplate/getTemplateById.do?templateId=" + rtn.templateId;
	...
	this.initDoc=function(){
		...
		if(this.config.fileId!="" && this.config.fileId>0){
			var path= __ctx + "/oa/system/sysFile/getFileById.do?fileId=" + this.config.fileId;
			try{
	...
	this.saveRemote=function(inputObjNum){
		var path= __ctx + "/oa/system/sysFile/saveFile.do";
	...
	this.signature=function(){
		...
		var url = __ctx + "/oa/system/seal/dialog.do";
		url = url.getNewUrl();		
		...
		DialogUtil.open({
			...
			sucCall:function(retVal){
				...
				var sealUrl=__ctx + "/oa/system/sysFile/getFileById.do?fileId=" + retVal.fileId;
				try{
	...
	this.officeToPdf=function(){
		...
		try{
			//保存数据到服务器。
			var pdfUrl = __ctx + "/oa/system/sysFile/saveFilePdf.do";
