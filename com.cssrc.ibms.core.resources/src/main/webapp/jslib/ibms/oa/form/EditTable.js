...
var controlList = [ {key : '1',value : '单行文本框'}, {key : '2',value : '多行文本框'},
                    {key : '10',value : '富文本框'}, {key : '3',value : '数据字典'},{key : '26',value : '密级管理'} , 
                    ...
                    {key : '11',value : '下拉单选项'},{key : '28',value : '下拉多选项'},{key : '13',value : '复选框'},
                    {key : '14',value : '单选按钮'},{key : '12',value : 'office控件'},
                    {key : '15',value : '日期控件'},{key : '20',value : '流程引用'},
                    {key : '21',value : 'WebSign控件'},{key : '22',value : '图片展示控件'},{key : '23',value : '关联关系列'},
                    {key : '24',value : '流程状态字段'},{key : '25',value : '数字范围控件'}
                    ];
/**
 * 密级管理基本类型
 */
var securityList = [ {key : '3',value : '公开'}, {key : '6',value : '秘密'},
                     {key : '9',value : '机密'}, {key : '12',value : '绝密'}
                    ];
/**
 * 是否外部表。
 */
...
//循环控件列表,搜索控件名称
function getControlTypeVal(controlType){
	var controlTypeVal;
	$(controlList).each(function(i, d) {
		if(d.key == controlType){
			controlTypeVal = d.value;
			return;
		}
	});
...
function handValueFrom(){
	$("#valueFrom").change(function(){
		...
		hdlEncryptTrShow(ft,vf,ct);
		hdlOptionTrShow(ft,vf,ct);
		hdlIsReferenceShow(vf,ct);
		hdlShowRelationTableTrShow(ft,vf,ct);
...
function handFieldType(){
	$("#fieldType").change(function(){
		...
		if(val==varchar_){
			$("#spanCharLen").show();
			$("#spanIntLen,#spanDecimalLen,#spanDateFormat,#trRelTableType,#trRelTableId").hide();
			$("#showComdify,#spanCoin,#spanCoinType").hide();
			//下拉框，复选框，单选按钮
			if(controlType=="11" || controlType=="13" || controlType=="14" || controlType=="28"){
				$("#trOption").show();
			}
			else{
				$("#trOption").hide();
			...
			$("#spanCharLen,#trOption,#spanDateFormat,#trRelTableType,#trRelTableId").hide();
			...
			$("#spanCharLen,#spanIntLen,#spanDecimalLen,#trOption,#trRelTableType,#trRelTableId").hide();
			$("#spanDateFormat").show();
			$("#showComdify,#spanCoin,#spanCoinType").hide();
			$("#showidentity").hide();
		}
		else{
			$("#spanCharLen,#spanIntLen,#spanDecimalLen,#trOption,#spanDateFormat,#trRelTableType,#trRelTableId").hide();
			$("#showComdify,#spanCoin,#spanCoinType").hide();
			$("#showidentity").hide();
		}
		//验证规则
...
function handControlType(){	
	...
	$("#controlType").change(function(){
		...
		if(typeof TableEXT=="function"){
			TableEXT.controlTypeChange(ft,vf,ct);
		}
	});
}
 ...
 * @param value
 */
function hdlShowRelationTableTrShow(ft,vf,value){
	//判断是否为23
	if(value==23){
		$("#trRule").hide();
		$("#trDict").hide();
...
function setControlByType(fieldType){
	...
	$(controlList).each(function(i, d) {
		...
		if(fieldType==varchar_){
			...
			//数字
			if(d.key=="1"|| d.key=="11" || d.key=="16" || d.key=="28"  ){//单行文本框、下拉选项、隐藏域
				objSelect.append(option);
			}
		}
	});
...
function initAdd(_isExternal){
	...
	$("#spanIntLen,#spanCoin,#spanCoinType,#spanDecimalLen,#spanDecimalLen,#showidentity,#trDict,#trScript,#trScriptID,#trOption,#spanDateFormat,#trCondition,#trRelTableType,#trRelTableId").hide();
	setControlByType("varchar");
	//动态加载数据字典。
	JsLoader.LoadCount=1;
	JsLoader.Load(__ctx +"/jslib/lg/plugins/catCombo.js","javascript1");
	var fieldType=$("#fieldType").val();
	if(fieldType=='number'){
		$("#showComdify").show();
		$("#spanCoin").show();
...
function resetField(){
	...
	setControlByType("varchar");
	$("#fieldName,#fieldDesc,#relTableType,#relTableId,#relDelType,#relDelLMType,#relFormDialog").val(""); 
	$("#isRequired,#isPkShow,#isFlowVar,#isUnique,#isQuery,#isReference,#isShowComdify,#coin,#isCurrentDate,#showCurUser,#showCurOrg,#showCurPos,#isWebSign,#isShowidentity").attr("checked",false);
...
function setFieldByValueFrom(field){
	...
	switch(from){
		...
		//流水号
		case 3:
			field.serialNumber=$("#identityAlias").val();
...
function setFieldByEncrypt(field){
	if(field.fieldType==varchar_){
		field.encrypt=$("#encrypt").val();
	}
}
...
function getOptionData(){
	...
	$("tr.editable-tr",$("#option-table")).each(function(){
		...
		if($("#statusIsShow").is(":hidden")){
			optionAry.push({key:optionKey,value:optionValue});
		}else{
			//对于下拉选项新增状态栏：1表示显示，0表示隐藏 Liubo
			$("select",me).each(function(){
				var me = $(this),
			      name = me.attr("name"),
			      val = me.val(),
			      memo = me.attr("title");
				optionStatus.push({lantype:name,lanres:val,lanmemo:memo});
			});
			optionAry.push({key:optionKey,value:optionValue,status:optionStatus});
		}
	});
	return optionAry;;
};
...
function getOptionData1(){
	...
	$("tr.editable-tr",$("#option-table")).each(function(){
		...
		if($("#statusIsShow").is(":hidden")){
			...
			var optionStatus = $("select[name='optionStatus']",me).val();
			optionAry.push({key:optionKey,value:optionValue,status:optionStatus});
		}
	});
	return optionAry;
...
function getField(){
	...
	var hoverTitleValue = $("#hoverTitleValue").val();
	var hoverTitle;
	if(isHoverTitle){
		hoverTitle={"displayHoverTitle":1,"hoverTitle":hoverTitleValue};
		field.ctlProperty = JSON2.stringify(hoverTitle);
	}else{
		hoverTitle={"displayHoverTitle":0,"hoverTitle":hoverTitleValue};
		field.ctlProperty = JSON2.stringify(hoverTitle);
	}
	...
	//13,复选框
	//14,单选按钮
	if(field.controlType==11 || field.controlType==13 || field.controlType==14 || field.controlType==28 ){
		var inputValues = $("input[name='optionValue']",$("#option-table")),
			optsAry;
		if(inputValues.length>0){//3.2版本
			optsAry = getOptionData1();
		...
		//下拉选项新增默认值
		var addDropDownDefault = $('#addDropDownDefault:checked').length==1;
		if(field.controlType==11&&addDropDownDefault){
			optsAry.unshift({key:"",status:"1",value:"请选择"});
		}
		field.options = JSON2.stringify(optsAry);
	}
	//设置日期格式
	...
	if(field.controlType==15 || field.fieldType==date_){
		...
		if(isCurrentDate!=undefined){
			if(isHoverTitle){
				json={"format":format,"displayDate":1,"displayHoverTitle":1,"hoverTitle":hoverTitleValue};
			}else{
				json={"format":format,"displayDate":1,"displayHoverTitle":0,"hoverTitle":hoverTitleValue};
			}
		}else{
			if(isHoverTitle){
				json={"format":format,"displayDate":0,"displayHoverTitle":1,"hoverTitle":hoverTitleValue};
			}else{
				json={"format":format,"displayDate":0,"displayHoverTitle":0,"hoverTitle":hoverTitleValue};
			}
		}
	...
	if(field.controlType==4){
		...
		if(isHoverTitle){
			json={"showCurUser":showCurUser,"displayHoverTitle":1,"hoverTitle":hoverTitleValue};
		}else{
			json={"showCurUser":showCurUser,"displayHoverTitle":0,"hoverTitle":hoverTitleValue};
		}
		field.ctlProperty=JSON2.stringify(json);
	}
	...
	if(field.controlType==18){
		...
		if(isHoverTitle){
			json={"showCurOrg":showCurOrg,"displayHoverTitle":1,"hoverTitle":hoverTitleValue};
		}else{
			json={"showCurOrg":showCurOrg,"displayHoverTitle":0,"hoverTitle":hoverTitleValue};
		}
		field.ctlProperty=JSON2.stringify(json);
	}
	// 显示当前岗位
	if(field.controlType==19){
		...
		if(isHoverTitle){
			json={"showCurPos":showCurPos,"displayHoverTitle":1,"hoverTitle":hoverTitleValue};
		}else{
			json={"showCurPos":showCurPos,"displayHoverTitle":0,"hoverTitle":hoverTitleValue};
		}
		field.ctlProperty=JSON2.stringify(json);
	}
	//必填
	...
	//是否主键显示值
	field.isPkShow=$("#isPkShow").attr("checked")?1:0;
	//控件类型 为"关联关系列"
	if(field.controlType==23) {
		//关联关系表Id
		field.relTableId=$("#relTableId").val();
		//关联关系类型
		field.relTableType=$("#relTableType").val();
		//rel关联表记录删除类型
		field.relDelType=$("#relDelType").val();
		//主表记录删除，rel关联表记录删除类型
		field.relDelLMType=$("#relDelLMType").val();
		//关联的自定义对话框信息
		field.relFormDialog=$("#relFormDialog").val();
	}else{
		field.relTableId='';
		field.relTableType='';
		field.relDelType='';
	...
	//唯一约束
	field.isUnique=$("#isUnique").attr("checked")?1:0;
	//是否直接上传文件
	if(field.controlType==9) {
		var isDirectUpLoad=$('#isDirectUpLoad').attr('checked')?1:0;
		if(isHoverTitle){
			json={"isDirectUpLoad":isDirectUpLoad,"displayHoverTitle":1,"hoverTitle":hoverTitleValue};
		}else{
			json={"isDirectUpLoad":isDirectUpLoad,"displayHoverTitle":0,"hoverTitle":hoverTitleValue};
		}
		field.ctlProperty=JSON2.stringify(json);
	}
	//是否作为超链接
	...
	if(field.fieldType==number_){
		...
		if(showCoin){
			...
			if(isHoverTitle){
				json.displayHoverTitle=1;
				json.hoverTitle=hoverTitleValue;
			}else{
				json.displayHoverTitle=0;
				json.hoverTitle=hoverTitleValue;
			}
			json.hoverTitle=hoverTitleValue;
			field.ctlProperty=JSON2.stringify(json);
...
//	field.isShowComdify=$("#isShowComdify").attr("checked")?1:0;
	...
	if(field.valueFrom==3){
		...
		if($("input[name='isShowidentity']:checked").val()){
			if(isHoverTitle){
				json={"isShowidentity":1,"displayHoverTitle":1,"hoverTitle":hoverTitleValue};
			}else{
				json={"isShowidentity":1,"displayHoverTitle":0,"hoverTitle":hoverTitleValue};
			}
			field.ctlProperty=JSON2.stringify(json);
		}else{
			if(isHoverTitle){
				json={"isShowidentity":0,"displayHoverTitle":1,"hoverTitle":hoverTitleValue};
			}else{
				json={"isShowidentity":0,"displayHoverTitle":0,"hoverTitle":hoverTitleValue};
			}
			field.ctlProperty=JSON2.stringify(json);
		}
	}
	...
	//设置加密算法
	setFieldByEncrypt(field);
	
	//EditTableEXT.js中对field属性进行修改
	var fieldCopy = $.extend(true,{},field)//深克隆
	var rtn = TableEXT.getField.call(this,fieldCopy);
	if(rtn){
		field = fieldCopy;
	}
	return field;
 ...
 * 显示控制
 */
function isShowDropDownDefaultVar(controlType){
	//下拉单选项
	if($.isEmpty(controlType)||controlType!=11){
		$("#addDropDownDefault").parent().hide();
		$("#addDropDownDefault").attr("checked",false);
	}else{
		$("#addDropDownDefault").parent().show();
		$("#addDropDownDefault").attr("checked",false);
	}			
}
/**
...
function initControlByField(field,allowEditColName,_isExternal){
	...
	//显示填好的悬停显示信息，若为空则表示不进行悬停显示
	if(field.ctlProperty == ""){
		$("#isHoverTitle").attr("checked",false);
		$("#hoverTitleValue").hide();
	}else{
		var property=eval("(" + field.ctlProperty +")");
		if(property.displayHoverTitle==1){
			$("#isHoverTitle").attr("checked",true);
			$("#hoverTitleValue").show();
			$("#hoverTitleValue").val(property.hoverTitle);
		}else{
			$("#isHoverTitle").attr("checked",false);
			$("#hoverTitleValue").hide();
		}
...
//	$("#ccisread").attr("checked",field.ccisread==1);
	...
	hdlIdentityTrShow(ft,vf,ct,field.serialNumber,field.script);
	...
	hdlEncryptTrShow(ft,vf,ct,field.encrypt);
	...
	$("#relTableId").val(field.relTableId);
	$("#relTableType").val(field.relTableType);
	$("#relDelType").val(field.relDelType);
	$("#relDelLMType").val(field.relDelLMType);
	$("#relFormDialog").val(field.relFormDialog);
}
/**
 * falg:是否为下拉选项，且设置了添加下拉选项默认值
 * flag为true：移除第一个元素，并且设置选中（下拉选项默认值）
 * @return {flag：true,field:field}
 */
function dropDownDefaultDeal(field){
	if(field.controlType=='11'&&$.parseJSON(field.options).length>0){
		var options = $.parseJSON(field.options);
		var frist = options[0];
		if(frist.key==""&&frist.value=="请选择"){
			 options.shift();
			 field.options = JSON2.stringify(options);
			 field.hasDropDownDefault = true;
			 $('#addDropDownDefault').attr("checked",true);
			 $('#addDropDownDefault').parent().show();
		}
		if(field.hasDropDownDefault){
			 $('#addDropDownDefault').attr("checked",true);
		}
	}
...
function setEditStatus(allowEditColName){
	...
	$("#relTableType").attr('disabled', 'disabled');
	$("#relTableId").attr('disabled', 'disabled');
	$("#relDelType").attr('disabled', 'disabled');
	$("#relDelLMType").attr('disabled', 'disabled');
	$("#relFormDialog").attr('disabled', 'disabled');
	if(isExternal==0){
...
function bindTable(data,allowEditTbColName,mainTableIsPublished){
	...
	if(table.isMain==0 ){
		...
		if(!allowEditTbColName && table.isPublished == 1)	{
			var href =__ctx + '/oa/form/formTable/get.do?canClose=1&hasClose=true&tableId='+table.mainTableId;
	...
	TableRow.fieldManage.setFields(fieldList);//FieldsManage.js中的this.setFields方法
	$("#tableColumnItem>tbody").append(TableRow.fieldManage.getHtml(conf));//在FieldsManage.js中搜索getHtml 方法。
	handisList();
...
function hdlControlTypeShow(ft,vf,value){
	...
	case varchar_://文本
		...
		case '2'://脚本输入 (隐藏)
			...
			controlList =  [ {key : '1',value : '单行文本框'}, {key : '2',value : '多行文本框'},
                    		...
                    		{key : '19',value : '岗位选择器(单选)'}, {key : '7',value : '岗位选择器(多选)'},
                    		{key : '16',value : '隐藏域'},{key : '9',value : '文件上传'},
                    		{key : '11',value : '下拉单选项'},{key : '28',value : '下拉多选项'},
                    		{key : '13',value : '复选框'},{key : '14',value : '单选按钮'},
		                    {key : '15',value : '日期控件'},{key : '20',value : '流程引用'}];
			break;
		case '3'://流水号
			break;
	...
	case number_://数字
		switch(vf){
		case '0'://表单输入
			controlList[2] = {key : '11',value : '下拉单选项'};
			controlList[3] = {key : '28',value : '下拉多选项'};
		case '1'://脚本输入 (显示)
		case '2'://脚本输入 (隐藏)
			controlTypeTr.show();
...
/**
 * 根据字段类型、值来源和控件类型，处理是否显示加密算法
 * @param fy 字段类型
 * @param vf 值来源
 * @param ct 控制类型
 */
function hdlEncryptTrShow(ft,vf,ct,value){
	vf=""+vf;
	ct=""+ct;
	var tr=$("#trEncrypt");
	tr.hide();
...
function addColumn1(data){
  ...
  if(typeof(data.key)!= undefined && data.key!=null && data.key!=''){
	  ...
	  if($("#statusIsShow").is(":hidden")){
		  ...
		  $("select[name='optionStatus']",newTr).val(data.status);	
	  }
  }
  return newTr;
};
 ...
 * @return {[type]}      [description]
 */
function addSecurityColumn(){
  var data = {
        key: "",
        value: ""
    }
  if($("#isAdd").val()==1&&$("tr.editable-tr",$("#option-table")).length==0){
	  $(securityList).each(function(i, d) {
		  var hiddenTable = $("#hiddenTable"),
	      tmpTr = $("tr.editable-tr",hiddenTable),
	      newTr = tmpTr.clone();
		  
		  $("input[name='optionKey']",newTr).val(d.key);
		  $("input[name='optionValue']",newTr).val(d.value);	
