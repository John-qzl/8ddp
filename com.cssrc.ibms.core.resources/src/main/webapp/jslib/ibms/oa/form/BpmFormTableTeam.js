...
BpmFormTableTeam.getFieldsByTableId = function(tableId) {
	...
	$.post('getFieldsByTableId.do?tableId=' + tableId, function(data) {
		var json = eval("("+data+")"),
			treeData = [];
...
function add(t,o){
	...
	if(!t){
		...
		$("[name='teamNameKey']", table).val(o.teamNameKey);
...
function saveData(){
	...
	$("#teamItem .table-detail").each(function(){
		...
		data.teamNameKey = $("input[name='teamNameKey']",me).val();
	...
	var url = __ctx + "/oa/form/formTable/teamSave.do"; 
