...
$(function(){
	...
	var conf = dialog.get("conf"); //注意提交的dialog参数名为conf
	...
	$("#sqlTip").qtip({
		content:{
			text:'<div>该脚本为 groovyScript脚本 ，返回值为可执行的sql语句,' +
				...
				'<br/>对参数(不管类型，日期类型也可以)可以通过 :key的方式做为参数(如:beginyjrq), 在参数paramMap中设置key和value.方式进行构造sql' +
			'<br/>Object beginyjrq = map.get("beginyjrq");' +
			'<br/>if(beginyjrq!=null){' +
			'sql+=" and F_YJRQ>=:beginyjrq ";' +
			'}' +
			'<br/>Object endyjrq = map.get("endyjrq");' +
			'<br/>if(endyjrq!=null){' +
			'sql+=" and F_YJRQ<=:endyjrq ";' +
			'}'
		}
	});
});
...
function ScriptDialog(conf){
	...
	if(!conf) conf={};	
	var url=__ctx + "/oa/bus/busQueryRule/script.do?tableName="+conf.tableName;	//条件脚本设计器
