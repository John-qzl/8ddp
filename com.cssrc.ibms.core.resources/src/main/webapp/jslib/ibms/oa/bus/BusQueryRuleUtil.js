...
BusQueryRuleUtil.eidtDialog = function(conf) {
	var url = __ctx + '/oa/bus/busQueryRule/edit_' + conf.tableName
			+ '.do', height = screen.availHeight - 35, width = screen.availWidth
			- 5;
	url = url.getNewUrl();
	$.openFullWindow(url);
}
...
BusQueryRuleUtil.saveFilter = function(conf) {
	...
	var target = "";
	if (!$.isEmpty(filterId)) {
		var url = __ctx + "/oa/bus/busQueryFilter/get.do";
	...
	function saveFilter(item, dialog, filterId) {
		...
		var url = __ctx + "/oa/bus/busQueryFilter/saveFilter.do";
...
BusQueryRuleUtil.myFilter = function(conf) {
	var url = __ctx + '/oa/bus/busQueryFilter/list.do?tableName='
...
BusQueryRuleUtil.setting = function(conf) {
	var url = __ctx + '/oa/bus/busQuerySetting/edit.do?tableName='+conf.tableName;
	url = url.getNewUrl();
