Namespace.register("com.ibms.oa.desktop");
/**
 * 获取栏目模板
 */
com.ibms.oa.desktop.getColumnTemp= function(cols,width,showTable){
...
com.ibms.oa.desktop.homeTemp= function(cols,width){	
  $.post(
	__ctx+"/oa/system/desktopMycolumn/getMycolumnData.do",
	function(jsonData){
		var colshtml=com.ibms.oa.desktop.getColumnTemp(cols,width,0);		
		$("#colshtml").append(colshtml);
		$(jsonData).each(function (i) {  
            //同步处理布局中的数据链接  
            columnUrl=jsonData[i].columnUrl;
            $.post( 
                __ctx + "/oa/system/desktopColumn/getTemp.do?id="+jsonData[i].columnId,
                function (jsonStr) { 
                    //创建布局窗格
                	var htt,htd;                	
 ...
 * 获取栏目数据的模板
 */
com.ibms.oa.desktop.myTemp= function(cols,columnId,width,showTable,desktop){
		    var columnUrl;
		    $('#news').attr('href',"news.do?id="+columnId);
		    $('#change').attr('href',"change.do?id="+columnId);
		    var colshtml;
	        $.ajax({  
	            type: "POST",  
	            url:__ctx + "/oa/system/desktopMycolumn/getMycolumnData.do",
	            beforeSend: function (XMLHttpRequest) { $("#londing").show(); },  
	            success: function (jsonData) {  
	            	//加载布局模板
	                colshtml=com.ibms.oa.desktop.getColumnTemp(cols,width,showTable,desktop);
	                $("#colshtml").append(colshtml);
	                ...
	                $(jsonData).each(function (i) {  
	                    ...
	                    columnUrl=jsonData[i].columnUrl;
	                    $.ajax({  
	                        url: __ctx + "/oa/system/desktopColumn/getTemp.do?id="+jsonData[i].columnId,  
	                        async: false,  
 ...
 * 保存布局
 */
com.ibms.oa.desktop.saveCol= function(layoutID,filename,layoutCols){
		var jsonSaveStr = "[";
		// 获取布局数据
		...
		$.ajax({
			type : "POST",
			url : __ctx + "/oa/system/"+filename+"/saveCol.do",
			data : {
			...
			success : function(result) {
				var msg = new com.ibms.form.ResultMessage(result);
				if (msg.isSuccess()) {
					ref = true;
