var DesktopManage=function(){
	...
	this.init=function(){
		...
		var url=__ctx+'/oa/system/desktop/getMyDesktop.do';
		if(layoutId){
			url=__ctx+"/oa/system/desktop/getLayoutData.do?layoutId="+layoutId;
		}
		...
		for(var i=0;i<data.columns.length;i++){
			...
			if(data.datas[i+1]!=[]&&data.datas[i+1]!=null){
				for(var j=0;j<data.datas[i+1].length;j++){
					...
					var url=__ctx+'/oa/system/desktopColumn/getColumnData.do?id='+columnId;
					var portlet={
								attrs: {
			                        id: columnId
			                    },
	...
	this.changLayout=function(){
		...
		var url=__ctx+"/oa/system/desktop/getLayoutData.do?layoutId="+layoutId;
		$.post(url,function(data){
			self.handerData(data);
			self.setDesktop();
			self.resetDesk();
	...
	this.sortDesk=function(){
		...
		$(div).find('.ui-portlet-column').each(function(){
			...
			$(this).find('.ui-portlet-item').each(function(index){
				...
				var url=__ctx + "/oa/system/desktopColumn/getColumnData.do?id="+ id;
	...
	this.saveLayout=function(){
		...
		var layoutId=$("#colsSelect").val();
		var url=__ctx+"/oa/system/desktopMycolumn/saveCol.do";
		if(!layoutId){
			layoutId=$("#layoutId").val();
			url=__ctx+"/oa/system/desktopLayoutcol/saveCol.do";
		}
...
function getMore(url){
	$.post(__ctx+"/oa/console/getResourceNode.do?columnUrl="+url,function(data){
		if(data==null){
