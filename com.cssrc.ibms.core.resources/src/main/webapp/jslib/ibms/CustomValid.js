...
(function($) {
	$.extend($.fn, {
		...
		valid : function(conf) {
			...
			if(form.length==0){
				//解决bug：对没有绑定流程的表单的，如有必填，input的框线为没有为红色。
				//对validateForm进行重新赋值。
				validateForm = document.getElementsByTagName('form');
			}
			$('[validate]', validateForm).each(function() {
		...
		handValidResult : function(obj) {
			//判断验证的控件是否是a标签并且不是附件，若是满足条件，则直接返回，否则进行验证
			var isATag=$(obj).is("a:not(.selectFile)");
			if(isATag) return;
			...
			if (msg != '') {
				...
				//var nameList = $(obj).attr("lablename");
				var nameListObj = $(obj).parents('td').prev();
				var nameList =  nameListObj.text().trim();
				if(nameList == ""){//校验无td元素的情况 TODO yangbo
					nameList = $(obj).attr("lablename");
					nameList += ":*";
					returnObj[nameList]=msg;
				}else if(nameList!=null&&nameList.length>0){
					returnObj[nameList]=msg;
				}else{
					returnObj.success=true;
				}
		...
		errorPlacement : function(element, msg) {
			...
			if($(element).attr('ctltype')=="selector") {
				parent.after($('<label class="error">' + msg
						+ '</label>'));
			}else if ($(element).hasClass('dicComboTree')) {
				parent.append($('<label style="margin-left: 227px;position: absolute;display: block;width: 50px;left: 68px;" class="error">' + msg
						+ '</label>'));
			}else{
