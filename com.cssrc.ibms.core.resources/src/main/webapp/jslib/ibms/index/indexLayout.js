	...
	Plugin.prototype = {
		...
		initCkeditor : function() {
			...
			contenthandle = CKEDITOR.replace('contenteditor', {
				language : 'zh-cn',
				contentsCss : [ __ctx
						+ '/styles/default/css/bootstrap/bootstrap-combined.min.css' ],
				allowedContent : true
			});
		},
		//初始化恢复数据
...
//			}
		...
		saveRemoteLayout:function () {
			...
			$.ajax({
				type : "POST",
				url : __ctx+"/oa/system/sysIndexLayoutManage/saveLayout.do",
				data : {
					id:$('#id').val(),
					name:$('#name').val(),
					memo:$('#memo').val(),
				...
				success : function(data) {
					...
					if (obj.result == 1) {
						BootstrapDialog.confirm("保存成功,是否继续操作？", function(rtn){
							if(rtn){
									window.location.href=__ctx+"/oa/system/sysIndexLayoutManage/design.do?id="+obj.message;
							}else{
								if(window.opener){
									try{
		...
		saveRemoteMyLayout:function(){
			...
			$.ajax({
				type : "POST",
				url : __ctx+"/oa/system/sysIndexMyLayout/saveLayout.do",
				data : {
					html : html.html(),
					designHtml:	designHtml.html()
				},
				success : function(data) {
					...
					if (obj.result == 1) {
						BootstrapDialog.confirm("保存成功,是否继续操作？", function(rtn){
							if(rtn){
									window.location.href=__ctx+"/oa/system/sysIndexMyLayout/design.do";
							}else{
								if(window.opener){
									try{
										window.opener.location.href=window.opener.location.href.getNewUrl();
		...
		selectTemplate:function(){
			...
			BootstrapDialog.show({
		        ...
		        url:__ctx+"/oa/system/sysIndexLayoutManage/selectTemplate.do",
		        //回调函数
		...
		deleteLayout:function(id){
			BootstrapDialog.confirm("该操作是删除自己的布局，首页的布局取默认的布局。是否删除当前设计的布局？", function(rtn){
				if(rtn){
					...
					$.ajax({
						type : "POST",
						url : __ctx+"/oa/system/sysIndexMyLayout/deleteLayout.do",
						data : {
							id : id
						},
						success : function(data) {
