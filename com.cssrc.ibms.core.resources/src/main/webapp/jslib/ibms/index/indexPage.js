 ...
 ******************************************************************************/
	...
	Plugin.prototype = {
		...
		addTab : function(url) {
			...
			$.post(__ctx + "/oa/console/getResourceNode.do?columnUrl="
					+ url2, function(data) {
				if (data == null | data == "") {
					alert("更多路径配置有误", '提示信息');
				} else {
		...
		handleLayerslider : function(conf) {
			...
			var div = conf.div, data = conf.data ? conf.data : [], height = conf.height ? conf.height
					...
					'.widget-layerslider', div.outerHTML);//这里要用 div.outerHTML 不能用div，否则无法获取'.widget-layerslider'
		...
		parserTemplate : function(el) {
			...
			var url=__ctx + "/oa/system/sysIndexColumn/getData.do";
