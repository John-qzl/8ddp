...
.directive('htDic', function(){
	return {
		...
		link: function(scope, element, attrs) {
			...
			var url = __ctx +"/oa/system/dataDict/getByTypeKeyForComBo.do?typeKey="+attrs.htDic;
			element.combotree({
				url:url,
	...
	return {
		...
		link: function(scope, element, attrs) {
			...
			scope.download = function(item){
				window.open(__ctx + "/oa/system/file/download.do?id="+item.id);
			}
 ...
 *			  ht-select-ajax="{url:'${ctx}/oa/system/sysType/getByGroupKey.do?groupKey=FORM_TYPE',field:'formTypeList'}">
...
.directive('htUserTag', ["BaseService",function(BaseService) {
	return {
		...
		link : function(scope, elm, attrs) {
			if(scope.htUserTag){
				BaseService.post(__ctx + "/oa/org/user/userInfo.do",{id:scope.htUserTag},function(data){
					scope.user = data;
				});
			}
		},
