	...
	$.LinkDivList = function($div, options) {
		...
		function drawLine(x0, y0, x1, y1, color) {
			...
			if(y0 == y1){ // 画横线
				...
				var css = { top: y0, left: x0, height: Math.abs(y1 - y0)+2, background: color, width: 2 };
				rs.push(getLineHtml(css));
			} else {
				var lx = x1 - x0
				var ly = y1 - y0
		...
		function addComputing(item) {
			...
			//select.css({ top: top - 15, left: left + width + 12, width: 52 });
			/*2017-05-11:解决添加规则出现的bug(rzq)*/
			select.css({ top: -(height/2+2), left: width + 12, width: 52 });
			item.append(select);
		};
