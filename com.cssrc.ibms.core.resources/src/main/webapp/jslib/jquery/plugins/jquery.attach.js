(function($){
	// 操作附件访问路径
	var fileUrl = __ctx+"/oa/system/sysFile/";
	...
	function attachUpload2(tdobj){
		var curobj = tdobj.parent().find('input:first');
		var url=__ctx+"/oa/system/sysFile/uploadDialog.do?typeId=0";
	 	url=url.getNewUrl();
	 	var param = window;
	 	param.curobj = curobj;
	...
	function initAttach(obj,attachs)
	{
		for(var idx=0;idx<attachs.length;idx++){
			...
			var downlink = $("<a></a>");
			var delurl = fileUrl+"download.do?fileId="+attach.fileId;
