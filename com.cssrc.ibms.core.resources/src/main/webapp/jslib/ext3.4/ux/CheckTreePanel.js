...
Ext.ux.tree.CheckTreePanel = Ext.extend(Ext.tree.TreePanel, {
			...
			initComponent : function() {
				...
				Ext.ux.tree.CheckTreePanel.superclass.initComponent.apply(this,
						arguments);
				var a = Ext.apply({
			...
			getValue : function() {
				...
				this.root.cascade(function(a) {
							if (true === a.attributes.checked) {
								if (false === this.deepestOnly
										|| !this.isChildChecked(a)) {
									b.push(a.id);
								}
							}
						}, this);
			...
			setValue : function(a) {
				//this.clearValue();//不知道为啥要注释掉，反正功能正常了
			...
			onRender : function() {
				Ext.ux.tree.CheckTreePanel.superclass.onRender.apply(this,
						arguments);
...
Ext.ux.tree.CheckTreeNodeUI = Ext.extend(Ext.tree.TreeNodeUI, {
	renderElements : function(e, h, g, j) {
		...
		var d = [
				...
				'<img src="',
				h.icon || this.emptyIcon,
				'" class="x-tree-node-icon',
				(h.icon ? " x-tree-node-inline-icon" : ""),
				(h.iconCls ? " " + h.iconCls : ""),
				'" unselectable="on" />',
				'<img src="' + this.emptyIcon + '" class="x-tree-checkbox'
						+ (true === i ? " x-tree-node-checked" : "") + '" />',
				'<a hidefocus="on" class="x-tree-node-anchor" href="', c,
				'" tabIndex="1" ',
				h.hrefTarget ? ' target="' + h.hrefTarget + '"' : "",
	...
	setChecked : function(b) {
		...
		if (a) {
			true === b ? a.addClass("x-tree-node-checked") : a
