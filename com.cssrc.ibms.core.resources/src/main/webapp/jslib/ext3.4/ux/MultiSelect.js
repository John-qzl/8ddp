...
Ext.ux.form.MultiSelect = Ext.extend(Ext.form.Field, {
			...
			onRender : function(c, b) {
				...
				a.doLayout();
				if (this.rowSize) {
					a.setHeight(a.getHeight() / this.store.getCount() * 2);
					a.doLayout();
				}
			},
			...
			setValue : function(a) {
				...
				for (var c = 0; c < a.length; c++) {
					b = this.view.store.indexOf(this.view.store.query(
							this.valueField, new RegExp("^" + a[c] + "$", "i"))
							.itemAt(0));
					d.push(b);
				}
				this.view.select(d);
...
Ext.extend(Ext.ux.form.MultiSelect.DragZone, Ext.dd.DragZone, {
			...
			getDragData : function(d) {
				...
				if (c) {
					...
					if (this.view.getSelectionCount() == 0 || d.ctrlKey
							|| d.shiftKey) {
						return false;
					}
					var b = {
...
Ext.extend(Ext.ux.form.MultiSelect.DropZone, Ext.dd.DropZone, {
			...
			getDropPoint : function(g, j, d) {
				if (j == this.ms.fs.body.dom) {
					return "below";
				}
			...
			onNodeDrop : function(b, j, h, f) {
				...
				if (this.ms.appendOnly) {
					...
					insertAt = b == this.ms.fs.body.dom ? this.view.store
							.getCount()
							- 1 : this.view.indexOf(b);
					if (k == "below") {
						insertAt++;
					}
				}
				...
				for (var g = 0; g < f.records.length; g++) {
					...
					this.view.store.insert(c == "down" ? insertAt : insertAt++,
							a);
