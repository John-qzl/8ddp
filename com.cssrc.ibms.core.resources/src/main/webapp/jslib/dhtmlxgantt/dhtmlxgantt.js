...
/**
 * Overrides event functionality. Includes all default methods from
 * dhtmlx.common but adds _silentStart, _silendEnd
 * 
 * @desc:
 * @type: private
 */
...
(function() {
	...
	function hasNonNegativeTabIndex(node) {
		return !isNaN(node.getAttribute("tabindex"))
				&& (node.getAttribute("tabindex") * 1 >= 0);
	}
	...
	gantt._getFocusableNodes = function getFocusableNodes(root) {
		var nodes = root.querySelectorAll([ "a[href]", "area[href]", "input",
		...
		for (var i = 0; i < nodesArray.length; i++) {
			...
			var isValid = (hasNonNegativeTabIndex(node) || isEnabled(node) || hasHref(node))
					&& isVisible(node);
			if (!isValid) {
...
/*
 * %d - the day as a number with a leading zero ( 01 to 31 ); %j - the day as a
 * number without a leading zero ( 1 to 31 ); %D - the day as an abbreviation (
 * Sun to Sat ); %l - the day as a full name ( Sunday to Saturday ); %W - the
 * ISO-8601 week number of the year. Weeks start on Monday; 1) %m - the month as
 * a number without a leading zero ( 1 to 12 ); %n - the month as a number with
 * a leading zero ( 01 to 12); %M - the month as an abbreviation ( Jan to Dec );
 * %F - the month as a full name ( January to December ); %y - the year as a
 * two-digit number ( 00 to 99 ); %Y - the year as a four-digit number (
 * 1900–9999 ); %h - the hour based on the 12-hour clock ( 00 to 11 ); %H - the
 * hour based on the 24-hour clock ( 00 to 23 ); %i - the minute as a number
 * with a leading zero ( 00 to 59 ); %s - the second as a number without a
 * leading zero ( 00 to 59 ); 2) %a - displays am (for times from midnight until
 * noon) and pm (for times from noon until midnight); %A - displays AM (for
 * times from midnight until noon) and PM (for times from noon until midnight).
 * 
 */
	...
	gantt._init_template = function(name, initial, template_name) {
		...
		if (this.config[name]
			...
			if (!(initial && this.templates[template_name])) {
				this.templates[template_name] = this.date
						.date_to_str(this.config[name]);
				registeredTemplates[template_name] = this.config[name];
			}
		}
	...
	gantt._init_templates = function() {
		...
		gantt
				.mixin(
						this.templates,
						{
							xml_date : this.date.str_to_date(c.xml_date,
									c.server_utc),
							...
							date_grid : function(date, item) {
								if (item && gantt.isUnscheduledTask(item)
										&& gantt.config.show_unscheduled) {
									return gantt.templates
											.task_unscheduled_time(item);
								} else {
									return gantt.templates
											.grid_date_format(date);
								}
							},

							task_time : function(start, end, ev) {
								if (gantt.isUnscheduledTask(ev)
										&& gantt.config.show_unscheduled) {
									return gantt.templates
											.task_unscheduled_time(ev);
								} else {
									return gantt.templates.task_date(start)
											+ " - "
											+ gantt.templates.task_date(end);
								}
							},
							...
							link_description : function(link) {
								var from = gantt.getTask(link.source), to = gantt
										.getTask(link.target);

								return "<b>" + from.text + "</b> &ndash;  <b>"
										+ to.text + "</b>";
							},
							...
							drag_link : function(from, from_start, to, to_start) {
								...
								if (to) {
									...
									text += "<b> "
											+ to.text
											+ "</b> "
											+ (to_start ? labels.link_start
													: labels.link_end)
											+ "<br/>";
								}
								return text;
							},
							drag_link_class : function(from, from_start, to,
									to_start) {
								var add = "";

								if (from && to) {
									var allowed = gantt.isLinkAllowed(from, to,
											from_start, to_start);
									add = " "
											+ (allowed ? "gantt_link_allow"
													: "gantt_link_deny");
								}
							...
							/*
							 * used for aria-labels of bar elements and for
							 * tooltip.js
							 */
							tooltip_date_format : gantt.date
									.date_to_str("%Y-%m-%d"),
							tooltip_text : function(start, end, event) {
								return "<b>Task:</b> "
										...
										+ "<br/><b>Start date:</b> "
										+ gantt.templates
												.tooltip_date_format(start)
										+ "<br/><b>End date:</b> "
										+ gantt.templates
												.tooltip_date_format(end);
							}
						});
...
gantt._on_click = function(e) {
	...
	var res = true;
	if (id !== null) {
		res = !gantt.checkEvent("onTaskClick")
...
gantt._on_contextmenu = function(e) {
	e = e || window.event;
	var src = e.target || e.srcElement, taskId = gantt.locate(src), linkId = gantt
			.locate(src, gantt.config.link_attribute);

	var res = !gantt.checkEvent("onContextMenu")
			|| gantt.callEvent("onContextMenu", [ taskId, linkId, e ]);
	if (!res) {
		if (e.preventDefault)
			e.preventDefault();
		else
		...
		if (css) {
			...
			for (var i = 0; i < css.length; i++) {
				...
				if (hash[css[i]]) {
					...
					res = res
							&& !(typeof handler != "undefined" && handler !== true);
				}
			}
		}
		trg = trg.parentNode;
...
gantt._on_dblclick = function(e) {
	...
	var trg = e.target || e.srcElement;
	var id = gantt.locate(e);
	var res = !gantt.checkEvent("onTaskDblClick")
			|| gantt.callEvent("onTaskDblClick", [ id, e ]);
	if (res) {
		var default_action = gantt._find_ev_handler(e, trg, gantt._dbl_click,
				id);
		if (!default_action)
		return;
...
gantt._DnD.prototype = {
	traceDragEvents : function(domElement) {
		...
		var mouseup = gantt.bind(
				function(e) {
					gantt.eventRemove(document.body, this._evs.move,
							limited_mousemove);
	...
	checkPositionChange : function(pos) {
		...
		var distance = Math.sqrt(Math.pow(Math.abs(diff_x), 2)
	...
	backupEventTarget : function(domEvent) {
		...
		// keep original event target in DOM in order to keep dnd on touchmove
		// event
	...
	dragMove : function(obj, e) {
		...
		if (!this.config.marker && !this.config.started) {
			...
			if (gantt.config.touch || this.checkPositionChange(pos)) {
				...
				if (this.callEvent("onBeforeDragStart", [ obj,
						this.config.original_target ]) === false) {
				...
				this.callEvent("onAfterDragStart", [ obj,
						this.config.original_target ]);
			} else {
				this.config.ignore = true;
			}
...
gantt._init_grid = function() {
	...
	this._click.gantt_grid_head_cell = this
			.bind(
					function(e, id, trg) {
						...
						if (this.config.sort) {
							...
							if (conf && conf.sort !== undefined
								...
								if (!sorting_method) { // column sort property
														// 'false', no sorting
									return;
								}
							}

							var sort = (this._sort && this._sort.direction && this._sort.name == column) ? this._sort.direction
									: "desc";
...
gantt._render_grid_header = function() {
	...
	for (var i = 0; i < columns.length; i++) {
		...
		width += colWidth;
		var sort = (this._sort && col.name == this._sort.name) ? ("<div class='gantt_sort gantt_"
				+ this._sort.direction + "'></div>")
				: "";
		...
		var cell = "<div class='" + cssClass + "' style='" + style + "' "
				+ ariaAttrs + " column_id='" + col.name + "'>" + label + sort
				+ "</div>";
		cells.push(cell);
	}
...
gantt._render_grid_item = function(item) {
	...
	for (var i = 0; i < columns.length; i++) {
		...
		if (col.name == "add") {
			...
			if (value.indexOf("a") > 0) {
				...
				value = "<div title=\"" + value.split("&nbsp").join("")
						+ "\" class='gantt_tree_content'>" + value + "</div>";
			}
		}
		...
		if (col.tree) {
			...
			bh = item["number"] == undefined ? ""
					: "<div class=\"gantt_tree_bh\">" + item["number"]
							+ "</div>";
		}
		if (col.text) {
			// majing 添加事件，可以自定义任务图标
			var customCss = [];
			var retVal = this.callEvent("onBeforeSetTaskIcon", [ item,
					customCss ]);
			var iconCss = "";
			if (retVal) {
				iconCss = customCss[0];
		...
		cell = "<div class='" + css + "' style='" + style + "' " + aria + ">"
				+ tree + bh + value + "</div>";
		cells.push(cell);
	}
	...
	if (this.templates.grid_row_class) {
		var css_template = this.templates.grid_row_class.call(this,
				item.start_date, item.end_date, item);
		if (css_template)
			css += " " + css_template;
	}
...
gantt.moveTask = function(sid, tindex, parent) {
	 ...
	 * 
	 * gantt.attachEvent("onBeforeTaskMove", function(id, parent, tindex){ var
	 * task = gantt.getTask(id); if(task.parent != parent) return false; return
	 * true; });
	 */
	...
	if (!tid) // adding as last element
		...
		tbranch = tbranch.slice(0, tindex).concat([ sid ]).concat(
				tbranch.slice(tindex));
	...
	if (tindex * 1 > 0) {
		if (id) {
			source.$drop_target = (this.getTaskIndex(sid) > this
					.getTaskIndex(id) ? "next:" : '')
					+ id;
		} else {
...
gantt._init_dnd = function() {
	var dnd = new gantt._DnD(this.$grid_data, {
	...
	dnd.attachEvent("onBeforeDragStart", this.bind(function(obj, e) {
		...
		if (!this.callEvent("onRowDragStart",
				[ id, e.target || e.srcElement, e ])) {
			return false;
		}
	...
	dnd.lastTaskOfLevel = function(level) {
		var ids = gantt._order, pull = gantt._pull, last_item = null;
					...
					this
							.bind(
									function(obj, e) {
										...
										var el = dnd._getTaskByY(targetY, gantt
												.getGlobalTaskIndex(item.id));

										if (!this.isTaskExists(el)) {
											el = dnd
													.lastTaskOfLevel(gantt.config.order_branch_free ? item.$level
															: 0);
											if (el == dnd.config.id) {
												el = null;
											}
										...
										function allowedLevel(next, item) {
											return (!(gantt.isChildOf(over.id,
													item.id)) && (next.$level == item.$level || gantt.config.order_branch_free));
										}
										...
										if (this.isTaskExists(el)) {
											...
											if (gantt
													.getGlobalTaskIndex(over.id)
													* this.config.row_height
													+ this.config.row_height
													/ 2 < targetY) {
												// hovering over bottom part of
												// item, check can be drop to
												// bottom
												var index = this
														.getGlobalTaskIndex(over.id);
												var next = this._pull[this._order[index + 1]]; // adds
																								// +1
																								// when
																								// hovering
																								// over
												...
												if (next) {
													...
													// we at end of the list,
													// check and drop at the end
													// of list
											...
											// if item is on different level,
											// check the one before it
											var index = this
													.getGlobalTaskIndex(over.id), prev = this._pull[this._order[index - 1]];

											var shift = 1;
											while ((!prev || prev.id == over.id)
													&& index - shift >= 0) {
												prev = this._pull[this._order[index
														- shift]];
												shift++;
											}
											...
											if (allowedLevel(over, item)
													&& item.id != over.id) {
												this.moveTask(item.id, 0, 0,
														over.id);

											} else if (over.$level == item.$level - 1
													&& !gantt
															.getChildren(over.id).length) {
												this.moveTask(item.id, 0,
														over.id);

											} else if (prev
													&& (allowedLevel(prev, item))
													&& (item.id != prev.id)) {
												this.moveTask(item.id, -1, this
														.getParent(prev.id));
	...
	dnd.attachEvent("onDragEnd", this.bind(function() {
		...
		if (this.callEvent("onBeforeRowDragEnd", [ dnd.config.id,
				dnd.config.parent, dnd.config.index ]) === false) {
			this.moveTask(dnd.config.id, dnd.config.index, dnd.config.parent);
			task.$drop_target = null;
		} else {
			// var newLevel = task.$level,
			// oldLevel = !this.isTaskExists(dnd.config.parent) ? 0 :
			// (this.getTask(dnd.config.parent).$level + 1);
			// var levelChanged = newLevel != oldLevel;

			this
					.callEvent("onRowDragEnd", [ dnd.config.id,
							task.$drop_target ]);
		}
...
/*
 * will be overwriten in order to provide hide/show column functionality in some
 * editions
 */
...
(function() {
	...
	gantt._waiAria = {
		...
		_taskCommonAttr : function(task, div) {
			div.setAttribute("aria-label", stripHTMLLite(gantt.templates
					.tooltip_text(task.start_date, task.end_date, task)));
			...
			div
					.setAttribute(
							"aria-selected",
							(gantt.getState().selected_task == task.id || (gantt.isSelectedTask && gantt
									.isSelectedTask(task.id))) ? "true"
									: "false");
		},
		...
		taskRowAttr : function(task, div) {
			...
			if (gantt._has_children(task.id)) {
				div
						.setAttribute("aria-expanded", task.$open ? "true"
								: "false");
			}
		},
		...
		linkAttr : function(link, div) {
			...
			var toStart = link.type == linkTypes.finish_to_start
					|| link.type == linkTypes.start_to_start;
			var fromStart = link.type == linkTypes.start_to_start
					|| link.type == linkTypes.start_to_finish;

			var content = gantt.locale.labels.link
					+ " "
					+ gantt.templates.drag_link(link.source, fromStart,
							link.target, toStart);
		...
		gridAttrString : function() {
			return [
					" role='treegrid'",
					gantt.config.multiselect ? "aria-multiselectable='true'"
							: "aria-multiselectable='false'", " " ].join(" ");
		},
...
gantt._scale_helpers = {
	...
	adjustSize : function(width, parts, from, to) {
		...
		for (var i = from; i <= to; i++) {
			var share = Math.floor(width
					* (full ? (parts[i] / full) : (1 / length)));
	...
	prepareConfigs : function(scales, min_coll_width, container_width,
			scale_height) {
	...
	setPosSettings : function(config) {
		for (var i = 0, len = config.trace_x.length; i < len; i++) {
			config.left.push((config.width[i - 1] || 0)
					+ (config.left[i - 1] || 0));
		}
	},
	...
	_ignore_time_config : function(date, scale) {
		if (this.config.skip_off_time) {
			...
			// check dates in case custom scale unit, e.g. {unit: "month", step:
			// 3}
	...
	initColSizes : function(config, min_col_width, full_width, line_height) {
		...
		var column_count = config.display_count === undefined ? config.count
		...
		for (var i = 0; i < config.trace_x.length; i++) {
			if (ignores[config.trace_x[i].valueOf()]
					|| (config.display_count == config.count)) {
				config.width[i] = 0;
			} else {
				config.width[i] = 1;
		...
		this
				.adjustSize(
						cont_width - this.getSum(config.width)/*
																 * 1 width per
																 * column from
																 * the code
																 * above
	...
	alineScaleColumns : function(lower_scale, upper_scale, from, to) {
		this.iterateScales(lower_scale, upper_scale, from, to, function(
				upper_start, upper_end, lower_start, lower_end) {
			var targetWidth = this.getSum(lower_scale.width, lower_start,
					lower_end - 1);
			var actualWidth = this.getSum(upper_scale.width, upper_start,
					upper_end - 1);
			if (actualWidth != targetWidth) {
				this.setSumWidth(targetWidth, upper_scale, upper_start,
						upper_end - 1);
			}
	...
	eachColumn : function(unit, step, callback) {
		var start = new Date(gantt._min_date), end = new Date(gantt._max_date);
	...
	limitVisibleRange : function(cfg) {
		...
		if (+dates[0] < +gantt._min_date && left != right) {
			var width = Math.floor(cfg.width[0]
					* ((dates[1] - gantt._min_date) / (dates[1] - dates[0])));
		...
		if (+outDate > +gantt._max_date && last > 0) {
			var width = cfg.width[last]
					- Math
							.floor(cfg.width[last]
									* ((outDate - gantt._max_date) / (outDate - lastDate)));
...
gantt._tasks_dnd = {
	...
	_move : function(ev, shift, drag) {
		var coords_x = this._drag_task_coords(ev, drag);
		var new_start = gantt.dateFromPos(coords_x.start + shift), new_end = gantt
				.dateFromPos(coords_x.end + shift);
		if (!new_start) {
			ev.start_date = new Date(gantt.getState().min_date);
			ev.end_date = gantt.dateFromPos(gantt.posFromDate(ev.start_date)
					+ (coords_x.end - coords_x.start));
		} else if (!new_end) {
			ev.end_date = new Date(gantt.getState().max_date);
			ev.start_date = gantt.dateFromPos(gantt.posFromDate(ev.end_date)
					- (coords_x.end - coords_x.start));
		} else {
	...
	_drag_task_coords : function(t, drag) {
		var start = drag.obj_s_x = drag.obj_s_x
				|| gantt.posFromDate(t.start_date);
	...
	_update_on_move : function(e) {
		...
		if (drag.mode) {
			...
			if (this._handlers[drag.mode]) {
				...
				gantt.callEvent("onTaskDrag", [ ev.id, drag.mode, copy,
						original, e ]);
	...
	on_mouse_down : function(e, src) {
		...
		if (!drag) {
			if (gantt.checkEvent("onMouseDown")
					&& gantt.callEvent("onMouseDown",
							[ className.split(" ")[0] ])) {
				if (src.parentNode)
					return this.on_mouse_down(e, src.parentNode);
			...
			if (drag.mode && drag.mode != gantt.config.drag_mode.ignore
					&& gantt.config["drag_" + drag.mode]) {
				id = gantt.locate(src);
				...
				if (gantt._is_flex_task(task)
						&& drag.mode != gantt.config.drag_mode.progress) {// only
																			// progress
																			// drag
																			// is
																			// allowed
	...
	_fix_dnd_scale_time : function(task, drag) {
		var unit = gantt._tasks.unit, step = gantt._tasks.step;
		...
		function fixStart(task) {
			if (!gantt.isWorkTime(task.start_date))
				task.start_date = gantt.calculateEndDate(task.start_date, -1,
		...
		if (drag.mode == gantt.config.drag_mode.resize) {
			...
			task.end_date = gantt.calculateEndDate(task.start_date,
					task.duration, gantt.config.duration_unit);
		}
	},
	...
	on_mouse_up : function(e) {
		...
		if (drag.mode && drag.id) {
			...
			if (!this._fireEvent("before_finish", drag.mode, [ drag.id,
					drag.mode, gantt.copy(drag.obj), e ])) {
				...
				this.clear_drag_state();
				gantt.updateTask(ev.id);
				this._fireEvent("after_finish", drag.mode, [ drag_id,
						drag.mode, e ]);
			}
	...
	_get_drag_mode : function(className) {
		...
		case "gantt_task_drag":
			...
			if (classes[1]
					&& classes[1].indexOf("left", classes[1].length
							- "left".length) !== -1) {
				drag.left = true;
			} else {
				drag.left = false;
	...
	_start_dnd : function(e) {
		...
		var cfg = gantt.config;
		var id = drag.id;
		if (!cfg["drag_" + drag.mode]
				|| !gantt.callEvent("onBeforeDrag", [ id, drag.mode, e ])
				|| !this._fireEvent("before_start", drag.mode, [ id, drag.mode,
						e ])) {
			this.clear_drag_state();
		} else {
			delete drag.start_drag;
...
gantt._render_link_element = function(link) {
	...
	var cssTemplate = this.templates.link_class ? this.templates
			.link_class(link) : "";
	if (cssTemplate) {
		css += " " + cssTemplate;
	}
...
gantt._drawer = {
	...
	get_wrapper_sizes : function(v) {
		var res, wrapper_size = gantt.config.link_wrapper_width, line_size = gantt.config.link_line_width, y = v.y
				+ (gantt.config.row_height - wrapper_size) / 2;
		switch (v.direction) {
		case this.dirs.left:
			res = {
				top : y,
	...
	get_line_sizes : function(v) {
		var res, line_size = gantt.config.link_line_width, wrapper_size = gantt.config.link_wrapper_width, size = v.size
				+ line_size;
		switch (v.direction) {
		case this.dirs.left:
		case this.dirs.right:
	...
	render_line : function(v) {
		...
		wrapper.style.cssText = [ "top:" + pos.top + "px",
				"left:" + pos.left + "px", "height:" + pos.height + "px",
				"width:" + pos.width + "px" ].join(';');
		wrapper.className = "gantt_line_wrapper";

		var innerPos = this.get_line_sizes(v);
		var inner = document.createElement("div");
		inner.style.cssText = [ "height:" + innerPos.height + "px",
				"width:" + innerPos.width + "px",
				"margin-top:" + innerPos.marginTop + "px",
				"margin-left:" + innerPos.marginLeft + "px" ].join(";");
...
gantt._path_builder = {
	...
	get_points : function(link) {
		...
		if (link.type == gantt.config.links.start_to_start) {
			...
			if (forward) {
				...
				this.point_to(dir.right, dx);
				this.point_to(dir.down, sign
						* (Math.abs(dy) - (xy.row_height / 2)));
				this.point_to(dir.right, shiftX);
			}
	...
	get_endpoint : function(link) {
		...
		var from = gantt._get_task_visible_pos(gantt._pull[link.source],
				from_start);
		var to = gantt
				._get_task_visible_pos(gantt._pull[link.target], to_start);
...
gantt._init_links_dnd = function() {
	var dnd = new gantt._DnD(this.$task_bars, {
		sensitivity : 0,
		updates_per_second : 60
	}), start_marker = "task_left", end_marker = "task_right", link_edge_marker = "gantt_link_point", link_landing_hover_area = "gantt_link_control";
	...
	dnd.attachEvent("onBeforeDragStart", gantt.bind(function(obj, e) {
		...
		if (gantt._locate_css(target, link_edge_marker)) {
			...
			if (gantt._get_safe_type(t.type) == gantt.config.types.milestone) {
				shift = (gantt._get_visible_milestone_width() - gantt
						._get_milestone_width()) / 2;
			}

			this._dir_start = getLinePos(t, !!gantt._link_source_task_start,
					shift);
			return true;
		} else {
			return false;
	...
	dnd.attachEvent("onDragMove", gantt.bind(
			function(obj, e) {
				var dd = dnd.config;
				...
				if (landing) {
					...
					this._dir_end = getLinePos(t,
							!!gantt._link_target_task_start, shift);
				} else {
					this._dir_end = gantt._get_mouse_pos(e);
				}

				var targetChanged = !(prevLanding == landing
						&& prevTarget == targ && prevToStart == to_start);
				if (targetChanged) {
					if (prevTarget)
				...
				showDirectingLine(this._dir_start.x, this._dir_start.y,
						this._dir_end.x, this._dir_end.y);
	...
	function updateMarkedHtml(marker) {
		...
		if (link.from && link.to) {
			if (gantt.isLinkAllowed(link.from, link.to, link.from_start,
					link.to_start)) {
		...
		var className = gantt.templates.drag_link_class(link.from,
				link.from_start, link.to, link.to_start);
		if (className)
			css.push(className);
		...
		var html = "<div class='"
				+ className
				+ "'>"
				+ gantt.templates.drag_link(link.from, link.from_start,
						link.to, link.to_start) + "</div>";
	...
	function showDirectingLine(s_x, s_y, e_x, e_y) {
		...
		if (gantt.templates.link_direction_class) {
			css.push(gantt.templates.link_direction_class(link.from,
					link.from_start, link.to, link.to_start));
		}

		var dist = Math.sqrt((Math.pow(e_x - s_x, 2))
				+ (Math.pow(e_y - s_y, 2)));
		...
		var sin = Math.sin(angle), cos = Math.cos(angle), top = Math.round(s_y), left = Math
				.round(s_x);

		var style = [ "-webkit-transform: rotate(" + angle + "rad)",
		...
		if (window.navigator.userAgent.indexOf("MSIE 8.0") != -1) {
			...
			var shiftLeft = Math.abs(Math.round(s_x - e_x)), shiftTop = Math
					.abs(Math.round(e_y - s_y));
			// fix rotation axis
	...
	function ieTransform(sin, cos) {
		return "progid:DXImageTransform.Microsoft.Matrix(" + "M11 = " + cos
				+ "," + "M12 = -" + sin + "," + "M21 = " + sin + "," + "M22 = "
	...
	function removeDirectionLine() {
		if (dnd._direction) {
			if (dnd._direction.parentNode) // the event line can be detached
											// because of data refresh
...
gantt._task_renderer = function(id, render_one, node, filter) {
	...
	this._task_area_renderers[id] = {
		render_item : function(item, container) {
			...
			// 创建checkBox div
			/*
			 * if(dom.innerHTML != ""){ var e_box =
			 * document.createElement("div"); var checkBox =
			 * document.createElement("input");
			 * checkBox.setAttribute("type","checkbox");
			 * checkBox.setAttribute("name","123456"); e_box.className =
			 * "checkBox"; e_box.appendChild(checkBox); dom = e_box.innerHTML +
			 * dom.innerHTML; }
			 */
		...
		append : function(item, node, container) {
			...
			if (this.rendered[item.id] && this.rendered[item.id].parentNode) {
				...
				if (!(node.innerHTML.indexOf("task") != -1 || node.innerHTML
						.indexOf("line") != -1)) {
					var taskId = item.id;
					...
					if (taskId.indexOf("&") > 0) {
						...
						checkBox.setAttribute("class", "pk");
					}
...
gantt._is_layer = function(dom_element) {
	return (dom_element && dom_element.hasAttribute && dom_element
			.hasAttribute(this.config.layer_attribute));
};
...
gantt._init_tasks = function() {
	...
	var links_layer = this.addLinkLayer({
		...
		filter : gantt._create_filter([ gantt._filter_link,
				gantt._is_chart_visible ].concat(this._get_link_filters()))
	});
	...
	var bar_layer = this.addTaskLayer({
		...
		filter : gantt._create_filter([ gantt._filter_task,
				gantt._is_chart_visible ].concat(this._get_task_filters()))
	});
	...
	this.addTaskLayer({
		...
		filter : gantt._create_filter([ gantt._filter_task,
				gantt._is_grid_visible ].concat(this._get_task_filters()))
	});
	this.addTaskLayer({
		renderer : this._render_bg_line,
		container : this.$task_bg,
		filter : gantt._create_filter([ gantt._filter_task,
				gantt._is_chart_visible, gantt._is_std_background ].concat(this
				._get_task_filters()))
	});
	...
	this._onTaskIdChange = this.attachEvent("onTaskIdChange", function(oldId,
			newId) {
	...
	this._onLinkIdChange = this.attachEvent("onLinkIdChange", function(oldId,
			newId) {
...
gantt._delete_link_handler = function(id, e) {
	if (id && this.callEvent("onLinkDblClick", [ id, e ])) {
		...
		var title = "";
		var question = gantt.locale.labels.link + " "
...
gantt._update_layout_sizes = function() {
	...
	// task bars layer
	var taskHeight = Math.max(gantt._y
			- (gantt._scroll_sizes().x ? gantt._scroll_sizes().scroll_size + 1
					: 0) - 1, 0);
	this.$task_data.style.height = Math.max(taskHeight
			- this.config.scale_height, 0)
			+ 'px';

	if (gantt.config.smart_rendering) {
		this.$task_bg.style.height = gantt.config.row_height
				* this.getVisibleTaskCount() + "px";
	} else {
	...
	if (this._is_grid_visible()) {
		...
		this.$grid_data.style.width = Math.max(width + 30, 0) + "px";
	}
};
...
gantt._init_tasks_range = function() {
	...
	if (this.config.start_date && this.config.end_date) {
		this._min_date = this.date[unit + "_start"](new Date(
	...
	this._min_date = this.calculateEndDate(this.date[unit + "_start"]
			(this._min_date), -1, unit);// one free column before first task

	this._max_date = this.date[unit + "_start"](this._max_date);
	this._max_date = this.calculateEndDate(this._max_date, 2, unit);// one free
																	// column
																	// after
																	// last task
};
...
gantt._prepare_scale_html = function(config, fromPos, toPos) {
	...
	var startIndex = 0, endIndex = config.count - 1;
	...
	for (var i = startIndex; i < endIndex; i++) {
		...
		var value = content.call(this, date), width = config.width[i], height = config.height
				- (this.config.smart_scales ? (config.index ? 1 : 0) : 0), left = config.left[i], style = "", template = "", cssclass = "";

		if (width) {
			var position = this.config.smart_scales ? ("position:absolute;left:"
					+ left + "px")
					: "";

			style = "width:" + (width) + "px;height:" + height + "px;"
					+ position;
			cssclass = "gantt_scale_cell"
					+ (i == config.count - 1 ? " gantt_last_cell" : "");
			...
			var cell = "<div class='" + cssclass + "'" + ariaAttr + " style='"
					+ style + "'>" + value + "</div>";
			cells.push(cell);
		} else {
...
gantt._get_scale_chunk_html = function(scales, fromPos, toPos) {
	...
	for (var i = 0; i < scales.length; i++) {
		...
		html.push("<div class=\"" + cssClass + "\" style=\"height:"
				+ (scales[i].height) + "px;position:relative;line-height:"
				+ (scales[i].height) + "px\">"
				+ this._prepare_scale_html(scales[i], fromPos, toPos)
				+ "</div>");
	}
...
gantt._refreshScales = function() {
	...
	this.$task_scale.innerHTML = this._get_scale_chunk_html(scales, left, left
...
gantt._render_tasks_scales = function() {
	...
	var scales_html = "", outer_width = 0, data_width = 0, scale_height = 0;
	...
	if (this._is_chart_visible()) {
		...
		var avail_width = resize.x ? Math
				.max(this.config.autosize_min_width, 0) : (Math.max(this._x
				- this._get_grid_width() - 2, 0));

		var cfgs = helpers.prepareConfigs(scales, this.config.min_column_width,
				avail_width, scale_height);
		var cfg = this._tasks = cfgs[cfgs.length - 1];
		this._scales = cfgs;

		// var scrollX = this.getScrollState()
		scales_html = this._get_scale_chunk_html(cfgs, 0, this._x
				- this._get_grid_width());

		var scrollSizes = this._scroll_sizes();
		outer_width = cfg.full_width
				+ (this._scroll_sizes().y ? scrollSizes.scroll_size : 0) + "px";
...
gantt._render_bg_line = function(item) {
	...
	if (gantt.config.show_task_cells) {
		for (var j = 0; j < count; j++) {
			...
			if (width > 0) {// do not render skipped columns
				...
				cssclass = "gantt_task_cell"
						+ (j == count - 1 ? " gantt_last_cell" : "");
				cssTemplate = this.templates.task_cell_class(item,
						cfg.trace_x[j]);
				if (cssTemplate)
	...
	var cssTemplate = gantt.templates.task_row_class(item.start_date,
			item.end_date, item);
	var css = "gantt_task_row" + (odd ? " odd" : "")
	...
	// var row = "<div class='" + css + "' " + this.config.task_attribute + "='"
	// + item.id + "'>" + cells.join("") + "</div>";
...
gantt._adjust_scales = function() {
	if (this.config.fit_tasks) {
		var old_min = +this._min_date, old_max = +this._max_date;
		this._init_tasks_range();
...
gantt._combine_item_class = function(basic, template, itemId) {
	...
	if (links.link_landing_area
		...
		var allowDrag = gantt.isLinkAllowed(from_id, itemId, from_start,
				to_start);
...
gantt._get_task_width = function(task, start, end) {
	return Math.round(this._get_task_pos(task, false).x
			- this._get_task_pos(task, true).x);
};
...
gantt._task_default_render = function(task) {
	...
	if (this._get_safe_type(task.type) == cfg.types.milestone
			&& cfg.link_line_width > 1) {
		// little adjust milestone position, so horisontal corners would match
		// link arrow when thickness of link line is more than 1px
		padd += 1;
	}
	...
	var css = this._combine_item_class("gantt_task_line", this.templates
			.task_class(task.start_date, task.end_date, task), task.id);
	if (task.color || task.progressColor || task.textColor) {
		css += " gantt_task_inline_color";
	}
	div.className = css;

	var styles = [ "left:" + pos.x + "px", "top:" + (padd + pos.y) + 'px',
	...
	if (!this._is_readonly(task)) {
		if (cfg.drag_resize && !this._is_flex_task(task)
				&& type != this.config.types.milestone) {
		...
		if (cfg.drag_links && this.config.show_links) {
			gantt._render_pair(div, "gantt_link_control", task, function(css) {
				...
				outer.className = css;
				outer.style.cssText = [ "height:" + height + 'px',
						"line-height:" + height + 'px' ].join(";");
				var inner = document.createElement("div");
...
gantt._render_task_element = function(task) {
	...
	var renderer = painters[this._get_safe_type(task.type)], defaultRenderer = this._task_default_render;
...
gantt._get_conditions = function(leftside) {
	if (leftside) {
		return {
			...
			$target : [ gantt.config.links.start_to_start,
					gantt.config.links.finish_to_start ]
		};
	} else {
		return {
			$source : [ gantt.config.links.finish_to_start,
...
gantt._render_task_content = function(task, width) {
	var content = document.createElement("div");
	if (this._get_safe_type(task.type) != this.config.types.milestone)
		content.innerHTML = this.templates.task_text(task.start_date,
				task.end_date, task);
...
gantt._render_task_progress = function(task, element, maxWidth) {
	...
	pr.innerHTML = this.templates.progress_text(task.start_date, task.end_date,
			task);
...
gantt.posFromDate = function(date) {
	...
	var pos = gantt._tasks.left[Math.min(wholeCells,
			gantt._tasks.width.length - 1)];
...
gantt._day_index_by_date = function(date) {
	...
	/*
	 * var day = null; for (var xind = 0, length = days.length-1; xind < length;
	 * xind++) { // | 8:00, 8:30 | 8:15 should be checked against 8:30 //
	 * clicking at the most left part of the cell, say 8:30 should create event
	 * in that cell, not previous one day = +days[xind+1]; if (pos < day &&
	 * !ignores[day]) break; }
	 */
	...
	return day_ind
			+ ((date - days[day_ind]) / gantt._get_coll_duration(gantt._tasks,
					days[day_ind]));

};
gantt._findBinary = function(array, target) {
	// modified binary search, target value not exactly match array elements,
	// looking for closest one
...
gantt._get_task_coord = function(task, to_start, x_correction) {
	...
	if (to_start || isMilestone) {
		...
		date = (task.end_date || this.calculateEndDate(this
				._default_task_date(task)));
	}
	var x = this.posFromDate(date), y = this.getTaskTop(task.id);
...
gantt._correct_shift = function(start, back) {
	return start -= ((new Date(gantt._min_date)).getTimezoneOffset() - (new Date(
		...
		return {
			...
			add : function(layer) {
				...
				for (var i = 0; i < pending.length; i++) {
					...
					this.renderers[id] = gantt._task_renderer(id,
							layer.renderer, node, layer.filter);
...
gantt._create_filter = function(filter_methods) {
	...
	return function(obj) {
		...
		for (var i = 0, len = filter_methods.length; i < len; i++) {
			...
			if (filter_method) {
				res = res
						&& (filter_method.apply(gantt, [ obj.id, obj ]) !== false);
			}
		}
...
gantt.addTaskLayer = gantt._add_generic_layer(gantt._task_layers, [
		gantt._filter_task, gantt._is_chart_visible ].concat(gantt
		._get_task_filters()));
...
gantt.addLinkLayer = gantt._add_generic_layer(gantt._link_layers, [
		gantt._filter_link, gantt._is_chart_visible ].concat(gantt
		._get_link_filters()));
...
/*
 * tasks and relations { data:[ { "id":"string", "text":"...",
 * "start_date":"Date or string", "end_date":"Date or string",
 * "duration":"number", "progress":"0..1", "parent_id":"string",
 * "order":"number" },...], links:[ { id:"string", source:"string",
 * target:"string", type:"string" },...], collections:{ collectionName:[ {key:,
 * label:, optional:...},... ],... } }
 * 
 * gantt._pull - id to object hash gantt._branch - array of per branch arrays of
 * objects|ids gantt._order - array of visible elements gantt._order_full -
...
gantt._process_loading = function(data) {
	...
	this._init_links(data.links
			|| (data.collections ? data.collections.links : []));
...
gantt._sync_order_item = function(item, hidden) {
	if (item.id !== gantt.config.root_id) { // do not trigger event for virtual
											// root
	...
	if (children) {
		for (var i = 0; i < children.length; i++)
			this._sync_order_item(this._pull[children[i]], hidden
					|| !item.$open);
	}
};
...
/*
 * <data> <task id:"some" parent_id="0" progress="0.5"> <text>My task 1</text>
 * <start_date>16.08.2013</start_date> <end_date>22.08.2013</end_date> </task>
 * <coll_options> <links> <link source='a1' target='b2' type='c3' /> </links>
 * </coll_options> </data>
 */
...
gantt.xml = {
	...
	_copyLink : function(obj) {
		return "<item id='" + obj.id + "' source='" + obj.source + "' target='"
				+ obj.target + "' type='" + obj.type + "' />";
	},
	_copyObject : function(obj) {
		return "<task id='" + obj.id + "' parent='" + (obj.parent || "")
				+ "' start_date='" + obj.start_date + "' duration='"
				+ obj.duration + "' open='" + (!!obj.open) + "' progress='"
				+ obj.progress + "' end_date='" + obj.end_date + "'><![CDATA["
				+ obj.text + "]]></task>";
	},
	serialize : function() {
		var tasks = [];
...
gantt._working_time_helper = {
	...
	_timestamp : function(settings) {
		...
		if ((settings.day || settings.day === 0)) {
			...
			// store worktime datestamp in utc so it could be recognized in
			// different timezones (e.g. opened locally and sent to the export
			// service in different timezone)
			timestamp = Date.UTC(settings.date.getFullYear(), settings.date
					.getMonth(), settings.date.getDate());
		}
		return timestamp;
	},
	...
	_check_is_working_unit : function(date, unit, order) {
		...
		if (order) {
			...
			// i.e. don't check particular hour if the whole day is marked as
			// not working
	...
	// methods for month-year-week can be defined, otherwise always return
	// 'true'
	...
	next_date : function(start, unit, step) {
		var start_value = +start, key = unit + "_" + step;
	...
	get_work_units_between_generic : function(from, to, unit, step) {
		var start = new Date(from), end = new Date(to), step = step || 1;
		var units = 0;
	...
	get_work_units_between_hours : function(from, to, unit, step) {
		...
		var firstDayEnd = gantt.date.add(gantt.date.day_start(new Date(start)),
				1, "day");
		...
		if (end.valueOf() <= firstDayEnd.valueOf()) {
			...
			var startPart = this.get_work_units_between_generic(firstDayStart,
					firstDayEnd, unit, step);
			var endPart = this.get_work_units_between_generic(lastDayStart,
					lastDayEnd, unit, step);

			var hourRange = this.get_work_hours_for_range(firstDayEnd,
					lastDayStart);
	...
	add_worktime : function(from, duration, unit, step) {
		...
		var start = new Date(from), added = 0, step = step || 1, duration = duration * 1;
		...
		if (!gantt.config.work_time) {
			...
			while (added < duration) {
				...
				if (this.is_working_unit(
						step > 0 ? new Date(next.valueOf() - 1) : new Date(next
								.valueOf() + 1), unit))
					added++;
				start = next;
			}
	...
	/*
	 * settings: { date:date, unit:'day'/'hour'..., dir:'future'/'past'/'any'/'' }
	 */
	get_closest_worktime : function(settings) {
		...
		var future_target = new Date(curr), prev_target = new Date(curr), tick = true, maximum_loop = 3000, // be
																											// extra
																											// sure
																											// we
																											// won't
																											...
																											// big
																											// enough
		count = 0, both_directins = (settings.dir == 'any' || !settings.dir);

		var inc = 1;
		if (settings.dir == 'past')
			inc = -1;

		// will seek closest working hour in future or in past, one step in one
...
gantt.getTaskByTime = function(from, to) {
	var p = this._pull, res = [], pos = 0, taken = 0;
...
gantt.isTaskVisible = function(id) {
	...
	var task = this._pull[id];
	if (!((+task.start_date < +this._max_date && +task.end_date > +this._min_date) || gantt
...
gantt._default_task_date = function(item, parent_id) {
	var parent = (parent_id && parent_id != this.config.root_id) ? this
			.getTask(parent_id) : false, startDate = '';
	if (parent) {
		startDate = parent.start_date;
	} else {
		var first = this._order[0];
		startDate = first ? (this.getTask(first).start_date ? this
				.getTask(first).start_date
				: (this.getTask(first).end_date ? this.calculateEndDate(this
						.getTask(first).end_date, -this.config.duration_step)
						: '')) : this.config.start_date
				|| this.getState().min_date;
	}
...
gantt._set_default_task_timing = function(task) {
	task.start_date = task.start_date
			|| gantt._default_task_date(task, this.getParent(task));
	task.duration = task.duration || this.config.duration_step;
	task.end_date = task.end_date
			|| this.calculateEndDate(task.start_date, task.duration);
};
...
gantt._get_duration_unit = function() {
	return (gantt._get_line(this.config.duration_unit) * 1000)
...
gantt.calculateDuration = function(start_date, end_date) {
	var helper = this._working_time_helper;
	return helper.get_work_units_between(start_date, end_date,
			this.config.duration_unit, this.config.duration_step);
};
gantt._hasDuration = function(start_date, end_date) {
	var helper = this._working_time_helper;
	return helper.is_work_units_between(start_date, end_date,
			this.config.duration_unit, this.config.duration_step);
};
...
gantt.calculateEndDate = function(start, duration, unit) {
	...
	return helper.add_worktime(start, Math.abs(duration), unit
			|| this.config.duration_unit, mult * this.config.duration_step);
};
...
gantt._init_task = function(task) {
	...
	if (duration) {
		if (task.start_date && !task.end_date) {
			task.end_date = this.calculateEndDate(task.start_date,
					task.duration);
		} else if (!task.start_date && task.end_date) {
			task.start_date = this.calculateEndDate(task.end_date,
					-task.duration);
		}
	}
	...
	if (!gantt.defined(task.$open)) {
		task.$open = gantt.defined(task.open) ? task.open
				: this.config.open_tree_initially;
	}
	task.$level = this.calculateTaskLevel(task);
	return task;
};
...
gantt._init_task_timing = function(task) {
	...
	if ((task.$no_end === undefined || task.$no_start === undefined)
			&& task_type != this.config.types.milestone) {
		if (task_type == this.config.types.project) {
			// project duration is always defined by children duration
			task.$no_end = task.$no_start = true;
			this._set_default_task_timing(task);
		} else {
			// tasks can have fixed duration, children duration(as projects), or
			// one date fixed, and other defined by nested items
...
gantt.getSubtaskDates = function(task_id) {
	var min = null, max = null, root = task_id !== undefined ? task_id
			: gantt.config.root_id;

	this.eachTask(function(child) {
		if (this._get_safe_type(child.type) == gantt.config.types.project
				|| this.isUnscheduledTask(child))
			return;

		if ((child.start_date && !child.$no_start)
				&& (!min || min > child.start_date.valueOf()))
			min = child.start_date.valueOf();
		if ((child.end_date && !child.$no_end)
				&& (!max || max < child.end_date.valueOf()))
...
gantt._assign_project_dates = function(task, from, to) {
	if (task.$no_start) {
		if (from && from != Infinity) {
			...
			task.start_date = this._default_task_date(task, this
					.getParent(task));
		}
	}
...
gantt._update_parents = function(taskId, silent) {
	...
	if (task.$no_start || task.$no_end) {
		var oldStart = task.start_date.valueOf(), oldEnd = task.end_date
				.valueOf();

		gantt.resetProjectDates(task);

		// not refresh parent projects if dates hasn't changed
		if (oldStart == task.start_date.valueOf()
...
gantt.roundDate = function(config) {
	...
	var date = config.date, steps = config.step, unit = config.unit;
	...
	if (unit == gantt._tasks.unit && steps == gantt._tasks.step
		...
		if (gantt._tasks.trace_x[colIndex]) {
			upper = gantt.date[unit + "_start"](gantt._tasks.trace_x[colIndex]);// end
																				// of
																				// time
																				// scale
		}
		...
		while (+upper < +date) {
			upper = gantt.date[unit + "_start"](gantt.date.add(upper, steps,
					unit));
...
gantt._dp_init = function(dp) {
	...
	dp.serverProcessor += (dp.serverProcessor.indexOf("?") != -1 ? "&" : "?")
			+ "editing=true";
	...
	this.attachEvent("onAfterTaskDelete", function(id, item) {
		...
		// not send delete request if item is not inserted into the db - just
	...
	this.attachEvent("onTaskIdChange", function(oldId, newId) {
		...
		var item = this.getTask(newId), itemLinks = this._get_task_links(item);
	...
	dp.attachEvent("onBeforeDataSending", function() {
		...
		if (this._tMode == "REST") {
			var mode = this._ganttMode.substr(0, this._ganttMode.length - 1);// links,
																				// tasks
																				// ->
																				// /link/id,
																				// /task/id

			url = url.substring(0, url.indexOf("?") > -1 ? url.indexOf("?")
					: url.length);
			// editing=true&
			this.serverProcessor = url + (url.slice(-1) == "/" ? "" : "/")
					+ mode;
		} else {
			this.serverProcessor = url + gantt._urlSeparator(url)
					+ "gantt_mode=" + this._ganttMode;
		}
	...
	dp._getRowData = gantt.bind(function(id, pref) {
		...
		if (dp._ganttMode == "tasks")
			task = this.isTaskExists(id) ? this.getTask(id) : {
				id : id
			};
		else
			task = this.isLinkExists(id) ? this.getLink(id) : {
				id : id
			};
...
gantt._get_linked_task = function(link, getTarget) {
	...
	gantt.assert(task, "Link " + role + " not found. Task id=" + taskId
			+ ", link id=" + link.id);
	return task;
};
...
gantt._isTask = function(task) {
	return (!task.type || task.type != gantt.config.types.project)
			&& !(task.$no_start || task.$no_end);
};
...
gantt._get_timepicker_step = function() {
	if (this.config.round_dnd_dates) {
		var scale = gantt._tasks, step = (this._get_line(scale.unit) * scale.step) / 60;// timepicker
																						// step
																						// is
																						// measured
																						// in
...
gantt.getLightbox = function(type) {
	...
	if (!this._lightbox || this.getLightboxType() != this._get_safe_type(type)) {
		...
		for (var i = 0; i < buttons.length; i++) {
			// needed to migrate from 'dhx_something' to 'gantt_something'
			// naming in a lightbox
			var button = this.config._migrate_buttons[buttons[i]] ? this.config._migrate_buttons[buttons[i]]
					: buttons[i];

			ariaAttr = this._waiAria.lightboxButtonAttrString(button);
			html += "<div " + ariaAttr
					+ " class='gantt_btn_set gantt_left_btn_set " + button
					+ "_set'><div dhx_button='1' class='" + button
					+ "'></div><div>" + this.locale.labels[button]
					+ "</div></div>";
		...
		for (var i = 0; i < buttons.length; i++) {
			...
			html += "<div " + ariaAttr
					+ " class='gantt_btn_set gantt_right_btn_set " + button
					+ "_set' style='float:right;'><div dhx_button='1' class='"
					+ button + "'></div><div>" + this.locale.labels[button]
					+ "</div></div>";
...
gantt._render_sections = function(sns) {
	...
	for (var i = 0; i < sns.length; i++) {
		...
		var button = "";
		if (sns[i].button) {
			button = "<div class='gantt_custom_button' index='" + i
					+ "'><div class='gantt_custom_button_" + sns[i].button
					+ "'></div><div>"
		...
		html += "<div id='" + sns[i].id
				+ "' class='gantt_cal_lsection'><label>" + button
				+ this.locale.labels["section_" + sns[i].name]
				+ "</label></div>" + block.render.call(this, sns[i]);
		html += "</div>";
	}
	return html;
};
...
gantt.resizeLightbox = function() {
	...
	d.style.height = con.scrollHeight + this.config.lightbox_additional_height
			+ "px";
	con.style.height = con.scrollHeight + "px"; // it is incredible , how ugly
												// IE can be
...
gantt._center_lightbox = function(box) {
	if (box) {
		...
		var scroll_top = window.pageYOffset || document.body.scrollTop
				|| document.documentElement.scrollTop;
		var scroll_left = window.pageXOffset || document.body.scrollLeft
				|| document.documentElement.scrollLeft;
		...
		if (scroll_top) // if vertical scroll on window
			box.style.top = Math.round(scroll_top
					+ Math.max((view_height - box.offsetHeight) / 2, 0))
					+ "px";
		else
			// vertical scroll on body
			box.style.top = Math.round(Math.max(
					((view_height - box.offsetHeight) / 2), 0) + 9)
					+ "px"; // +9 for compatibility with auto tests
		...
		if (document.documentElement.scrollWidth > document.body.offsetWidth) // if
																				...
																				// the
																				// window
			box.style.left = Math.round(scroll_left
					+ (document.body.offsetWidth - box.offsetWidth) / 2)
					+ "px";
		else
			// horizontal scroll on the body
			box.style.left = Math
					.round((document.body.offsetWidth - box.offsetWidth) / 2)
					+ "px";
	}
};
gantt.showCover = function() {
	...
	var _document_height = ((document.height !== undefined) ? document.height
			: document.body.offsetHeight);
	var _scroll_height = ((document.documentElement) ? document.documentElement.scrollHeight
			: 0);
	this._cover.style.height = Math.max(_document_height, _scroll_height)
			+ 'px';
...
gantt._init_lightbox_events = function() {
	...
	this
			.event(
					...
					function(e) {
						...
						if (src && className
								&& className.indexOf("gantt_btn_set") === 0) {
							src = src.firstChild;
							className = gantt._getClassName(src);
						}
						if (src && className) {
							var func = gantt
									.defined(gantt.lightbox_events[src.className]) ? gantt.lightbox_events[src.className]
	...
	gantt.getLightbox().onkeydown = function(e) {
		...
		var target = e.target || e.srcElement;
		var buttonTarget = !!(gantt._getClassName(target).indexOf(
				"gantt_btn_set") > -1);
...
gantt._save_lightbox = function() {
	var task = this.getLightboxValues();
	if (!this.callEvent("onLightboxSave", [ this._lightbox_id, task,
			!!task.$new ]))
		return;
...
gantt._set_lightbox_values = function(data, box) {
	...
	if (gantt.templates.lightbox_header) {
		lightboxHeader.push("");
		lightboxHeader.push(gantt.templates.lightbox_header(task.start_date,
				task.end_date, task));
		s[1].innerHTML = "";
		s[2].innerHTML = gantt.templates.lightbox_header(task.start_date,
				task.end_date, task);
	} else {
		lightboxHeader.push(this.templates.task_time(task.start_date,
				task.end_date, task));
		lightboxHeader.push((this.templates.task_text(task.start_date,
				task.end_date, task) || "").substr(0, 70)); // IE6 fix
		s[1].innerHTML = this.templates.task_time(task.start_date,
				task.end_date, task);
		s[2].innerHTML = (this.templates.task_text(task.start_date,
				task.end_date, task) || "").substr(0, 70); // IE6 fix
	}
	...
	for (var i = 0; i < sns.length; i++) {
		...
		if (!this.form_blocks[section.type]) {
			continue;// skip incorrect sections, same check is done during
						// rendering
		}
		...
		var map_to = gantt._resolve_default_mapping(sns[i]);
		var value = this.defined(task[map_to]) ? task[map_to]
...
gantt.getLightboxSection = function(name) {
	...
	var result = {
		...
		getValue : function(ev) {
			return gantt.form_blocks[section.type].get_value.call(gantt, node,
					(ev || {}), section);
		},
		setValue : function(value, ev) {
			return gantt.form_blocks[section.type].set_value.call(gantt, node,
					value, (ev || {}), section);
		}
	};
...
gantt._move_while_dnd = function(e) {
	if (gantt._dnd_start_lb) {
		...
		var now = (e && e.target) ? [ e.pageX, e.pageY ] : [ event.clientX,
				event.clientY ];
		lb.style.top = gantt._lb_start[1] + now[1] - gantt._dnd_start_lb[1]
				+ "px";
		lb.style.left = gantt._lb_start[0] + now[0] - gantt._dnd_start_lb[0]
				+ "px";
	}
};
gantt._ready_to_dnd = function(e) {
	...
	gantt._dnd_start_lb = (e && e.target) ? [ e.pageX, e.pageY ] : [
...
gantt.form_blocks = {
		...
		for (var p = 0; p < time_format.length; p++) {
			...
			case "%m":
				...
				for (var i = 0; i < 12; i++)
					options += "<option value='" + i + "'>"
							+ this.locale.date.month_full[i] + "</option>";
				break;
			case "%d":
			...
			case "%H:%i":
				...
				while (i < last) {
					...
					options += "<option value='" + i + "'>" + time
							+ "</option>";
					sns._time_values.push(i);
					dt.setTime(dt.valueOf() + this._get_timepicker_step() * 60
							* 1000);
					var diff = (dt.getDate() != tdate) ? 1 : 0; // moved or not
																// to the next
																// day
	...
	_fill_lightbox_select : function(s, i, d, map, cfg) {
		...
		if (gantt.defined(map[3])) {
			...
			v = Math.round(v / gantt._get_timepicker_step())
	...
	template : {
		render : function(sns) {
			...
			return "<div class='gantt_cal_ltext gantt_cal_template' style='height:"
					+ height + ";'></div>";
		},
	...
	textarea : {
		render : function(sns) {
			...
			return "<div class='gantt_cal_ltext' style='height:" + height
	...
	select : {
		render : function(sns) {
			...
			for (var i = 0; i < sns.options.length; i++)
				html += "<option value='" + sns.options[i].key + "'>"
						+ sns.options[i].label + "</option>";
			html += "</select></div>";
			return html;
		},
	...
	time : {
		render : function(sns) {
			...
			if (sns.single_date) {
				...
				parts
						.push("<span style='font-weight:normal; font-size:10pt;'> &nbsp;&ndash;&nbsp; </span>");
			}
		...
		set_value : function(node, value, ev, config) {
			...
			if (cfg.auto_end_date) {
				var _update_lightbox_select = function() {
					start_date = new Date(s[map[2]].value, s[map[1]].value,
							s[map[0]].value, 0, 0);
					end_date = gantt.calculateEndDate(start_date, 1);
					this.form_blocks._fill_lightbox_select(s, map.size,
							end_date, map, cfg);
				};
				for (var i = 0; i < 4; i++) {
			...
			var end_date = ev[mapping.end_date]
					|| gantt.calculateEndDate(start_date, 1);

			this.form_blocks._fill_lightbox_select(s, 0, start_date, map, cfg);
			this.form_blocks._fill_lightbox_select(s, map.size, end_date, map,
					cfg);
		},
		...
		get_value : function(node, ev, config) {
			...
			var start_date = new Date(s[map[2]].value, s[map[1]].value,
					s[map[0]].value, hours, minutes);
			...
			var end_date = new Date(s[map[2] + map.size].value, s[map[1]
					+ map.size].value, s[map[0] + map.size].value, hours,
					minutes);

			if (end_date <= start_date)
				end_date = gantt.date.add(start_date, gantt
						._get_timepicker_step(), "minute");
	...
	duration : {
		render : function(sns) {
			...
			var duration = "<div class='gantt_duration' "
					+ singleDate
					+ ">"
		...
		set_value : function(node, value, ev, config) {
			...
			function _calc_date() {
				var start_date = gantt.form_blocks.duration._get_start_date
						.call(gantt, node, config);
				var duration = gantt.form_blocks.duration._get_duration.call(
						gantt, node, config);
			...
			var end_date = ev[mapping.end_date]
					|| gantt.calculateEndDate(start_date, 1);
			var duration_val = Math.round(ev[mapping.duration])
					|| gantt.calculateDuration(start_date, end_date);
		...
		_get_start_date : function(node, config) {
			...
			return new Date(s[map[2]].value, s[map[1]].value, s[map[0]].value,
					hours, minutes);
		},
		...
		get_value : function(node, ev, config) {
			var start_date = gantt.form_blocks.duration._get_start_date(node,
					config);
			var duration = gantt.form_blocks.duration._get_duration(node,
					config);
	...
	parent : {
		_filter : function(options, config, item_id) {
			...
			for (var i = 0; i < options.length; i++) {
				var task = options[i];
				if (task.id == item_id || gantt.isChildOf(task.id, item_id)
						|| filter(task.id, task) === false) {
					options.splice(i, 1);
					i--;
				}
			}
		...
		_display : function(config, item_id) {
			...
			for (var i = 0; i < tasks.length; i++) {
				var label = text.apply(gantt, [ tasks[i].start_date,
						tasks[i].end_date, tasks[i] ]);
				if (label === undefined) {
					label = "";
				}
				options.push({
		...
		set_value : function(node, value, ev, config) {
			...
			return gantt.form_blocks.select.set_value.apply(gantt, [
					newOptions, value, ev, config ]);
		},
		get_value : function() {
...
gantt._silent_redraw_lightbox = function(type) {
	...
	if (this.getState().lightbox) {
		...
		var formData = this.getLightboxValues(), task = this.copy(this
				.getTask(taskId));
	...
	var optional_time = {
		...
		set_value : function(node, value, task, section) {
			...
			if (!task[mapping.start_date]
					|| (mapping.start_date == "start_date" && this
							._isAllowedUnscheduledTask(task))) {
				optional_time.disable(node, section);
				var val = {};

				for ( var i in mapping) {
					// take default values from the time control from task
					// start/end dates
					val[mapping[i]] = task[i];
				}

				return duration.set_value
						.call(gantt, node, value, val, section);// set default
																// value
			} else {
				optional_time.enable(node, section);
				return duration.set_value.call(gantt, node, value, task,
						section);
			}
		},
		...
		update_block : function(node, section) {
			...
			if (section.button) {
				var button = node.previousSibling.firstChild.firstChild, labels = gantt.locale.labels;

				var button_text = section.disabled ? labels[section.name
						+ "_enable_button"] : labels[section.name
						+ "_disable_button"];
		...
		button_click : function(index, el, section, container) {
			if (gantt.callEvent("onSectionButton", [ gantt._lightbox_id,
					section ]) === false) {
				return;
			}
...
gantt.form_blocks.duration_optional = gantt
		._extend_to_optional(gantt.form_blocks.duration);
gantt.form_blocks.time_optional = gantt
		._extend_to_optional(gantt.form_blocks.time);
/**
...
gantt.dataProcessor.prototype = {
	...
	sendData : function(rowId) {
		...
		this.messages = [];
		if (!this.checkBeforeUpdate(rowId)
				&& this
						.callEvent("onValidationError",
								[ rowId, this.messages ]))
			return false;
		...
		if (typeof id != "undefined")
			...
			for ( var key in data)
				if (data.hasOwnProperty(key)) {
					stack.push(this.serialize_one(data[key], key
							+ this.post_delim));
					keys.push(key);
				}
	...
	serialize_one : function(data, pref) {
		...
		for ( var key in data)
			if (data.hasOwnProperty(key)) {
				if ((key == "id" || key == this.action_param)
						&& this._tMode == "REST")
					continue;
				stack.push(this.escape((pref || "") + key) + "="
	...
	_sendData : function(a1, rowId) {
		...
		if (!this.callEvent("onBeforeDataSending", rowId ? [ rowId,
				this.getState(rowId), a1 ] : [ null, null, a1 ]))
			return false;
		...
		var a3 = this.serverProcessor
				+ (this._user ? (gantt._urlSeparator(this.serverProcessor) + [
						"dhx_user=" + this._user,
						"dhx_version=" + this.obj.getUserData(0, "version") ]
						.join("&")) : "");

		if (this._tMode == "GET")
			gantt.ajax.get(a3 + ((a3.indexOf("?") != -1) ? "&" : "?")
					+ this.serialize(a1, rowId), back);
			...
			if (this._payload)
				for ( var key in this._payload)
					url = url + this._urlSeparator(url) + this.escape(key)
							+ "=" + this.escape(this._payload[key]);
	...
	sendAllData : function() {
		...
		if (!valid
				&& !this.callEvent("onValidationError", [ "", this.messages ]))
			return false;
			...
			for (var i = 0; i < this.updatedRows.length; i++)
				if (!this._in_progress[this.updatedRows[i]]) {
					if (this.is_invalid(this.updatedRows[i]))
						continue;
					this._beforeSendData(this._getRowData(this.updatedRows[i]),
							this.updatedRows[i]);
					if (this._waitMode
	...
	afterUpdateCallback : function(sid, tid, action, btag) {
		...
		if ((this._uActions) && (this._uActions[action])
				&& (!this._uActions[action](btag)))
		...
		if (this._in_progress[marker] != "wait") {
			...
			delete this._in_progress[marker];
			this.setUpdated(tid, true, this.obj.getUserData(sid,
					this.action_param));
		}
	...
	afterUpdate : function(that, xml, id) {
		...
		// xml response
		var top = gantt.ajax.xmltop("data", xml.xmlDoc); // fix incorrect
															// content type in
															// IE
		if (!top)
	...
	finalizeUpdate : function() {
		...
		if ((this.obj.mytype == "tree" || this.obj._h2)
	...
	loadUpdate : function() {
		...
		var url = this.serverProcessor
				+ gantt._urlSeparator(this.serverProcessor)
				+ [ "dhx_user=" + this._user, "dhx_version=" + version ]
						.join("&");
		...
		this.getUpdates(url, function(xml) {
			...
			if (upds.length) {
				...
				for (var i = 0; i < upds.length; i++) {
					...
					switch (status) {
					case 'inserted':
						self.callEvent("insertCallback",
								[ upds[i], id, parent ]);
						break;
					case 'updated':
						self.callEvent("updateCallback",
								[ upds[i], id, parent ]);
						break;
					case 'deleted':
						self.callEvent("deleteCallback",
								[ upds[i], id, parent ]);
						break;
					}
				}
...
gantt._update_callback = function(upd, id) {
	...
	for ( var key in data) {
		...
		case "end_date":
			...
			break;
		case "duration":
			objData.end_date = gantt.calculateEndDate(objData.start_date,
					property);
			break;
		}
...
gantt._insert_callback = function(upd, id, parent, mode) {
	var data = upd.data || gantt.xml._xmlNodeToJSON(upd.firstChild), methods = {
		add : gantt.addTask,
...
/*
 * asserts will be removed in final code, so you can place them anythere without
 * caring about performance impacts
 */
gantt.assert = function(check, message) {
	// jshint -W087
	if (!check) {
		if (gantt.config.show_errors
				&& gantt.callEvent("onError", [ message ]) !== false) {
			gantt.message({
				type : "error",
				text : message,
				expire : -1
...
gantt._init_html_area = function(node) {
	...
	var html = "<div class='gantt_container'><div class='gantt_grid' "
			+ gridAriaAttr + "></div><div class='gantt_task'></div>";
	...
	this.$container = this._obj.childNodes[0];
	...
	this.$grid.innerHTML = "<div class='gantt_grid_scale' "
			...
			+ "></div><div class='gantt_grid_data' " + gridDataAriaAttr
			+ "></div>";
...
gantt._calculate_content_height = function() {
	var scale_height = this.config.scale_height, rows_height = this._order.length
			* this.config.row_height, hor_scroll_height = this._scroll_hor ? this.config.scroll_size + 1
			: 0;
...
gantt._calculate_content_width = function() {
	var grid_width = this._get_grid_width(), chart_width = this._tasks ? this._tasks.full_width
			: 0, ver_scroll_width = this._scroll_ver ? this.config.scroll_size + 1
			: 0;
	...
	return grid_width + chart_width + 50;
};
...
gantt._get_box_styles = function() {
	...
	if (window.getComputedStyle) {
		...
		// IE with elem.currentStyle does not calculate sizes from %, so will
	...
	for (var i = 0; i < properties.length; i++) {
		styles[properties[i]] = computed[properties[i]] ? this
				._clean_el_size(computed[properties[i]]) : 0;
	}

	var box = {
		horPaddings : (styles.paddingLeft + styles.paddingRight
				+ styles.borderLeftWidth + styles.borderRightWidth),
		vertPaddings : (styles.paddingTop + styles.paddingBottom
...
gantt._set_sizes = function() {
	...
	// same height
	this.$grid.style.height = this.$task.style.height = Math.max(this._y
			- this.$scroll_hor.offsetHeight - 2, 0)
			+ "px";

	var dataHeight = Math.max((this._y - (this.config.scale_height || 0)
			- this.$scroll_hor.offsetHeight - 2), 0);
	this.$grid_data.style.height = this.$task_data.style.height = dataHeight
			+ "px";

	// share width
	var gridWidth = Math.max(this._get_grid_width() + 30, 0);
	...
	this.$grid_data.style.width = Math.max(this._get_grid_width() + 1, 0)
			+ "px";
	this.$task.style.width = Math.max(this._x - this._get_grid_width() - 2, 0)
			+ "px";
};
...
gantt._save_scroll_state = function(x, y) {
	...
	// getting-setting scrollLeft for restoring scroll position after render
	// takes surprisingly big amount of time
	// 2x-3x times more than setting innerHTML (if using
	// gantt.config.static_background)
	// Will store scroll position in memory instead of getting actual values
	// from DOM
	var pos = {};
...
gantt.scrollTo = function(left, top) {
	...
	if (top * 1 == top) {
		...
		this._save_scroll_state(undefined,
				this.config.show_chart ? this.$task_data.scrollTop
						: this.$scroll_ver.scrollTop);
	}

	var scroll = gantt._restore_scroll_state();
	this.callEvent("onGanttScroll", [ oldScrollState.x, oldScrollState.y,
			scroll.x, scroll.y ]);
};
...
gantt._on_resize = gantt.setSizes = function() {
	...
	// Set_sizes and scroll_resize affects each other result depending each one
	// called first.
	// Need to call set_sizes twice to ensure sizes are calculated correctly
	// after resize
	gantt._set_sizes();
...
gantt.render = function() {
	...
	if (pos) {
		visible_date = gantt
				.dateFromPos(pos.x + this.config.task_scroll_offset);
	}
...
gantt._set_scroll_events = function() {
	this.event(this.$scroll_hor, "scroll", function() {
		// in safari we can catch previous onscroll after setting new value from
		// mouse-wheel event
	...
	this
			.event(
					...
					"scroll",
					function() {
						var left = gantt.$task.scrollLeft, barLeft = gantt.$scroll_hor.scrollLeft;
						if (barLeft != left)
	...
	this
			.event(
					...
					"scroll",
					function() {
						var top = gantt.$task_data.scrollTop, barTop = gantt.$scroll_ver.scrollTop;
						if (barTop != top)
...
gantt._scroll_sizes = function() {
	...
	var scroll_ver = this._scroll_ver = resize.y ? false
			: (task_data_height > task_height);

	var task_data_width = Math.max(this._tasks.full_width
			- (!scroll_ver ? scroll_size : 0), 0);

	var scroll_hor = this._scroll_hor = resize.x ? false
			: (task_data_width > task_width);
	...
	if (scroll_ver) {
		scrolls.y = Math
				.max(
						(this._y - (scroll_hor ? scroll_size : 0) - this.config.scale_height),
						0);
	}
	return scrolls;
};
	...
	while (trg) {
		...
		if (css) {
			...
			if (ind >= 0) {
				...
				var right = ((ind + classname.length >= css.length))
						|| (!gantt._trim(css.charAt(ind + classname.length)));
...
gantt.getState = function() {
	return {
		...
		drag_mode : this._tasks_dnd.drag ? this._tasks_dnd.drag.mode
				: undefined,
		drag_from_start : this._tasks_dnd.drag ? this._tasks_dnd.drag.left
				: undefined,
...
gantt._correct_dst_change = function(date, prevOffset, step, unit) {
	...
	if (time_unit > 60 * 60 && time_unit < 60 * 60 * 24) {
		// correct dst change only if current unit is more than one hour and
		// less than day (days have own checking), e.g. 12h
...
(function() {
	...
	gantt._disableMethod = function(methodName, dummyMethod) {
		dummyMethod = typeof dummyMethod == "function" ? dummyMethod
				: function() {
				};
...
gantt.env = {
	isIE : (navigator.userAgent.indexOf("MSIE") >= 0 || navigator.userAgent
			.indexOf("Trident") >= 0),
	isIE6 : (!window.XMLHttpRequest && navigator.userAgent.indexOf("MSIE") >= 0),
	isIE7 : (navigator.userAgent.indexOf("MSIE 7.0") >= 0 && navigator.userAgent
			.indexOf("Trident") < 0),
	isIE8 : (navigator.userAgent.indexOf("MSIE 8.0") >= 0 && navigator.userAgent
			.indexOf("Trident") >= 0),
	isOpera : (navigator.userAgent.indexOf("Opera") >= 0),
	isChrome : (navigator.userAgent.indexOf("Chrome") >= 0),
	isKHTML : (navigator.userAgent.indexOf("Safari") >= 0 || navigator.userAgent
			.indexOf("Konqueror") >= 0),
...
gantt.ajax = {
	...
	// if false - dhxr param will added to prevent caching on client side
	// (default),
	...
	xmltop : function(tagname, xhr, obj) {
		if (typeof xhr.status == "undefined" || xhr.status < 400) {
			var xml = (!xhr.responseXML) ? gantt.ajax.parse(xhr.responseText
					|| xhr) : (xhr.responseXML || xhr);
		...
		if (obj !== -1)
			gantt.callEvent("onLoadXMLError", [ "Incorrect XML", arguments[1],
					obj ]);
	...
	xpath : function(xpathExp, docObj) {
		...
		if (gantt.env.isIE) {
			...
			var rows = [];
			var first;
			var col = (docObj.ownerDocument || docObj).evaluate(xpathExp,
					docObj, null, XPathResult.ANY_TYPE, null);
	...
	query : function(config) {
		gantt.ajax._call((config.method || "GET"), config.url, config.data
				|| "", (config.async || true), config.callback, null,
				config.headers);
	},
	...
	post : function(url, postData, onLoad) {
		if (arguments.length == 1) {
			postData = "";
				...
				&& (typeof (postData) == "function" || typeof (window[postData]) == "function")) {
			onLoad = postData;
			postData = "";
		} else {
	...
	postLong : function(url, postData, onLoad) {
		if (arguments.length == 2
				&& (typeof (postData) == "function" || typeof (window[postData]))) {
			onLoad = postData;
			postData = "";
		}
		this._call("POST", url, postData, true, onLoad, {
	...
	_call : function(method, url, postData, async, onLoad, longParams, headers) {
		...
		var t = (window.XMLHttpRequest && !gantt.env.isIE ? new XMLHttpRequest()
				: new ActiveXObject("Microsoft.XMLHTTP"));
		var isQt = (navigator.userAgent.match(/AppleWebKit/) !== null
				&& navigator.userAgent.match(/Qt/) !== null && navigator.userAgent
				.match(/Safari/) !== null);

		if (!!async) {
			t.onreadystatechange = function() {
				if ((t.readyState == 4) || (isQt && t.readyState == 3)) { // what
																			// for
																			// long
																			// response
																			// and
						...
						if (typeof (onLoad) == "function") {
							onLoad.apply(window, [ {
								...
								filePath : url
									...
									// response.xmlDoc.responseXML/responseText
						}
						if (longParams) {
							if (typeof (longParams.postData) != "undefined") {
								gantt.ajax.postLong(longParams.url,
										longParams.postData, onLoad);
							} else {
		...
		} else if (method.toUpperCase() == "POST" || method == "PUT"
				|| method == "DELETE") {
...
(function() {
		...
		if (_dhx_msg_cfg) {
			...
			if (gantt.message.keyboard) {
				if (code == 13 || code == 32) {
					...
					var target = e.target || e.srcElement;
					if (gantt._getClassName(target).indexOf(
							"gantt_popup_button") > -1
							&& target.click) {
						target.click();
					} else {
	...
	function button(text, result) {
		...
		var button_css = "gantt_" + text.toLowerCase().replace(/ /g, "_")
				+ "_button" + " dhtmlx_"
				+ text.toLowerCase().replace(/ /g, "_") + "_button"; // dhtmlx_ok_button,
																		// dhtmlx_click_me_button
		return "<div " + buttonAriaAttrs
				+ " class='gantt_popup_button dhtmlx_popup_button "
				+ button_css + "' result='" + result + "' ><div>" + text
				+ "</div></div>";
	}
	...
	function info(text) {
		...
		message.className = "gantt-info dhtmlx-info gantt-" + text.type
				+ " dhtmlx-" + text.type;
	...
	function _boxStructure(config, ok, cancel) {
		...
		box.className = " gantt_modal_box dhtmlx_modal_box gantt-"
				+ config.type + " dhtmlx-" + config.type;
		...
		if (config.title)
			inner += '<div class="gantt_popup_title dhtmlx_popup_title">'
					+ config.title + '</div>';
		inner += '<div class="gantt_popup_text dhtmlx_popup_text" id="'
				+ contentId
				+ '"><span>'
				+ (config.content ? '' : config.text)
				+ '</span></div><div  class="gantt_popup_controls dhtmlx_popup_controls">';
		if (ok)
			inner += button(getFirstDefined(config.ok,
					gantt.locale.labels.message_ok, "OK"), true);
		if (cancel)
			inner += button(getFirstDefined(config.cancel,
					gantt.locale.labels.message_cancel, "Cancel"), false);
		if (config.buttons) {
	...
	function _createBox(config, ok, cancel) {
		...
		var x = Math
				.abs(Math
						.floor(((window.innerWidth || document.documentElement.offsetWidth) - box.offsetWidth) / 2));
		var y = Math
				.abs(Math
						.floor(((window.innerHeight || document.documentElement.offsetHeight) - box.offsetHeight) / 2));
...
gantt.date = {
	...
	date_part : function(date) {
		...
		if (date.getHours()
				...
				(date.getDate() < old.getDate()
						|| date.getMonth() < old.getMonth() || date
						.getFullYear() < old.getFullYear()))
	...
	_add_days : function(date, inc) {
		...
		if (inc >= 0
				...
				(ndate.getDate() <= date.getDate()
						|| ndate.getMonth() < date.getMonth() || ndate
	...
	add : function(date, inc, mode) {
		...
		case "hour":
			/*
			 * adding hours/minutes via setHour(getHour() + inc) gives weird
			 * result when adding one hour to the time before switch to a
			 * Daylight Saving time
			 * 
			 * example: //Sun Mar 30 2014 01:00:00 GMT+0100 (W. Europe Standard
			 * Time) new Date(2014, 02, 30, 1).setHours(2) >>Sun Mar 30 2014
	...
	str_to_date : function(format, utc) {
		...
		for (var i = 0; i < mask.length; i++) {
			...
			case "%s":
				...
				break;
			case "%M":
				splt += "set[1]=gantt.locale.date.month_short_hash[temp[" + i
						+ "]]||0;";
				break;
			case "%F":
				splt += "set[1]=gantt.locale.date.month_full_hash[temp[" + i
						+ "]]||0;";
				break;
			default:
				break;
		...
		if (utc)
			code = " Date.UTC(" + code + ")";
		return new Function("date", "var set=[0,0,1,0,0,0]; " + splt
	...
	getISOWeek : function(ndate) {
		...
		var year_number = first_thursday.getFullYear(); // year of the first
														// Thursday
		var ordinal_date = Math.round((first_thursday.getTime() - new Date(
				year_number, 0, 1).getTime()) / 86400000); // ordinal date of
															// the first
															// Thursday - 1 (so
															// not really
															// ordinal date)
		...
		$.fn.dhx_gantt = function(config) {
			...
			if (typeof (config) === 'string') {
				if (methods[config]) {
					...
					$.error('Method ' + config
							+ ' does not exist on jQuery.dhx_gantt');
				}
			} else {
				var views = [];
				this.each(function() {
	...
	if (this.config.touch != "force")
		this.config.touch = this.config.touch
				&& ((navigator.userAgent.indexOf("Mobile") != -1)
						...
						|| (navigator.userAgent.indexOf("Android") != -1) || (navigator.userAgent
						.indexOf("Touch") != -1));

	if (this.config.touch) {
		if (window.navigator.msPointerEnabled) {
			this._touch_events([ "MSPointerMove", "MSPointerDown",
					"MSPointerUp" ], function(ev) {
				if (ev.pointerType == ev.MSPOINTER_TYPE_MOUSE)
					return null;
				return ev;
			}, function(ev) {
			...
			this._touch_events([ "touchmove", "touchstart", "touchend" ],
					function(ev) {
						if (ev.touches && ev.touches.length > 1)
							return null;
						if (ev.touches[0])
...
// we can't use native scrolling, as we need to sync momentum between different
// parts
...
gantt._touch_events = function(names, accessor, ignore) {
						...
						function(e) {
							...
							if (!gantt._prevent_touch_scroll) {
								if (source && action_start) {
									...
									if (scroll_mode) {
										gantt.scrollTo(scroll_state.x + dx,
												scroll_state.y + dy);
										var new_scroll_state = gantt
												.getScrollState();
	...
	gantt.event(this.$container, names[1], function(e) {
		...
		long_tap_timer = setTimeout(function() {
			...
			if (taskId
					&& !gantt._locate_css(action_start, "gantt_link_control")
					&& !gantt._locate_css(action_start, "gantt_grid_data")) {
	...
	function cloneTaskRendered(taskId) {
		...
		if (task && gantt.isTaskVisible(taskId)) {
			for ( var i in renders) {
				...
				if (task && task.getAttribute("task_id")
						&& task.getAttribute("task_id") == taskId) {
...
(function() {
	...
	function deprecated(badCode, goodCode) {
		...
		var args = [ message ];
		if (!gantt.env.isIE) {
			args = args.concat([ "font-weight:bold", "font-weight:normal",
	...
	var dhtmlxMethods = [ "message", "alert", "confirm", "modalbox", "uid",
			"copy", "mixin", "defined", "bind", "assert" ];
