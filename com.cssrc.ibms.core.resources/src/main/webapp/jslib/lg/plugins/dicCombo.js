...
$(function() {
	...
	function process(dicCombo, prop) {
		...
		var url=ctx + "/oa/system/dictionary/getMapByNodeKey.do";
		
		//是否是在查询区域
		var isSearchForm = false;
		if ($(dicCombo).parents("div.panel-search").length>0) isSearchForm = true;
		var params={nodeKey:prop.nodeKey,isSearchForm:isSearchForm};
		$.post(url,params,function(data){
			...
			$.each(dicList, function(i, d) {
				if(isObjNull(dictValue)==false){
					if(dictValue==d.text){
						//$("#" + prop.valueFieldID).val(d.text);
						$("input[id='"+prop.valueFieldID+"']").val(d.id);
					}
				}else {
					dictValue = d.text;
					ligerComboObj._changeValue(d.dicId,d.itemName);
					return false;
				}
			});
			
			//数据字典高亮
			$(ligerComboObj.element.parentElement.parentElement).addClass("form-component-hightlight");
		});
	}
	// dicCombo
	$.fn.dicCombo = function(option) {
		$(this).each(function() {
