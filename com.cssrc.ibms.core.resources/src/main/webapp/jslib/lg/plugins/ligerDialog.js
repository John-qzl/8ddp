...
(function ($)
{
    ...
    $.ligerDefaults.Dialog = {
        ...
        showToggle: true,                          //是否显示收缩窗口按钮 有无折叠
        ...
        _render: function ()
        {
            ...
            if (p.buttons)
            {
                $(p.buttons).each(function (i, item)
                {
                    ...
                    item.cls && btn.addClass(item.cls);//按钮样式图标
                });
            } else
            {
...
//////////////////////////////###########################################################################
        ...
        extend: function ()
        {
            ...
            //防止多次展开收缩出现页面变矮的
            var contentHeight = g._height  - g.dialog.buttons.outerHeight();
            if (p.slide)
    ...
    {
        $.ligerDialog(p);
       /* $('.l-dialog-winbtn').addClass('abc');
		$('.l-dialog-icon').addClass('abc');
		setTimeout(function () {
			$('.l-dialog-winbtn').removeClass('abc');
			$('.l-dialog-icon').removeClass('abc');
		},400)*/
		return false;
    };
    ...
    $.ligerDialog.alert = function (content, title, type, callback,width,height,allowClose)
    {
        content = content || "";
		...
		$('.l-dialog-winbtn').removeClass('l-dialog-close');
		$('.l-dialog-icon').removeClass('l-dialog-icon');
		setTimeout(function () {
			$('.l-dialog-winbtn').addClass('l-dialog-close');
			$('.l-dialog-tc-inner>div:first-child').addClass('l-dialog-icon');
		},50);
		if (p.type == 'success' || p.type == 'donne' || p.type == 'ok')
        {
            $('.l-dialog-image').removeClass('l-dialog-image-donne');
            setTimeout(function () {
    			$('.l-dialog-image').addClass('l-dialog-image-donne');
    		},50);
        }
        else if (p.type == 'error')
        {     
            $('.l-dialog-image').removeClass('l-dialog-image-error');
            setTimeout(function () {
    			$('.l-dialog-image').addClass('l-dialog-image-error');
    		},50);
        }
        else if (p.type == 'warn')
        {
            $('.l-dialog-image').removeClass('l-dialog-image-warn');
            setTimeout(function () {
    			$('.l-dialog-image').addClass('l-dialog-image-warn');
    		},50);
        }
        else if (p.type == 'question'||p.type == 'confirm')
        {
            setTimeout(function () {
    			$('.l-dialog-image').addClass('l-dialog-image-question');
    		},50);
        }
    ...
    $.ligerDialog.confirm = function (content, title, callback)
    {
        ...
        p = {
            type: 'question',
            content: content,
            buttons: [{ text: $.ligerDefaults.DialogString.yes, onclick: btnclick, type: 'ok', cls: 'l-dialog-btn-ok' }, { text: $.ligerDefaults.DialogString.no, onclick: btnclick, type: 'no', cls: 'l-dialog-btn-no'}]
        };
        if (typeof (title) == "string" && title != "") p.title = title;
        $.extend(p, {
            showMax: false,
    ...
    $.ligerDialog.warning = function (content, title, callback)
    {
        ...
        p = {
            type: 'question',
            content: content,
            buttons: [{ text: $.ligerDefaults.DialogString.yes, onclick: btnclick, type: 'yes', cls: 'l-dialog-btn-ok' }, { text: $.ligerDefaults.DialogString.no, onclick: btnclick, type: 'no', cls: 'l-dialog-btn-no' }, { text: $.ligerDefaults.DialogString.cancel, onclick: btnclick, type: 'cancel', cls: 'l-dialog-btn-cancle'}]
        };
        if (typeof (title) == "string" && title != "") p.title = title;
        $.extend(p, {
            showMax: false,
    ...
    $.ligerDialog.waitting = function (title)
    {
        ...
        return $.ligerDialog.open({ cls: 'l-dialog-waittingdialog', type: 'none', content: '<div style="padding-top:20px;width:25%;margin:0 auto;font-size:14px">' + title + '</div>', allowClose: false });
    };
    $.ligerDialog.closeWaitting = function ()
    {
        ...
        //  $(".l-window-mask").remove();
            if(d.dialog!="undefined"&&d.dialog!=undefined&&d.dialog!=null&&d.dialog!=""){// by YangBo临时添加解决空报错
    ...
    $.ligerDialog.waittingForBack = function (title)
    {
    	title = title || $.ligerDefaults.Dialog.waittingMessage;
    	p = {
            type: 'none',
            content: '<div style="padding-top:18px;width:93%;margin:0 auto;font-size:16px">' + title + '</div>'
        };
        $.extend(p, {
            showMax: false,
            showToggle: false,
    ...
    $.ligerDialog.closeWaittingForBack = function ()
    {
    	var dialogs = l.find(l.controls.Dialog);
        for (var i in dialogs)
        {
    ...
    //不允许关闭
    $.ligerDialog.successNoClose = function (content, title, onBtnClick)
    {
        return $.ligerDialog.alert(content, title, 'success', onBtnClick,"","",false);
    };
    ...
    $.ligerDialog.err = function (title,msg,detail, onBtnClick){   
    	...
    	var obj= $.ligerDialog.alert(content, title, 'error', onBtnClick,420,230);
   ...
   $.ligerDialog.tipDialog = function (title,msg,detail,type,onBtnClick)
    ...
    $.ligerDialog.prompt = function (title, value, multi, callback)
    {
        ...
        p = {
            ...
            target: target,
            width: 320,
            buttons: [{ text: $.ligerDefaults.DialogString.ok, onclick: btnclick, type: 'yes', cls: 'l-dialog-btn-ok' }, { text: $.ligerDefaults.DialogString.cancel, onclick: btnclick, type: 'cancel', cls: 'l-dialog-btn-cancle'}]
        };
        return $.ligerDialog(p);
    };
    //扩展没有关闭按钮
