...
(function($) {
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function() {
            ...
            if (p.valueFieldID) {
                var tableId = p.valueFieldID.split(":")[0];
                if ($("#" + tableId).length > 0) {
                    $.ligerDialog.error("当前表单中存在id为" + tableId + "的元素，造成了数据字典的初始化出错，请更改该元素id！");
                    return;
                }
            ...
            if (p.selectBoxWidth) {
                ...
                g.selectBox.css('width', p.selectBoxWidth + 'px');
            } else {
                //g.selectBox.css('width', g.wrapper.width());
                p.selectBoxWidth = g.wrapper.width();
                g.selectBox.css('width', p.selectBoxWidth + 'px');
            }
        ...
        showIframe: function(selectBox) {
            ...
            obj.show();
            obj.css({ top: selectBox.css("top"), left: selectBox.css("left"), width: selectBox.width() + 2, height: p.selectBoxHeight });
            this.inputText.focus();
        },
        _setWidth: function(value) {
            var g = this;
            if (value > 20) {
                //g.wrapper.css({ width: "100%" });//l-text l-text-combobox宽度
                //g.inputText.css({ width: "90%" });//数据字典输入框宽度
                //对查询条件中的数据字典样式做特殊处理
                if (g.inputText.parents(".panel-search").length > 0) {
                    g.inputText.css({ border: 0 });
                }
                //g.selectBox.css('width', '278px');
                //g.textwrapper.css({ width: value });//数据字典textwrapper 的div宽度
            }
        },
        _setHeight: function(value) {
            ...
            if (value > 10) {
                //ie下高度做调整
                if (ie) {
                    //g.wrapper.height(value-5);
                    //g.inputText.height("66%");
                    g.link.height("100%"); //数据字典开关
                } else {
                    //g.wrapper.height(value-4);//数据字典combobox外框高度
                    //g.inputText.height("66%");//数据字典input高度
                    g.link.height("100%");
                }

                //g.textwrapper.css({ height: value+8 });
            }
        },
        ...
        updateSelectBoxPosition: function() {
            ...
            if (p.absolute) {
            	...
            	var c = g.wrapper.offset().top + 3 + g.wrapper.outerHeight() ;
            	var s = g.selectBox.outerHeight();
            	var t = g.wrapper.offset().top + 3 + g.wrapper.outerHeight();
            	if(c+s>w){
            		t = g.wrapper.offset().top - s;
            	}
                g.selectBox.css({ left: g.wrapper.offset().left - 1, top: t});
            } else {
