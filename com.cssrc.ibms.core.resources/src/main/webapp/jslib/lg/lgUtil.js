...
function initData(context) {
    ...
    grid = $("#grid").ligerGrid({
        ...
        usePager: true, //是否用分页
        url: "getList.do", //url  这里使用url 那么dataAction需要设为'server'
        root: "Rows", //json数据的开头  不需要修改
        record: 'Total', //总数的开头  不需要修改
        dataAction: 'server', //数据从服务器获取
        pageSize: 10, //当前页的显示数据大小
        pageSizeOptions :[5,10,15,20, 30, 40, 50, 100,150,200,500],
        width: 'auto', //设置宽度
...
function delFunction(index, id) {
    ...
    $.ligerDialog.confirm('确认删除所选数据吗？', '提示信息', function(rtn) {
        if (rtn) {
            $.post("del.do", params, function(response) {
...
function submitChange(rowid, id) {
    ...
    $.post("save.do", jsonObj, function(response) {
