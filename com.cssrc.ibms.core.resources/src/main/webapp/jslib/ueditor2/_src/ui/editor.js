        ...
        initEditorUI:function () {
            ...
            var popup = new baidu.editor.ui.Popup({
                	...
                	if(child){
                		...
                		switch(child.tagName.toLowerCase())
                		{
	                		...
	                		case "textarea":
	                			...
	                			switch(name){
	                			case "tablefilesetting":
	                				editor.curTableFile=popup.anchorEl;                			
		            	    		editor.ui._dialogs['tablefileDialog'].open();
		                			break;
	                			default :	
                		...
                		{
	                		case "input_groupopinion":
	                			editor.curGroupopinion=popup.anchorEl;
	            	    		editor.ui._dialogs['groupopinionDialog'].open();
	                			break;
	                	}
                ...
                _onInputDialogButtonClick:function(){
                	...
                	if(child){
            			...
            			//  执行方法定义在  WebRoot/jslib/ueditor2/_src/extend/plugins/customdialog.js
                ...
                //⑩扩展
                //删除分组
                _onDivEditButtonClick:function(){
                	this.hide();
                	if(popup.anchorEl){
                		//删除整个分组
                		popup.anchorEl.parentNode.remove();
                	}
                },
                //分组后插入行
                _onDiAftAddRowButtonClick:function(){
                	this.hide();
                	if(popup.anchorEl){
                		//分组后插入行
                		var parentNode=popup.anchorEl.parentNode;
                		var className=parentNode.className;
                		var input='<div class="tableNew_con clearfix ">'
                			...
                			+'</div>'	
                			+'</div>';		
            			parentNode.innerHTML=parentNode.innerHTML+input;
                		
                	}
                },
                //分组前插入行
                _onDiPreAddRowButtonClick:function(){
                	this.hide();
                	if(popup.anchorEl){
                		//分组前插入行
                		var parentNode=popup.anchorEl.parentNode;
            			...
            			+'</div>'	
            			+'</div>';	
            			parentNode.innerHTML=input+parentNode.innerHTML;
                		
                	}
                },
                //后插入div
                _onDiAftAddDivButtonClick:function(){
                	this.hide();
                	if(popup.anchorEl){
                		//后插入div
                		var parentNode=popup.anchorEl.parentNode;
                		var className=parentNode.className;
                		var input=' <div class=" tableBox ">new div html'+'</div>';	
            			parentNode.innerHTML=parentNode.innerHTML+input;
                		
                	}
                },
                //追加div
                _onDivAppendDivButtonClick:function(){
                	...
                	if(popup.anchorEl){
                		...
                		var className=parentNode.className;
                		var input=' <div class=" tableBox ">new div html'+'</div>';	
            			parentNode.outerHTML=parentNode.outerHTML+input;
            ...
            if (editor.options.imagePopup) {
                editor.addListener('mouseover', function (t, evt) {
    	    		...
    	    		var divedit=el.parentElement.className=="tableNew_title";
    	    		...
    	    		if(ifEl||ifElParent||divedit){
    	    			if(readOnly){
    	    				...
    	    				if(isflag){
		                       ...
		                       '<nobr>'+editor.getLang("property")+': <span onclick="$$._onInputCutButtonClick(event, this);" class="edui-clickable">剪切</span>&nbsp;&nbsp;'+
		                        ...
		                        '<span onclick="$$._onInputDialogButtonClick(event, this);" class="edui-clickable">自定义对话框</span>&nbsp;&nbsp;' +
		                       '</nobr>' );
    	    				}else{
    	    				 var html = popup.formatHtml(
		                       '<nobr>'+editor.getLang("property")+': <span onclick="$$._onInputDelButtonClick()" class="edui-clickable">'+editor.getLang("delete")+'</span>&nbsp;&nbsp;' +	                       
    				   ...
    				   if(isSelect){
    					   ...
    					   //div 形式的编辑表格
    					   html = popup.formatHtml(
	    	                       '<nobr>'+editor.getLang("property")+': <span onclick="$$._onDivEditButtonClick()" class="edui-clickable">'+editor.getLang("delete")+'</span>&nbsp;&nbsp;' +	                       
	    	                       '<span onclick="$$._onDiAftAddRowButtonClick(event, this);" class="edui-clickable">后插入行</span>&nbsp;&nbsp;' +
	    	                       '<span onclick="$$._onDiPreAddRowButtonClick(event, this);" class="edui-clickable">前插入行</span>&nbsp;&nbsp;'+
	    	                       '<span onclick="$$._onDiAftAddDivButtonClick(event, this);" class="edui-clickable">后插入div</span>&nbsp;&nbsp;'+
	    	                       '<span onclick="$$._onDivAppendDivButtonClick(event, this);" class="edui-clickable">追加div</span>&nbsp;&nbsp;'+
