		...
		if( strxy!="" && (strxy.split("&").length==2)){
			...
			DialogUtil.open({
				...
				sucCall:function(retVal){
					...
					var signUrl=__ctx + "/oa/system/sysFile/getFileById.do?fileId=" + retVal.fileId;
					try{
...
//		var aryInput = $("input:text,input:hidden,textarea,select,input:checkbox,input:radio,span");
	...
	{	
		if(power=="w"||right=="b"){
			var path= __ctx + "/oa/system/sysFile/saveFileInfo.do";
	...
	{	
    	if((this.espFileId!="")&&(this.espFileId!=null)&&('undefined' != typeof (this.espFileId))){
    		var path= __ctx + "/oa/system/sysFile/getFileById.do?fileId=" + this.espFileId;
