...
(function($) {
    ...
    liger.win = {
        ...
        mask: function(win) {
            ...
            var windowMask = $("div[id=mask-"+win.id+"]");
            if (!windowMask || windowMask.length==0){
            //if (!this.windowMask) {
                //this.windowMask = $("<div class='l-window-mask' style='display: block;'></div>").appendTo('body');
            	windowMask = $("<div class='l-window-mask' style='display: block;' id='mask-"+win.id+"'></div>").appendTo('body');
        ...
        setFront: function(win) {
            ...
            for (var i in wins) {
                var w = wins[i];
                if (w == win) {
                	var zIndex = i*10+9200;
                    $(w.element).css("z-index", zIndex);

                    var windowMask = $("div[id=mask-"+w.id+"]");
                    if (windowMask && windowMask.length>0){
                    	windowMask.css("z-index",zIndex-1);
                    }
                    this.activeTask(w);
                } else {
                	var zIndex = i*10+9100;
                    $(w.element).css("z-index", zIndex);
                    var windowMask = $("div[id=mask-"+w.id+"]");
                    if (windowMask && windowMask.length>0){
                    	windowMask.css("z-index",zIndex-1);
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
                ...
                if (p.load) {
                    ...
                    setTimeout(function() {
                        if (g.dialog.body.find(".l-dialog-loading:first").length == 0)
                            g.dialog.body.append("<div class='l-dialog-loading' style='display:block;'><img class='l-dialog-loading-img' src='../../styles/default/css/Aqua/images/common/loading.gif' /></div>");
        ...
        _setHeight: function(value) {
            ...
            if (value == "auto") {
                ...
                var height = value - this._borderY - g.dialog.buttons.outerHeight() - g.dialog.header.outerHeight();
        ...
        _setImage: function() {
            var g = this,
                p = this.options;
            if (p.type) {
                var alertCss = { paddingLeft: 74, paddingRight: 15, paddingBottom: 50 };
...
(function($) {
    ...
    $.ligerDefaults.Form.editors.textarea = {
        create: function(container, editParm, p) {
            var editor = $('<textarea class="x-textarea-mltext" />');
    ...
    $.ligerDefaults.Form.editorBulider = function(jinput) {
        ...
        if (jinput.is("select") && ltype != "none") {
            ...
            jinput.addClass("x-textarea-mltext");
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        showFieldError: function(name, errorText) {
            ...
            if (element.hasClass("x-textarea-mltext")) {
        ...
        selectTab: function(index) {
            ...
            var links = jtab.find(".ui-tabs-nav li"),
                contents = jtab.find(".ui-tabs-panel");
...
(function($) {
    ...
    $.ligerui.controls.Layout.ligerExtend($.ligerui.core.UIComponent, {
        ...
        setLeftCollapse: function(isCollapse) {
            ...
            if (g.isLeftCollapse) {
                var iconArr = [];
                for (var int = 0; int < $('#sidemenu>li').length; int++) {
                    iconArr[int] = $('#sidemenu>li').eq(int).attr('iconname')
                }
                if ($('.l-layout-collapse-left div').length == 1) {
                    for (var int = 0; int < iconArr.length; int++) {
                        $('.l-layout-collapse-left .l-layout-collapse-left-toggle').before('<div style="margin:5px 0;width:100%;height:20px;color:white;text-align:center;" class=' + iconArr[int] + '></div>')
                    }
                } else {
                    null;
                }
...
(function($) {
    ...
    $.ligerui.controls.Spinner.ligerExtend($.ligerui.controls.Input, {
        ...
        _getDefaultValue: function() {
            var g = this,
                p = this.options;
            if (p.type == 'float' || p.type == 'int') { return 0; } else if (p.type == 'time') { return "00:00"; }
        },
...
(function($) {
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function() {
            ...
            $("> div", g.tab.content).each(function(i, box) {
                ...
                if (contentitem.attr("icon")) {
                    $("a", li).addClass(contentitem.attr("icon"));
                }
        ...
        selectTabItem: function(tabid) {
            ...
            var _currentLi = $("li[tabid=" + tabid + "]", g.tab.links.ul);
            var _ul = g.tab.links.ul;
            var leftDistance = _currentLi.offset().left - _currentLi.closest('.l-tab-links').offset().left;
            var boxWidth = _currentLi.closest('.l-tab-links').width();

            var leftVal = parseInt(_ul.css('left'));

            if (leftDistance < 0) {
                var leftNeed = leftVal - leftDistance + 140;
                _ul.css('left',leftNeed+'px');
            } else if (leftDistance > boxWidth) {
                var leftNeed = leftVal - (leftDistance - boxWidth) -140;
                _ul.css('left',leftNeed+'px');
            }
        ...
        addTabItem: function(options) {
            ...
            if (g.makeFullHeight) {
                ...
                contentitem.height(g.tab.content.height());
            }
            tabitem.attr("tabid", tabid);
            contentitem.attr("tabid", tabid);
            if (url) {
            ...
            $("a", tabitem).text(text);
            //add icon font
            if(icon!=undefined) $("a", tabitem).addClass(icon);
        ...
        setContentHeight: function() {
            ...
            var newheight = g.tab.height() - g.tab.links.height();
            g.tab.content.height(newheight);
            $("> .l-tab-content-item", g.tab.content).height(g.tab.content.height());
        },
