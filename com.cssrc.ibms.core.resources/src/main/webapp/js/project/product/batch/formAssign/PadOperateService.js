...
/*    var records = [];
    ...
    var acceptancePlanId = $.getParameter("acceptancePlanId");
    var url = __ctx + "/dataPackage/tree/ptree/checkReport.do";
    var check;
    $.ajax({
        type: "POST",
        url: url,
    ...
    if (check) {
        ...
        $.ligerDialog.warn("当前策划未下发验收总结模板。请先下发验收总结模板！");
    }
    /*----------------------------服务器代码终点----------------------------*/
}
...
PadOperater.init = function (menu) {
    var menuJSON = [
        {
            text: '下发到摆渡机', click: function () {
                PadOperater.downLoad();
            }
        },
        {
            text: '取消下载到摆渡机', click: function () {
                PadOperater.cancelDownLoad();
            }
        },
    ...
    $(menu).ligerMenuBar({
        items: [
            {
                text: '<a href="####" class="link run"><span></span>摆渡机操作集合&nbsp;&nbsp;</a>',
                menu: {width: 160, items: menuJSON}
            }
        ]
    });
