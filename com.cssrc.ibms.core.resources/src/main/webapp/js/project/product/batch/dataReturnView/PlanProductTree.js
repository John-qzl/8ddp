 ...
 * 批次-验收策划--产品结构树
 */
PlanProductTree = function(catKey, divId, conf, mappingUrl, id, nodeName, acceptancePlanId) {
	{
		this.catKey = catKey;
		this.divId = divId;
		this.conf = conf;
		...
		$(function() {
			$.ajax({
				...
				data:{
					...
					acceptancePlanId : _self.acceptancePlanId
		        },
				...
				error : function() {
					$.ligerDialog.error('产品验收策划--产品结构树加载失败！');
				}
			});
		});
	};
...
// 产品批次-验收策划结构树左击事件 
function planProductTreeOnClick(event, treeId, treeNode) {
	var url = null;
	if (treeNode.type == "product"){
		// 验收策划-产品节点（演示虚构）
		var clickUrl = "/oa/form/dataTemplate/preview.do?__displayId__=10000022400033";
//		url = __ctx + clickUrl + "&projectId=10000021920521&projectName=制导DK&__dbomSql__=F_SSSJB=10000021920882";
		url = __ctx + clickUrl + "&projectId=" + treeNode.id + "&__dbomSql__=F_PRODUCTID=" + treeNode.id;
	}else {
		// --展示明细
