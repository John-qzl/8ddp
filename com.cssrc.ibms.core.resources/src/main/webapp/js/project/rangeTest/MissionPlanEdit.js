...
$(function () {
    ...
    $("#importTeamFromExcel").on("click", function () {
        var url = __ctx + "/mission/importTeamForPlan.do";
        title = 'excel导入组员信息';
        url = url.getNewUrl();
        url = encodeURI(url);
   ...
   //     if (handleType == "add") {
        ...
        //    $("input[name='m:bcrwchbgb:gdlczt']").val("未生成PDF");
            //		这个用脚本做了
            //		$("input[name='m:bcrwchbgb:spzt']").attr("value", "未审批");
 //       } else if (handleType == "start") {

            $("input[name='m:bcrwchbgb:gdlczt']").attr("value", "未归档");
            //填充策划报告编号
        var date = new Date();
        var year = date.getFullYear();
        var lastNextNumber="";
        $.ajax({
            url:__ctx+"/rangeTest/mission/plan/getNextRangeTestPlanNumber.do",
            data:{'xhId':moduleId},
            dataType:"json",
            async:false,
