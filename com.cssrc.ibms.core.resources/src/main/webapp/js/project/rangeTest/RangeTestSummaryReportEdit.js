...
$(function(){
    ...
    if(typeof(taskDefinitionKey) != "undefined") {
        if (taskDefinitionKey == "task3" || taskDefinitionKey == "task4" || taskDefinitionKey == "task5") {
    ...
    if(type=="add"||type=="startAdd"){
     ...
     //   $("#originFile").hide();//隐藏试验依据文件的原生控件
            ...
            success:function(data){
                if(data!=null&&data!=""){
               	...
               	if(data.F_SYYJWJ!=null){
	    			...
	    			if(file.length>0){
	    				$("textarea[name='m:bcsybgb:syyjwj']").attr("value",data.F_YSYJWJ);
        ...
        //显示验收组长的签章
/*        var fileId=$("#syzzqs").html();
        fileId=fileId.substring(1)
        $("#syzzqspic").attr("src","/dp/oa/system/sysFile/getFileById.do?fileId="+fileId);
        //显示产品验收里的 承制方的签章(在靶场里是 测试方
        var fileId1=$("#syfqs").html();
        fileId1=fileId1.substring(1)
        $("#syfqspic").attr("src","/dp/oa/system/sysFile/getFileById.do?fileId="+fileId1);*/
    }
})
