 ...
 * （型号）通用模板管理结构树定义js
 */
CommonTemplateTree=function(catKey,divId,conf,mappingUrl,id,nodeName){
	{
		this.catKey=catKey;
		//ztree的id
		this.divId=divId;
	...
	this.loadGlobalTree=function(){
		...
		$(function(){
			$.ajax({
		        ...
		        type: 'POST',
		        dataType : "json",
		        url: __ctx + _self.mappingUrl + "getTreeData.do?moduleId="+_self.id+"&moduleName="+_self.nodeName,
...
function importTemplate(){
	...
	var url=__ctx+"/product/category/batch/importTemplate.do";
	DialogUtil.open({
		height:h*0.4,
		width: w*0.4,
		title : '产品导入',
