...
<head>
    <!-- 这是靶场实验 - 某一型号 - 全部任务 的 表单模板 页面
    实则是用来复制通用模板到此页面的表单建模页面 -->
    <title>表单模板管理</title>
    ...
    <script type="text/javascript" src="${ctx}/js/dataPackage/tree/missionFormTableTree.js"></script>
...
/*       fcid是型号id*/
        var id = "<%=request.getParameter("fcId")%>";
        /*       fcid是“全部任务" */
        var nodeName = "<%=request.getParameter("fcName")%>";
        ...
        function onClick(event, treeId, treeNode) {
            ...
            if (treeNode.id == "1") {
                ...
                //表单模板
                    ...
                    //检查项页
                    var itemUrl = "${ctx}/oa/form/dataTemplate/preview.do?__displayId__=10000000720002&Q_table_temp_ID_S=" + treeNode.id;
                    //检查条件页
        ...
        function zTreeOnRightClick(event, treeId, treeNode) {
    		...
    		$.ajax({
    					...
    					url : "${ctx}/model/user/role/getUseRoleByTest.do",
        ...
        function createFolder() {
            ...
            if (node != null) {
                if (node.type == "folder" || node.type == "root") {
                    DialogUtil.open({
                        url: "${ctx}/oa/form/dataTemplate/editData.do?__displayId__=10000000720049&pid=" + id + "&fcid=" + node.id + "&flag=create",
                        width: 800,
                        height: 700,
        ...
        function editFolder() {
            ...
            if (node != null) {
                if (node.type == "folder" && node.parentId != -1) {
                    DialogUtil.open({
                        url: "${ctx}/oa/form/dataTemplate/editData.do?__displayId__=10000000720049&__pk__=" + node.id + "&pid=" + id + "&flag=editor",
                        width: 800,
                        height: 700,
                        sucCall: function (rtn) {
                            reFresh();
                    ...
                    if (node.type == "folder") {
                        ...
                        if (len == undefined) {
                            $.ligerDialog.confirm('是否删除该文件夹', function (rtn) {
                                if (rtn) {
                                    url = "${ctx}/dp/form/dpDataTemplate/deleteData.do?__displayId__=10000000720049&__pk__=" + node.id;
					            ...
					            if (node != null) {
					                if (node.type == "folder" || node.type == "root") {
					                    DialogUtil.open({
					                        ...
					                        url: "${ctx}/dp/form/addFormTemplate.do?pid=" + id + "&fcid=" + node.id,
					                        width: 1200,
					                        height: 700,
					                        title: "新增模板",
					                        sucCall: function (rtn) {
                                    ...
                                    DialogUtil.open({
                                        //	url:"${ctx}/editor/checkform.jsp?pid="+id+"&fcid="+node.id,
                                        url: "${ctx}/dp/form/addFormTemplate.do?pid=" + id + "&fcid=" + id,
                                        width: 1200,
                                        height: 700,
                                        title: "新增模板",
                                        sucCall: function (rtn) {
        ...
        function editModel() {
            ...
            $.post(url, params, function (data) {
                if (data.success == "true") {
                    ...
                    if (isCreatePeople == "1") {
                        DialogUtil.open({
                            url: "${ctx}/dp/form/editFormTemplate.do?id=" + id + "&fcName=" + nodeName,
                            width: 1200,
                            height: 700,
                            title: "编辑表单模板",
                            sucCall: function (rtn) {
            ...
            if (node != null) {
                if (node.type != "folder" && node.type != "root") {
                    ...
                    if (isCreatePeople == "1") {
                        ...
                        $.ligerDialog.warn("只有表单模版创建人或者项目办该型号负责人才有权限编辑该表单模版");
                    }*/
                            }else {
                                $.ligerDialog.warn("您没有权限！");
                            }
    						...
    						if (data.useRole == "1") {
    							 var node = pTree.currentNode;
    					            ...
    					            if (node != null) {
    					                if (node.type == "folder" || node.type == "root") {
    					                    DialogUtil.open({
    					                        url: "${ctx}/editor/tempFileUpload.jsp?pid=" + id + "&fcid=" + node.id + "&type=" + "excel"+"parentId="+id,
    					                        height: 300,
    					                        width: 500,
    					                        title: "导入模板Excel",
    					                	    isResize: true,
                                        ...
                                        DialogUtil.open({
                                            url: "${ctx}/editor/tempFileUpload.jsp?pid=" + id + "&fcid=" + "1" + "&type=" + "excel"+"parentId="+id,
                                            height: 300,
                                            width: 500,
                                            title: "导入模板Excel",
                                            isResize: true,
    					...
    					success : function(data) {
    						if (data.useRole == "1") {
    				            ...
    				            if (node != null) {
    				                if (node.type == "folder" || node.type == "root") {
    				                    DialogUtil.open({
    				                        url: "${ctx}/dp/form/selectMutilModel.do?fcId=" + xhId + "&fcName=" + nodeName + "&nodeid=" + node.id,
    				                        width: 300,
    				                        height: 500,
    				                        title: "表单模板选择器",
    				                        sucCall: function (rtn) {
        ...
        function copyCommon(){
    		...
    		$.ajax({
    					...
    					success : function(data) {
    						if (data.useRole == "1") {
    							DialogUtil.open({
    				                url: "${ctx}/template/manage/commonTemplateCopyView.do?categoryId=" + id,
    				                width: 300,
    				                height: 500,
    				                title: "通用模板选择器",
    				                sucCall: function (rtn) {
...
<div id="defLayout">
    <div position="left" title="表单模板"
        ...
        <div class="tree-toolbar" id="pToolbar">
            ...
            <div class="group"><a class="link copy" id="copyCommon"
                                  href="javascript:copyCommon()">复制模板</a></div>
            ...
            <div class="group"><a class="link collapse" id="TemplateListDelete" href="javascript:createModel()">新增模板</a></div>
            <div class="group"><a class="link collapse" id="TemplateListDelete" href="javascript:uploadFile()">Excel导入</a></div>
            <div class="group">
        ...
        <div class="layui-tab layui-tab-brief template2">
            ...
            <div class="layui-tab-content">
                <div class="layui-tab-item layui-show">
                    ...
                    <br/>
                    <h2><font face="arial" color="red">★规范1：</font>清单项表单模板中只能是以行列式的表格！</h2>
                    <br/>
                    ...
                    <br/>
                    <h2><font face="arial" color="red">★规范2：</font>清单项表单模板中不允许出现单元格合并的情况！</h2>
                    <br/>
                    <h2><font face="arial" color="red">★规范3：</font>清单项表单模板中不允许出现图片！（数学公式请使用普通字符代替）</h2>
                    <br/>
                    <br/>
                    <br />
					<h2>模板制作方式:</h2>
