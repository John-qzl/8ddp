<%@page import="com.cssrc.ibms.core.util.string.StringUtil"%>
<%@page language="java" pageEncoding="UTF-8"
	import="java.util.*,com.cssrc.ibms.worktime.model.CalendarSetting"%>
...
<script type="text/javascript"
	src="${ctx}/servlet/ValidJs?form=sysCalendar"></script>
<link rel="stylesheet" href="${ctx}/styles/default/css/ibms/calendar.css"></link>
...
<script type="text/javascript">
		...
		function jumpTo(flag,year,month){
			var url = 'edit.do?id=${sysCalendar.id}&wtYear='+year+'&wtMon='+month;
			if(flag=='pre'){
	...
	<div class="panel">
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<div class="group">
						<a class="link prev" id="previousMonth"
							href="javascript:noticeSave('pre',${wtYear},${wtMon})"><span></span>上月</a>
					</div>
					<div class="l-bar-separator"></div>
					<div class="group">
						<a class="link next" id="nextMonth"
							href="javascript:noticeSave('next',${wtYear},${wtMon})"><span></span>下月</a>
					</div>
					<div class="l-bar-separator"></div>
					<div class="group">
						<form id="searchForm" method="post" action="edit.do">
							<ul>
								<li style="float: left;"><span class="label"
									style="min-width: 0px;">年份:</span></li>
								<li style="float: left; margin-top: 2px;"><select
									id="wtYear" name="wtYear">
										<c:forEach var="year" items="${yearlist}">
											<option value="${year}"
												<c:if test="${wtYear==year}">selected="selected"</c:if>>${year}年</option>
										</c:forEach>
								</select></li>
								<li style="float: left;"><span class="label"
									style="min-width: 0px;">月份:</span></li>
								<li style="float: left; margin-top: 2px;"><select
									id="wtMon" name="wtMon">
										<c:forEach var="i" begin="1" end="12" step="1">
											<option value="${i}"
												<c:if test="${wtMon==i}">selected="selected"</c:if>>${i}月</option>
										</c:forEach>
								</select></li>
								<li style="float: left;">&nbsp;&nbsp;<input type="hidden"
									name="id" value="${sysCalendar.id}" />
								</li>
								<li style="float: left;"><a href="javascript:;"
									class="link search" onclick="jump()"><span></span>跳转</a></li>
							</ul>
						</form>
					</div>
					<div class="l-bar-separator"></div>
					<div class="group">
						<a class="link back" href="list.do"><span></span>返回</a>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="panel-body">
			<div class="panel-data">
				<form id="sysCalendarForm" method="post" action="save.do">
					...
					<table class="table-detail" cellpadding="0" cellspacing="0"
						...
						<tr>
							<th style="text-align: left;">
								<ul class="row">
									<li>名称: <input type="text" size="10" id="name" name="name"
										value="${sysCalendar.name}" class="inputText" /></li>
									<li>描述:<input type="text" id="memo" name="memo"
										value="${sysCalendar.memo}" class="inputText" /></li>
								</ul>
							</th>
							<th style="text-align: left;">
								<ul class="row">
									<li style="margin-left: 5px; min-width: 100px;">班次： <select
										id="selWt" name="selWt" onchange="changeCboWt(this)">
											<option value="1" selected="selected">上班</option>
											<option value="2">休息</option>
									</select></li>
									<li style="min-width: 100px;"><span id="spanWorkTime">
											时间： <select id="selWtSet" name="selWtSet">
									...
									<li><span
										style="color: red; font: 80%; margin-right: 5px; line-height: 24px;">点击日期选中进行修改</span></li>
									<li style="min-width: 90px;"><a href='#' class='button'
										onclick="changeSelItemWt()"><span>修改选中项</span></a></li>
					...
					<table id="cal" class="wtMonth">
						<caption>
							<span class="wtDate">${wtYear}年${wtMon}月</span> <span
								class="monthFlag"> <c:if test="${monthFlag==true}">
							此月份已排班
						</c:if>
							</span>
							...
							<td column="<%=index%>" class="<%=clsName %>">
								...
								<div name="divDay" day="<%=curDay%>">
									...
									<% 
										...
										%>
									<div name="divSetting"
										worktimeid="<%=calSetting.getWorkTimeId()%>" wtType="1">
										<div>上班</div>
										<div>
