...
<script type="text/javascript">
	...
	function updSn(tbodyObj){
		...
		$.post('${ctx}/oa/flow/userCondition/updateSn.do',{"conditionIds":conditionIds},function(data){
	...
	function conditionDialog(tableId,edit){
		...
		var url=__ctx + '/oa/flow/definition/conditionEdit.do?defId='+defJson.defId+'&conditionType='+conditionType+'&nodeId='+defJson.nodeId;
		url=url.getNewUrl();
	...
	function editCondition(){
		...
		var hw = $.getWindowRect();
		dialogH = hw.height*19/20;
		dialogW = hw.width*19/20+50;
		var url=__ctx + '/oa/flow/definition/conditionEdit.do?defId=' + defJson.defId + '&conditionType='+conditionType+'&conditionId='+ conditionId+"&nodeId="+defJson.nodeId;
		url=url.getNewUrl();
	...
	function delCondition(){
		...
		var url =__ctx + '/oa/flow/userCondition/delByAjax.do';
	...
	function messageSet(conditionId){
		...
		var url=__ctx + '/oa/system/message/copyEdit.do?actDefId=' + defJson.actDefId;
	...
	<div class="panel">
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					<div class="group"><a class="link ok" href="javascript:dialog.get('sucCall')();dialog.close();" >确定</a></div>
					
					<div class="group"><a class="link add" onclick="conditionDialog()">添加</a></div>
					
					<div class="group"><a class="link update" onclick="editCondition()">编辑</a></div>
					
					<div class="group"><a class="link del" onclick="delCondition()">删除</a></div>
					
					<div class="group"><a class="link save" onclick="saveGroupNo(this)" id="btnSaveGroupNo" >保存批次号</a></div>
				</div>
			</div>
		</div>
