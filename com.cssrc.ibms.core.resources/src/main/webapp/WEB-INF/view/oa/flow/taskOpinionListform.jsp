...
<body>
	<div class="panel" style="overflow-x:auto;">
		<div class="panel-body" style="overflow:initial;">
			<display:table name="taskOpinionList" id="taskOpinionItem" requestURI="list.do" sort="external" cellpadding="0" cellspacing="0" class="table-grid">
				...
				<display:column  title="执行开始时间">
					<fmt:formatDate value="${taskOpinionItem.startTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</display:column>
				<display:column  title="结束时间">
					<fmt:formatDate value="${taskOpinionItem.endTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</display:column>
				<display:column title="持续时间">
				  ${f:getTime(taskOpinionItem.durTime)}
				</display:column>
				<display:column property="exeFullname" title="执行人名"></display:column>
				<display:column property="opinion" title="审批意见" style="white-space: normal;" ></display:column>
