...
<html>
    <head>
        ...
        <script type="text/javascript">
     			var catKey="${CAT_FLOW}";
     			var glTypeTreeUrl = __ctx+'/oa/system/globalType/getByCatKeyForCompleted.do';
         		function getClickUrl(treeNode){
         			if($.isEmpty(treeNode.parentId) || treeNode.parentId == 0 ){
         				return "${ctx}/oa/flow/processRun/completedMattersList.do"
         			}
         			//else if(treeNode.isLeaf==1){
         				//三级节点才会执行到此处  办结只有两级
         				//return "${ctx}/oa/flow/userTask/completedMattersList.do?defId="+treeNode.typeId;
         			//}
         			else{
         				return "${ctx}/oa/flow/processRun/completedMattersList.do?nodePath="+treeNode.nodePath;
         			}
         		}
         </script> 
    ...
    <body>
      	<div id="defLayout" >
            ...
            <div position="center">
          		<iframe id="defFrame" height="100%" width="100%" frameborder="0" src="${ctx}/oa/flow/processRun/completedMattersList.do?porIndex=${porIndex}&tabIndex=${tabIndex}"></iframe>
            </div>
        </div>
    </body>
</html>
