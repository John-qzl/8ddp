...
<script type="text/javascript">
	...
	function conditionDialog(tableId,edit){
		var defVal = $("#bpmDefinition").val(),
			defJson = eval("("+defVal+")");
		var url=__ctx + '/oa/flow/definition/conditionEdit.do?defId='+defJson.defId+'&conditionType=1&nodeId=-1';
	...
	function editCondition(){
		...
		if(conditionObj.length!=1){
			$.ligerDialog.warn('要编辑的规则必须而且只能选择一项!','提示');
			return;
		}
		...
		var url=__ctx + '/oa/flow/definition/conditionEdit.do?defId=' + defJson.defId + '&conditionType=1&nodeId=-1&conditionId='+ conditionId;
		var winArgs="height=600,width=980,status=no,toolbar=no,menubar=no,location=no,resizable=1,scrollbars=1";
		if(parentActDefId){
			url += "&parentActDefId="+parentActDefId;
		}
			...
			if(r){
				$.post(url,{id:conditionIds},function(t){
					...
					if(resultData.result){
						$.ligerDialog.success(resultData.message,'提示',function(rtn){
							refresh();
						});
					}else{
	...
	<div class="panel">
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					<div class="group"><a class="link save" onclick="saveCcMessageType()">保存</a></div>
