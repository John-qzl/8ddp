...
<script type="text/javascript">
	...
	function exportXml(){	
		...
		var url=__ctx + "/oa/flow/definition/export.do?bpmDefIds="+bpmDefIds;
		ImportExportXml.showModalDialog({url:url,height:400});
	}
	
	//导入流程定义
	function importXml(){
		var url=__ctx + "/oa/flow/definition/import.do";
	...
	function cleanData(defId){
		$.ligerDialog.confirm("确定清除数据?",function(btn){
			if(!btn) return;
			$.post('cleanData.do',{defId:defId},function(data){
	...
	function setCategoryOk(item, dialog){
		...
		var url="${ctx}/oa/flow/definition/setCategory.do";
...
<body>      
	<div class="panel">
		<div class="panel-top">	
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<a class="link search" id="btnSearch">查询</a></div>
					...
					<c:if test="${currentUser.username!='right'}">
						<div class="group-new"><a class="link del"  href="####"  onclick="delDef()" >删除</a></div>
					
						<div class="group"><a class="link flowDesign" onclick="jQuery.openFullWindow('design.do')">在线流程设计</a></div>
						
						<div class="group"><a class="link export"  href="####" onclick="exportXml()">导出</a></div>
						
						<div class="group"><a class="link import"  href="####" onclick="importXml()">导入</a></div>
						...
						<!-- 
							 暂时去掉分管授权
							<div class="group"><a onclick="abatchGrant()" href="####" class="link grant"><spr:message code="menu.button.batchGrant"/></a></div> 
						-->
					</c:if>
				</div>	
			</div>
				...
				<form id="searchForm" class="plat-form" method="get" action="list.do">	    
					<ul class="row plat-row">
						...
						<span class="label">流程分类:</span><input type="text" name="Q_typeName_S" class="inputText" value="${param['Q_typeName_S']}"/>
						 -->
						<li><span class="label" title="流程定义Key:">流程定义Key:</span><input type="text" name="Q_defKey_SL"  class="inputText" value="${param['Q_defKey_SL']}"/></li>
		...
		<div class="panel-body">
			    ...
			    <display:table name="bpmDefinitionList" id="bpmDefinitionItem" requestURI="list.do" sort="external" cellpadding="1" cellspacing="1" export="false"  class="table-grid">
					...
					<display:column property="defKey" title="别名" sortable="true" sortName="defKey" ></display:column>
					
					<display:column property="descp" title="描述" sortable="true" sortName="descp" ></display:column>
					...
					<display:column title="管理" media="html" style="width:50px;text-align:center" class="rowOps">
						<c:if test="${bpmDefinitionItem.authorizeRight.managementDel=='Y' && currentUser.username!='right'}">
							<f:a alias="delProcess" href="del.do?defId=${bpmDefinitionItem.defId}" css="link del" >删除</f:a>
						</c:if>
						
						<c:if test="${bpmDefinitionItem.authorizeRight.managementEdit=='Y' && currentUser.username!='right'}">
							<f:a alias="flex" href="design.do?defId=${bpmDefinitionItem.defId}" target="_blank" css="link flowDesign" >设计</f:a>
						</c:if>
						
						<c:if test="${bpmDefinitionItem.authorizeRight.managementSet=='Y'&& bpmDefinitionItem.status!=0 }">
							<f:a alias="setBpm" href="detail.do?defId=${bpmDefinitionItem.defId}&time=${time}&returnUrl=${returnUrl}"  css="link setting" >设置</f:a>
						</c:if>
						
						<c:if test="${bpmDefinitionItem.authorizeRight.managementStart=='Y' && currentUser.username!='right'}">
						...
						<c:if test="${bpmDefinitionItem.status==0 && currentUser.username!='right'}">
							<f:a alias="publishProcess" href="deploy.do?defId=${bpmDefinitionItem.defId}" css="link deploy" >发布</f:a>
						</c:if>
						
                 <%--  <c:if test="${bpmDefinitionItem.authorizeRight.managementInternational=='Y'}">
							<c:if test="${bpmDefinitionItem.status!=0}">
								<f:a alias="internationalization" href="${ctx}/oa/flow/formLanguage/defList.do?actdefid=${bpmDefinitionItem.actDefId}" css="link comment" >国际化</f:a>
							</c:if>
						</c:if> --%>
						
						<c:if test="${bpmDefinitionItem.authorizeRight.managementClean=='Y' && currentUser.username!='right'}">
							<a  href="javascript:cleanData('${bpmDefinitionItem.defId}')" class="link clean" >清除数据</a>
					    </c:if>
					</display:column>
				</display:table>
				<ibms:paging tableId="bpmDefinitionItem"/>
				...
				<form id="frmDel">
					<table class="table-detail" cellpadding="0" cellspacing="0" border="0">
						<tr>
							...
							<td>
								<input class="catComBo" catKey="FLOW" valueField="typeId" catValue="" name="typeName" height="150" width="276"/>
							</td>
						</tr>
					</table>
				</form>
