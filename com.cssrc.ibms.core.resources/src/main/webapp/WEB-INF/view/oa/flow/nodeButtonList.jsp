...
function initAll(defId){
	var url =  __ctx+'/oa/flow/nodeButton/initAll.do?defId='+defId;
...
function init(defId,nodeId){
	...
	var url =  __ctx+'/oa/flow/nodeButton/init.do?defId='+defId+'&nodeId='+nodeId;
	$.ligerDialog.confirm('确认初始化按钮吗？','提示',function(rtn) {
		if(rtn){
			gotoUrl(url);
		}
...
function clearAll(defId){
	var url =  __ctx+'/oa/flow/nodeButton/delByDefId.do?defId='+defId;
...
function del(defId,nodeId){
	if($.isEmpty(nodeId)) nodeId ='';
	var url =  __ctx+'/oa/flow/nodeButton/deByDefNodeId.do?defId='+defId+'&nodeId='+nodeId;
	$.ligerDialog.confirm('确认清除该流程定义的所有按钮吗？','提示',function(rtn) {
		if(rtn){
			gotoUrl(url);
		}
...
function gotoUrl(url){
  ...
  function editOne(defId,returnUrl){
    href="getByNode.do?defId="+defId+"&returnUrl="+returnUrl;
    location.href=href;
}
  function editTwo(defId,nodeId,returnUrl){
   href="getByNode.do?defId="+defId+"&nodeId="+nodeId+"&returnUrl="+returnUrl;
   location.href=href;
}
	...
	<div class="panel-container">
		...
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					<div class="group"><a class="link init"  href="####" onclick="initAll('${bpmDefinition.defId}');">初始化全部按钮</a></div>
					
					<div class="group"><a class="link del" href="####" onclick="clearAll('${bpmDefinition.defId}')">清除按钮配置</a></div>
				</div>
			</div>
		</div>
				    ...
				    <input type="hidden" name="defId" value="${bpmDefinition.defId}"/>
					    ...
					    <thead>
							...
							<tr>
                                    ...
                                    <td >
										<a class="link edit" href="javascript:void(0);" onclick="editOne('${bpmDefinition.defId}','${returnUrl}');">编辑</a>
										
										<a class="link init" href="init.do?defId=${bpmDefinition.defId}">初始化</a>
										
										<a  class="link del"  href="deByDefNodeId.do?defId=${bpmDefinition.defId}">删除</a>
									</td>
							</tr>
							...
							<tr>
								...
								<td>
									<c:choose>
										...
										<c:otherwise>
											<a class="link edit" href="####" onclick="editTwo('${bpmDefinition.defId}','${item.nodeId}','${returnUrl}');" >编辑</a>
