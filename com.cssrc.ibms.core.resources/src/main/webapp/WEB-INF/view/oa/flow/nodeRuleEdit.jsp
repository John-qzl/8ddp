...
<script type="text/javascript">
	...
	function save(){
		InitMirror.editor.save();
		 ...
		 if(!rtn) return;
		 var url=__ctx+ "/oa/flow/nodeRule/save.do";
	...
	function showDetail(){
		...
		var url=__ctx + "/oa/flow/nodeRule/getById.do?ruleId=" + ruleId;
	...
	function loadRuleList(){
		var url=__ctx + "/oa/flow/nodeRule/getByDefIdNodeId.do?actDefId=" + actDefId +"&nodeId=" + nodeId;
		url=url.getNewUrl();
	...
	function delByRule(){
		...
		$.ligerDialog.confirm('确认删除吗？','提示信息',function(rtn) {
			if(!rtn) return;
			var url=__ctx + "/oa/flow/nodeRule/del.do?ruleId=" + ruleId;
			$.get(url,function(data) {
	...
	function reSort(){
		...
		var url=__ctx + "/oa/flow/nodeRule/sortRule.do";
	...
	function updateIsJumpForDef(ck){
		var url=__ctx+"/oa/flow/nodeRule/updateIsJumpForDef.do";
		var params={
			nodeId:nodeId,
			actDefId:actDefId,
		...
		<div class="panel-body">
			<div id="layoutFlowRule" style="width: 100%">
				...
				<div  id="framecenter" position="center">
					<form id="bpmNodeRuleForm" method="post" action="save.do">
