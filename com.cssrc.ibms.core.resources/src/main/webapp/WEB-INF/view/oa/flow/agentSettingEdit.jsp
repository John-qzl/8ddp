	...
	<script type="text/javascript">
		...
		*/
		function validateComplictByFlowKey(){
			...
			var url=__ctx+"/oa/flow/agentSetting/validateSettingComplictByFlow.do";
			$.ajax({
				url:url,
				data:params,
				async:false
		...
		*/
		function validateComplictAgainstGeneral(){
			...
			var url=__ctx+"/oa/flow/agentSetting/validateSettingComplictAgainstGeneral.do";
			$.ajax({
				url:url,
				data:params,
				async:false
		...
		*/
		function validateSettingComplictAgainstAll(){
			...
			var url=__ctx+"/oa/flow/agentSetting/validateSettingComplictAgainstAll.do";
			$.ajax({
				url:url,
				data:params,
				async:false
		...
		function showResponse(responseText) {
			...
			if (obj.isSuccess()) {
				$.ligerDialog.success(obj.getMessage(),'提示',function(){
					...
					if(agentType==1){   //管理入口
						window.location.href = "${ctx}/oa/flow/agentSetting/manageList.do";
					}else{
						window.location.href = "${ctx}/oa/flow/agentSetting/list.do";
					}					
				});
		...
		*/
		function setCondition(){
			...
			var url="${ctx}/oa/flow/agentCondition/edit.do?flowKey=" +flowkey ;
...
<div class="panel">
	...
	<form id="agentSettingForm" method="post" action="save.do">
			...
			<div id="bpmAgent"  <c:if test="${agentSetting.authtype==0 or agentSetting.authtype==2}"> style="display: none"</c:if> >
				<div class="panel-top">
					...
					<div class="panel-toolbar">
						<div class="toolBar">
							<div class="group">
								<a class="link add" href="####" onclick="addFlow();">添加流程定义</a>
							</div>
						</div>
					</div>
				</div>
					    			...
					    			<c:forEach items="${agentSetting.agentDefList}" var="bpmAgentItem">
							    		<tr id="def_${bpmAgentItem.flowkey}" type="subdata">
							    			<td>
							    				...
							    				<a href="${ctx}/oa/flow/definition/get.do?defKey=${bpmAgentItem.flowkey}" target="_blank">${bpmAgentItem.flowname}</a>
							    			</td>
							    			<td>
...
<textarea id="tableRowTemplate" style="display: none;">
	<tr id="def_#defKey"}" type="subdata">
			...
			<a href="${ctx}/oa/flow/definition/get.do?defKey=#defKey" target="_blank">#subject</a>
			</td>
			<td>
