...
<head>
	...
	function saveParam(){
		...
		$.post("saveParam.do",params,function(msg){
	...
	function openCcUserList(){
		var url=__ctx +'/oa/flow/definition/copyUserList.do?defId=${bpmDefinition.defId}&parentActDefId=${parentActDefId}';
	...
	function fileCallback(fileIds,fileNames,filePaths){
		...
		var url=__ctx+"/oa/system/sysFile/file_"+fileIds +".do";
	...
	function referDefinition(){
		var url=__ctx +'/oa/flow/definition/defReferSelector.do?defId=${bpmDefinition.defId}';
			         ...
			         { text: '确定', onclick: function (item, dialog) {
			        	 ...
			        	 var aryDefKey = [];
				     		...
				     		if(aryDefKey.length > 0){
				    			$.post("saveReferDef.do",params,function(map){
				    				var spanHtml='';
				    				for (var key in map) {  
	...
	function delRefer(refId){
		$.post("delReferDef.do",{refId:refId},function(msg){
			var obj=new com.ibms.form.ResultMessage(msg);
			if(obj.isSuccess()){
			...
			<table class="table-detail" cellpadding="0" cellspacing="0" border="0">
				...
				<tr>
					...
					<td>
							...
							<label><input type="checkbox" name="informStart" value="4" <c:if test="${fn:contains(bpmDefinition.informStart,4)}">checked="checked"</c:if>/>RTX消息</label>
							<!-- 
				...
				<tr>
					...
					<td>
						...
						<c:if test="${sysFile !=null }">
							<a href="${ctx}/oa/system/sysFile/file_${sysFile.fileId}.do" target="_blank" id="file">${sysFile.fileName }</a><a  class="link del" onclick="del(this)" style="cursor:pointer;">删除</span>
						</c:if>
				...
				<tr>
					...
					<td>
						...
						<span id="spanMessage" <c:if test="${bpmDefinition.status==2||bpmDefinition.status==3}">style="display:none;"</c:if>>
							...
							<label><input type="checkbox" name="informType" value="4" <c:if test="${fn:contains(bpmDefinition.informType,4)}">checked="checked"</c:if>/>RTX消息</label>
							<!-- 
				...
				<tr>
					...
					<td>
					 	<label><input id="allowMobile" type="checkbox" name="allowMobile" value="1" <c:if test="${bpmDefinition.allowMobile==1}">checked="checked"</c:if> /></label>
					</td>	
				</tr>
				 -->
			</table>
