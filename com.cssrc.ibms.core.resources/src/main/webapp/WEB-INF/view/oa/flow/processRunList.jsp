...
<html>
	...
	<head>
		...
		<script type="text/javascript" src="${ctx}/jslib/ibms/oa/bpm/FlowDetailWindow.js"></script>
		<script type="text/javascript" src="${ctx }/jslib/ibms/oa/system/SysDialog.js"></script>
		<script type="text/javascript">
			function addUser() {
				UserDialog({
					isSingle: true,
					callback: function(userIds, fullnames, emails, mobiles) {
	...
	<body>
		<div class="panel">
			<div class="panel-top">
				...
				<div class="panel-toolbar">
					<div class="toolBar">
						...
						<div class="group">
							<f:a alias="delRun" css="link del" action="del.do">删除</f:a>
						</div>
					</div>
				</div>
				<div class="panel-search">
					<form id="searchForm" method="post" action="list.do">
						<ul class="row plat-row">
							<li><span class="label" title="流程定义名称:">流程定义名称:</span><input type="text" name="Q_processName_SL" class="inputText" value="${param['Q_processName_SL']}" /></li>
							<li><span class="label" title="流程实例标题:">流程实例标题:</span><input type="text" name="Q_subject_SL" class="inputText" value="${param['Q_subject_SL']}" /></li>
							...
							<li>
								...
								<input type="text" id="cmpNames" name="creator" class="inputText" value="" />
								<input type="button" onclick="addUser();" value="..." />
							</li>
							<li>
								<input type="hidden" id="orgIds" name="Q_startOrgId_L" value="" />
								<span class="label">创建部门:</span>
								<input type="text" id="orgNames" name="startOrgName" class="inputText" value="" />
			...
			<div class="panel-body">
				...
				<display:table name="processRunList" id="processRunItem" requestURI="list.do" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
					...
					<display:column title="管理" media="html" style="width:140px">
						<c:if test="${processRunItem.authorizeRight.instanceDel=='Y'}">
							<f:a alias="delRun" href="del.do?runId=${processRunItem.runId}" css="link del">删除</f:a>
						</c:if>
						<c:if test="${processRunItem.authorizeRight.instanceLog=='Y'}">
							<a href="${ctx}/oa/flow/runLog/list.do?runId=${processRunItem.runId}" class="link log">操作日志</a>
						</c:if>
					</display:column>
				</display:table>
