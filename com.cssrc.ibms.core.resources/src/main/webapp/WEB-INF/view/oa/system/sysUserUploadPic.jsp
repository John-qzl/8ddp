...
<script type="text/javascript">
  ...
  function upload(imgObj){
	...
	function onload()
	{
		window.parent.photoid = "";
		window.parent.originalphotoid = "";
		<c:choose>
	  		<c:when test="${originalphotopath==''}">
	  			imgSrc="${CALL_FILE_SYS_TEMP_PATH}default_image_pool.jpg" ;	
	  		</c:when>
	  		<c:otherwise>
	  			imgSrc="${CALL_FILE_PATH}${originalphotopath}";
	  		</c:otherwise>
  		</c:choose>
	};
    ...
    function docut(){
    	...
    	var userId = ${userId};
  	   	var options = {
			 waitMsg : "正在提交数据...",
			 url:'${ctx}/oa/system/sysUser/doCut.do?x='+this.x+'&y='+this.y+'&w='+this.w+'&h='+this.h+'&imgSrc='+this.imgSrc+'&userId='+userId,
			 type:'POST',
			 scope : this,
			 success:function(d){
				window.parent.photoid = d.photoid;
				window.parent.originalphotoid = d.originalphotoid;
				var tempphotopath = d.tempphotopath;
				tempphotopath = "${CALL_FILE_TEMP_PATH}"+tempphotopath+"."+extName+"?radom="+Math.random();
				window.parent.closeTab("SysUserUploadPic",tempphotopath);
			 },
			 failure : function() {
...
<div class="container">
	  ...
	  <div class="file-box">
	  	...
	  	<c:choose>
	  		...
	  		<c:otherwise>
	  			<img  id="target" class="source-img" src="${CALL_FILE_PATH}${originalphotopath}" />
	  		</c:otherwise>
	  	</c:choose>
	  ...
	  <div id="preview-pane">
	  	...
	  	<c:choose>
	  		<c:when test="${originalphotopath==''}">
	  			<img class="jcrop-preview" src="${CALL_FILE_SYS_TEMP_PATH}default_image_pool.jpg" id="imgPreview"/>
	  		</c:when>
	  		<c:otherwise>
	  			<img class="jcrop-preview" src="${CALL_FILE_PATH}${originalphotopath}" id="imgPreview"/>
	  		</c:otherwise>
	  	</c:choose>
