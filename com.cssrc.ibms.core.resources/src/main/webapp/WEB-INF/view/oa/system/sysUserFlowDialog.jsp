...
<title>选择用户 </title>
	...
	<script type="text/javascript">
		...
		$(function(){
		    ...
		    var  src="${ctx}/oa/system/sysUser/selector.do?isSingle=${isSingle}&type="+type+"&typeVal="+typeVal;
		...
		*/
		...
		function load_Pos_Tree(){
			...
			var setting = {
		    		...
		    		callback: {
						...
						var p="?isSingle=${isSingle}&searchBy="+"${SEARCH_BY_POS}"+"&posId=" + treeNode.posId+"&type="+type+"&typeVal="+typeVal;
							$("#userListFrame").attr("src", url + p);
							setCenterTitle("按岗位查找:" + treeNode.posName);
						}
					}
				};
				var url="${ctx}/oa/system/position/getBySupOrgId.do?type="+type+"&typeVal="+typeVal;
		...
		function handPosClick(event, treeId, treeNode){
			...
			if(rtn){
				...
				var url="${ctx}/oa/system/sysUser/selector.do";
				var p="?isSingle=${isSingle}&searchBy="+"${SEARCH_BY_POS}"+"&includSub="+includSub+"&posId="+treeNode.posId+"&nodePath="+treeNode.nodePath;
				$("#userListFrame").attr("src",url+p);
				setCenterTitle("按岗位查找:"+treeNode.posName);
			}
		}
		...
		function handOrgClick(event, treeId, treeNode){
			...
			if(rtn){
				...
				var url="${ctx}/oa/system/sysUser/selector.do";
				var p="?isSingle=${isSingle}&searchBy="+"${SEARCH_BY_ORG}"+"&includSub="+includSub+"&orgId="+treeNode.orgId+"&path="+treeNode.path;
				$("#userListFrame").attr("src",url+p);
		...
		function handRoleClick(event, treeId, treeNode){
			...
			if(rtn){
				...
				var url="${ctx}/oa/system/sysUser/selector.do";
				var p="?isSingle=${isSingle}&searchBy="+"${SEARCH_BY_ROL}"+"&roleId=" + treeNode.roleId+"&type="+type+"&typeVal="+typeVal;
				$("#userListFrame").attr("src", url + p);
			...
			function load_Onl_Tree(){
				...
				var setting = {
			    		...
			    		callback: {
							onClick: function(event, treeId, treeNode){
								var url="${ctx}/oa/system/sysUser/selector.do";
								var p="?isSingle=${isSingle}&searchBy="+"${SEARCH_BY_ONL}"+"&path="+treeNode.path+"&type="+type+"&typeVal="+typeVal;
								$("#userListFrame").attr("src",url+p);
								setCenterTitle("按组织查找:"+treeNode.orgName);
							}
						}
...
<style type="text/css">
<f:link href="from-jsp.css"></f:link>.l-layout-right{left:528px;}
...
<body>
	<div id="defLayout">
		...
		<div position="right" title="<span><a onclick='javascript:dellAll();' class='link del'>清空 </a><input type='text' class='quick-find' placeholder='查找' style='width:60px;' /></span>"
			style="height:94%;overflow:auto;">
			<table width="145" id="sysUserList" class="table-grid table-list" 	id="0" cellpadding="1" cellspacing="1">
				<tr class="hidden"></tr>
			</table>
		</div>
		<div position="bottom"  class="bottom" style='margin-top:10px;padding-top:4px'>
