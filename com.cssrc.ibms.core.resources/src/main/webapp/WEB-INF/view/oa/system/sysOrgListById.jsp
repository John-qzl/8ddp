...
<head>
	...
	<script type="text/javascript">
		...
		function addWin(orgId,flag,path)
		{
			...
			{
				location.href="edit.do?orgId="+orgId+"&flag=LA&path="+path;
			}
		}
					...
					$.post(url,function(data)  { 
						...
						if(obj.isSuccess()){
							 $.ligerDialog.success(obj.getMessage(),"提示信息",function(rtn){
								 var orgId = "${orgId}"
								 location.href="listById.do?orgId=" + orgId;
		...
		{
			parent.parent.$("#A").attr("src","get.do?orgId="+orgId+"&flag=1");
		}
		//树列表的收展
       		...
       		<c:otherwise>
			      ...
			      <div class="panel-toolbar">	       
					<div class="toolBar">
						...
						-->
						<div class="group"><a class="link del" id="btnDel" href="javascript:;" >删除</a></div>
						
						<div class="group"><a class="link reset" onclick="$.clearQueryForm()">重置</a></div>
					</div>	
				  </div>
				  <div class="panel-search">
		           <form id="searchForm" method="post" action="listById.do">
	   				...
	   				<table id="dicTable" class="table-grid table-list" id="0" cellpadding="1" cellspacing="1">
			    		...
			    		<c:forEach items="${sysOrgList}" var="sysOrgItem" varStatus="status">
			    			...
			    			<c:choose>
			    			 ...
			    			 <a href="orgdel.do?orgId=${sysOrgItem.orgId}" class="link del">删除</a>
							 <a href="edit.do?orgId=${sysOrgItem.orgId}&demId=${sysOrgItem.demId}&flag=LE&path=${path}" class="link edit">编辑</a>
							 <a href="edit.do?orgId=${sysOrgItem.orgId}&demId=${sysOrgItem.demId}&action=add" class="link add">添加子组</a>
							 <a href="get.do?orgId=${sysOrgItem.orgId}&flag=1&path=${path}" class="link detail">明细</a>
							 </td>
			    			 </tr>
			    		</c:forEach>					    						    					    	
			    	</tbody>					    	
			   	</table>
			   <ibms:paging tableId="sysOrgItem"/>
