	...
	<script type="text/javascript">
		$(function() {
			$("a.save").click(function() {
				$("#orgAuthForm").attr("action","save.do");
				submitForm();
			});
			addOrgAuth("posPerms");
				...
				$.ligerDialog.success(obj.getMessage(),"提示信息", function(rtn) {
					if(rtn){
						if(window.opener){
							...
							this.close();
							window.location.href="${isGrade==true ? 'gradeList':'list'}.do?orgId=${orgId}";
						}
					}
				});
			} else {
		...
		function editRole(isGrade){
			...
			},isSingle:false,isGrade:isGrade,ids:ids,names:names}); 
		};
		function resetRole(){
			$("#roleIds").val("");
			$("#roleNames").val("");	
	...
	<div class="panel-top">
		<div class="tbar-title">
		    <c:choose>
			    <c:when test="${orgAuth.id !=null}">
			        <span class="tbar-label">编辑分级管理员</span>
			    </c:when>
			    <c:otherwise>
			        <span class="tbar-label">添加分级管理员</span>
			    </c:otherwise>			   
		    </c:choose>
		</div>
		<div class="panel-toolbar">
			<div class="toolBar">
				...
				<div class="group">
				<a class="link back" href="${isGrade==true ? 'gradeList':'list'}.do?orgId=${orgId}&isGrade=${param.isGrade}&topOrgId=${param.topOrgId}">
				返回</a></div>
			</div>
		</div>
	</div>
	<div class="panel-body">
		<form id="orgAuthForm" method="post" action="save.do">
			<table class="table-detail" cellpadding="0" cellspacing="0" border="0" type="main">
		        ...
		        <tr>
					...
					<td>
					<textarea id="roleNames" rows="1" cols="1" readonly="readonly" style="margin: 0px; width: 164px; height: 26px;resize:none;"><c:forEach items="${roleList}" var="role">${role.roleName},</c:forEach></textarea>
