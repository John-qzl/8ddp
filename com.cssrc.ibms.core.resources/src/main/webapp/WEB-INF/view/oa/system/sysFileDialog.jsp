...
<script type="text/javascript" src="${ctx}/jslib/ibms/oa/form/GlobalTypeFolder.js"></script>
<script type="text/javascript" src="${ctx}/jslib/ibms/oa/system/FileMenu.js"></script>
<script type="text/javascript">
	...
	var url="${ctx}/oa/system/sysFileFolder/tree.do";
	
	var fileMenu=new FileMenu();
	
	var curMenu;
	
	var conf={url:url,onClick:treeClick};
	
	var globalType=new GlobalTypeFolder(catKey,"glTypeTree",conf);
	...
	function treeClick(treeNode) {
		...
		var url = "${ctx}/oa/system/sysFile/selector.do?folderId=" + typeId  +"&isSingle=${isSingle}";
	...
	function getRow(fileObj){
		...
		//by weilei:文件上传返回的是fileName参数，而文件查询中返回的是filename参数
		var fileName = fileObj.fileName;
		if(fileObj.fileName == undefined){
			fileName = fileObj.filename;
		}
		var template=$("#trTemplate").val();
		template=template.replaceAll("#fileId",fileObj.fileId).replaceAll("#fileName",fileName);
		return template;
	}
	...
	function fileUpload(){
		...
		if(currentNode && currentNode.isRoot==undefined){
			url+="?typeId="+currentNode.id;
		}else{
			$.ligerDialog.warn("上传文件必须在个人目录");
			return;
		}
	 	var that = this;
	 	DialogUtil.open({
	...
	function selectFile(){
		...
		$.each(aryFileData, function(i, ch) {
			...
			aryPath.push("/oa/system/sysFile/file_id" +data.fileId +".do");
			aryExt.push(ext);
		});
...
<body>
	<div id="defLayout" style="height:100%;">
		...
		<div class="l-layout-header">附件列表</div>			
			<iframe id="fileFrame" name="fileFrame"  height="92%"  width="100%" frameborder="0" src="${ctx}/oa/system/sysFile/selector.do?isSingle=${isSingle}"></iframe>
		</div>
		<c:if test="${isSingle == 0}">
			<div position="right" title="<span><a onclick='javascript:dellAll();' class='link del'>清空</a><input type='text' class='quick-find' placeholder='查找'/></span>" style="height:92%;overflow:auto;">
