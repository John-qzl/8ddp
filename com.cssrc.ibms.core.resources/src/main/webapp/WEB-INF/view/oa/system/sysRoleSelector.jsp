	...
	<div class="panel">
		<div class="panel-top">
			<div class="panel-search">
				<form id="searchForm" method="post" action="selector.do?isSingle=${isSingle}" target="roleFrame">
					<div class="row">
						<span class="label">角色名称:</span><input type="text" name="Q_roleName_S" value="${param.Q_roleName_S }"  class="inputText" size="20" style="height: 25px;" /> &nbsp;
		...
		<div class="panel-body">
			    ...
			    <display:table name="sysRoleList" id="sysRoleItem" requestURI="selector.do" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
						<display:column title="${checkAll}" media="html" style="width:30px;">
						  	<c:if test="${isSingle==false}">
								<input type="checkbox" class="pk" name="roleId"  value="${sysRoleItem.roleId}#${sysRoleItem.roleName}"/>
							</c:if>
							<c:if test="${isSingle==true}">
								<input type="radio" class="pk" name="roleId" onclick="singleCheck()" value="${sysRoleItem.roleId}#${sysRoleItem.roleName}">
							</c:if>
						</display:column>
						<display:column property="roleName" title="角色名" sortable="true" sortName="roleName"></display:column> 
						<display:column property="roleDesc" title="备注" ></display:column>
					</display:table>
					<ibms:paging tableId="sysRoleItem" showExplain="false"/>	
			</div>
		</div>				
	</div> 
</body>
