...
<head>
	...
	<script type="text/javascript" src="${ctx}/servlet/ValidJs?form=serialNumber"></script>
...
<div class="panel">
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<div class="group"><a class="link back" href="showlist.do">返回</a></div>
					</c:when>
					<c:otherwise>
		...
		<div class="panel-body">
				<form id="serialNumberForm" method="post" action="save.do">
					
						<table class="table-detail" cellpadding="0" cellspacing="0" border="0">
							<tr>
								<th width="20%">名称: </th>
								<td><input type="text" id="name" name="name" value="${serialNumber.name}"  class="inputText longInputText"/></td>
							</tr>
							<tr>
								<th width="20%">别名: </th>
								<td><input type="text" id="alias" name="alias" value="${serialNumber.alias}"  class="inputText longInputText"/></td>
							</tr>
							<tr>
								<th width="20%">规则: </th>
								<td>
									...
									<c:choose>
										...
										<c:otherwise>
											<input type="text" id="rule" name="rule" size="80" value="${serialNumber.rule}"  class="inputText longInputText"/>
										</c:otherwise>
									</c:choose>
									<br>
									{yyyy}{MM}{dd}{NO}
							...
							<tr>
								...
								<td>
									<c:choose>
										...
										<c:otherwise>
											<input type="radio"  name="genType" <c:if test="${serialNumber.genType==1}">checked="checked"</c:if>  value="1" />每天生成
											<input type="radio"  name="genType" <c:if test="${serialNumber.genType==2}">checked="checked"</c:if>  value="2" />每月生成
											<input type="radio"  name="genType" <c:if test="${serialNumber.genType==3}">checked="checked"</c:if>  value="3" />每年生成
											<input type="radio"  name="genType" <c:if test="${serialNumber.genType==0}">checked="checked"</c:if>  value="0" />递增
										</c:otherwise>
									</c:choose>
									<br>
									流水号生成规则：
							...
							<tr>
								...
								<td>
									<c:choose>
										...
										<c:otherwise>
											<input type="text" id="noLength" name="noLength" value="${serialNumber.noLength}"  class="inputText"/>
										</c:otherwise>
									</c:choose>
									<br>
							...
							<tr>
								...
								<td>
									<c:choose>
										...
										<c:otherwise>
											<input type="text" id="initValue" name="initValue" value="${serialNumber.initValue}"  class="inputText"/>
										</c:otherwise>
									</c:choose>
							...
							<tr>
								...
								<td>
									<c:choose>
										...
										<c:otherwise>
											<input type="text" id="step" name="step" value="${serialNumber.step}"  class="inputText"/>
										</c:otherwise>
									</c:choose>
									<br>
