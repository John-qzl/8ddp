...
<script type="text/javascript">
	$(function() {
		$("a.save").click(function() {
			$("#messageLinkmanGroupForm").attr("action", "save.do");
			submitForm();
		});
	});
	//提交表单
					...
					.confirm(
							...
							function(rtn) {
								if (rtn) {
									...
									window.location.href = "${ctx}/oa/system/messageLinkmanGroup/list.do";
								}
							});
		} else {
...
<body>
	<div class="panel">
			...
			<div class="tbar-title">
				<c:choose>
					<c:when test="${messageLinkmanGroup.id !=null}">
						<span class="tbar-label">编辑常用联系人组</span>
					</c:when>
					<c:otherwise>
						<span class="tbar-label">添加常用联系人组</span>
					</c:otherwise>
				</c:choose>
			</div>
		...
		<div class="panel-body">
			<form id="messageLinkmanGroupForm" method="post" action="save.do">
