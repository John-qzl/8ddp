...
<script type="text/javascript">
	function test(id){
		...
		var param={id:id};
		$.post("test.do",param,function(data){
...
<body>
	<div class="panel">
		...
		<div class="panel-body">
		    ...
		    <display:table name="outMailUserSetingList" id="outMailUserSetingItem" requestURI="list.do" sort="external" cellpadding="1" cellspacing="1"  class="table-grid">
				...
				<display:column title="管理" media="html" style="width:50px;text-align:center" class="rowOps">
					<c:choose>
						...
						<c:otherwise>
							<a style="float: left" class="link setting" href="setDefault.do?id=${outMailUserSetingItem.id}"><span >设置默认</span></a>
						</c:otherwise>
					</c:choose>
					<a style="float: left" href="del.do?id=${outMailUserSetingItem.id}" class="link del">删除</a>
					<a style="float: left" href="edit.do?id=${outMailUserSetingItem.id}" class="link edit">编辑</a>
					<a style="float: left" href="get.do?id=${outMailUserSetingItem.id}" class="link detail">明细</a>
			...
			<ibms:paging tableId="outMailUserSetingItem"/>
