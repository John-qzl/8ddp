...
<head>
	...
	<script type="text/javascript">
		...
		$(function()
		{
			loadTree();
			layout();
			$('#listFrame').attr('src','${ctx}/oa/mail/outMail/list.do');
		});
		//布局
		function layout(){
			$("#layout").ligerLayout( {
		...
		function loadTree(){
			...
			$.post("${ctx}/oa/mail/outMail/getMailTreeData.do",
				 function(result){
		...
		function zTreeOnLeftClick(event, treeId, treeNode){
			if(treeNode.parentId!=0){
				returnUrl="${ctx}/oa/mail/outMail/list.do?id="+treeNode.parentId+"&types="+treeNode.types;
