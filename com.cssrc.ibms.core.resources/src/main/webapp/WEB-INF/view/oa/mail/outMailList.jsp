...
<script type="text/javascript">
	...
	function emailsync(type){
		...
		$.post("sync.do",{id:outMailSetId,types:type},function(data){
			$.ligerDialog.closeWaitting();
			var obj=new com.ibms.form.ResultMessage(data);
			if(obj.isSuccess()){
				$.ligerDialog.success(obj.getMessage(),'成功',function(){
					window.location.href="list.do?id="+outMailSetId+"&types=1";
				});
			}else{
	...
	function emailedit(type){
		...
		if(defaultMail==null||defaultMail.trim()==""){
			$.ligerDialog.confirm("请先设置默认邮箱<br/>或者在'邮箱配置管理里设置'!","提示信息",function(rtn){
				if(rtn){
					window.location.href="../../mail/outMailUserSeting/edit.do";
				}
			});
		}else{
...
<body>
	...
	<div class="hide-panel">
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<div class="group"><a class="link addMessage " href="javascript:;"  onclick="emailedit(${types})">写邮件</a></div>
					<c:if test="${types==1}">
						
						<div class="group"><a class="link reload" onclick="emailsync()"" id="emailsync">同步邮件</a></div>
					</c:if>
					
					<div class="group"><a class="link del"  id="deleteBtn" action="del.do">删除</a></div>
		...
		<div class="panel-body">
			    ...
			    <display:table name="outMailList" id="outMailItem" requestURI="list.do" sort="external" cellpadding="1" cellspacing="1" export="false"  class="table-grid">
					...
					<display:column title="主题" sortable="true" sortName="title" style="width:380px;" href="get.do" paramId="mailId" paramProperty="mailId">
						<a href="get.do?mailId=${outMailItem.mailId}&outMailSetId=${outMailSetId}">${outMailItem.title}</a>
					</display:column>
					...
					<display:column title="管理" media="html" style="width:180px;text-align:center">
						<c:choose>
						 	<c:when test="${outMailItem.types==4 }">
								<a href="del.do?mailId=${outMailItem.mailId}&outMailSetId=${outMailSetId}&types=${outMailItem.types}" id="delEnd" class="link del">彻底删除</a>
							</c:when>
							<c:when test="${outMailItem.types==3 }">
								<a href="edit.do?mailId=${outMailItem.mailId}&outMailSetId=${outMailSetId}&types=${outMailItem.types}" id="delEnd" class="link edit">编辑</a>
								<a href="del.do?mailId=${outMailItem.mailId}&outMailSetId=${outMailSetId}&types=${outMailItem.types}" class="link del">删除</a>
							</c:when>
							<c:otherwise>
								<a href="del.do?mailId=${outMailItem.mailId}&outMailSetId=${outMailSetId}&types=${outMailItem.types}" class="link del">删除</a>
							</c:otherwise>
						</c:choose>
						<a href="get.do?mailId=${outMailItem.mailId}&outMailSetId=${outMailSetId}" class="link preview">查看</a>
					</display:column>
				</display:table>
				<ibms:paging tableId="outMailItem"/>
