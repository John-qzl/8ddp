...
<head>
	...
	<script type="text/javascript">
			...
			if (obj.isSuccess()) {
				$.ligerDialog.confirm( obj.getMessage()+",是否继续操作","提示信息", function(rtn) {
					if(rtn){
						...
						window.location.href = "${ctx}/oa/form/formQuery/list.do";
					}
				});
			} else {
		...
		function showSettingDialog(dsName,objectname,istable,id){
			...
			var url=__ctx+"/oa/form/formQuery/setting.do?dsName=" +dsName +"&objectName=" + objectname + "&istable=" + istable +"&id=" + id;
			url=url.getNewUrl();
           	DialogUtil.open({
                height:800,
                width: 1300,
                	...
                	if(rtn==undefined) return;
                 	if(rtn.length>0){
                 		 $("#settingobj").val(objectname);
                   		 $("#conditionfield").val(rtn[0]);
                   		 $("#resultfield").val(rtn[1]); 
                   		 $("#sortfield").val(rtn[2]); 
                 	}
                }
            });
...
<div class="panel">
	...
	<div class="panel-body">
		<form id="bpmFormQueryForm" method="post" action="save.do">
