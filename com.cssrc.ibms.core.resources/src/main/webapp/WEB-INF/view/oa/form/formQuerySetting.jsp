...
<html>
	<head>
		...
		<f:link href="fromJsp/formQuerysetting.css"></f:link>
		<script type="text/javascript"
			src="${ctx}/jslib/ibms/oa/system/ScriptDialog.js">
</script>
		<script type="text/javascript"
			src="${ctx}/jslib/javacode/codemirror.js">
</script>
		<script type="text/javascript"
			src="${ctx}/jslib/javacode/InitMirror.js">
</script>
<script type="text/javascript">
...
function init() {
	...
	if (fields.displayField)
		initDisplayField(fields.displayField);
	if (fields.conditionField)
		initConditionField(fields.conditionField);
	if (fields.resultField)
		initResultField(fields.resultField);
	if (fields.sortField)
		initSortField(fields.sortField);
}
//未保存的自定义对话框，在已经设置列以后再次打开设置列的窗口时 初始化已设置的列
function initDisplayField(displayField) {
	var fieldObj = eval("(" + displayField + ")"), objContainer = $("#trDisplayField");
	objContainer.empty();
...
function initConditionField(conditionField) {
	...
	if (fieldObj.length > 0 && fieldObj[0].defaultType == 5) {
		for ( var i = 0; i < fieldObj.length; i++) {
			...
			if (c.field == '')
				continue;
			var btn = $('div.fieldBtnDiv a:last').clone(true)
					.css('display', '');
	...
	for ( var i = 0, c; c = fieldObj[i++];) {
		...
		var curTr = $("#condition" + c.field), curValTr = $("#conditionVal"
				+ c.field);
...
function initResultField(resultField) {
	var fieldObj = eval("(" + resultField + ")"), objContainer = $("#trRtnField");
	objContainer.empty();
	if (!fieldObj)
		return;
	for ( var i = 0, c; c = fieldObj[i++];) {
...
function initSortField(sortField) {
	if (!sortField.trim())
		return;
	var fieldObj = eval("(" + sortField + ")"), objContainer = $("#trSortField");
	objContainer.empty();
	if (!fieldObj)
		return;
	for ( var i = 0, c; c = fieldObj[i++];) {
...
function getConditionField(fieldName, comment, dbType) {
	...
	sb
			.append("<td><a alt='上移' href='####' class='link moveup' onclick='sortConditionTr(this,true)'></a>");
	sb
			.append("<a alt='下移' href='####' class='link movedown' onclick='sortConditionTr(this,false)'></a>");
	sb
			.append("<a href='####' class='link del'  onclick='delConditionTr(this)' >删除</a></td>");
	sb.append("</tr>");
	sb.append("<tr id='conditionVal" + fieldName + "'>");
	sb.append("<td>默认值</td>");
	sb
			.append("<td><select name='changeDefalut' onchange='changeDefault(this)'>");
	sb.append("<option value='1'>参数传入</option>");
	sb
			.append("<option value='2'>固定值</option><option value='3'>脚本</option></select></td>");
	sb
			.append("<td colspan='2'><a style='display:none;' href='####' name='btnScript' class='link var' title='常用脚本' onclick='selectScript(this)'>常用脚本</a>");
...
function getRtnField(fieldName, comment) {
	var sb = new StringBuffer();
	sb.append("<tr id='rtn" + fieldName + "' name='" + fieldName
			+ "' comment='" + comment + "' >");
	sb.append("<td >" + fieldName + "</td>");
	sb.append("<td >" + comment + "</td>");
	sb
			.append("<td><a alt='上移' href='####' class='link moveup' onclick='sortTr(this,true)'>&nbsp;&nbsp;</a>");
	sb
			.append("<a alt='下移' href='####' class='link movedown' onclick='sortTr(this,false)'>&nbsp;&nbsp;</a>");
	sb
			.append("<a href='####' class='link del' onclick='delRow(this)' >删除</a></td>");
	sb.append("</tr>");
	return sb.toString();
}
...
function getConditionSelect(dbType, name, comment) {
	var sb = new StringBuffer();
	sb.append("<select class='condition' name='" + name + "' dbType='" + dbType
			+ "' comment='" + comment + "'>");
	switch (dbType) {
	case "varchar":
			...
			$("input[name='fieldName']:checked").each(function() {
				...
				var fieldName = $(this).val(); //id
					...
					if (fnBtns == undefined || fnBtns.length < 1) {
						var btn = $('div.fieldBtnDiv').children('.button:last')
								.clone(true).css('display', '');
						var delBtn = $('div.fieldBtnDiv')
								.children('.divX:last').clone(true);
...
function buildTreeJson() {
	...
	displayStr = "{id:'" + treeId + "',pid:'" + parentId + "',displayName:'"
			+ displayName + "'}";
...
function selectForm() {
	...
	var rerurnlist = new Array(conditionStr, rtnStr, sortStr);
	...
	<body>
		<div id="defLayout">
			<div position="left" title="获取字段列表"
				style="overflow: auto; width: 520px; height: 444px;">
				<table cellpadding="1" class="table-grid table-list" cellspacing="1">
					...
					<c:forEach var="col" items="${tableModel.columnList }"
						...
						<tr class="${clsName}">
							...
							<td>
								<input type="text" name="comment" class="inputText"
									value="${col.comment }">
							</td>
						</tr>
					</c:forEach>
			...
			<div position="center">
				<ul class="btnContainer">
					<li class="btn" style="text-align: center;">
						<a href='####' class='button' onclick="setField()"><span>=></span>
						</a>
					</li>
				</ul>
			</div>
			<div id="fieldSetting" position="right" title="字段设置">
				<div class="panel-detail conditionContainer">
					...
					<div class="content">
						<div class="tab-box"> 
							<div class="tagMenu">
							<ul class="tab-menu"><li>条件选择</li><li>Java脚本</li><li>帮助</li></ul>
									...
									<table cellpadding="1" class="table-grid table-list" cellspacing="1">
										...
										<tbody id="trConditionField">
											<c:forEach items="${ bpmFormDialog.conditionList}" var="field">
												<tr class='trCondition' id='condition${field.fieldName}'
													name='${field.fieldName}' comment='${field.comment}'>
													<td>
														${field.fieldName}
													</td>
													<td>
														<select class='condition' name='${field.fieldName}'
															dbType='${field.fieldType}' comment='${field.comment}'>
															<c:choose>
																<c:when test="${field.fieldType=='varchar'}">
																	<option value='='
																		<c:if test="${field.condition=='='}">selected</c:if>>
																		等于
																	</option>
																	<option value='like'
																		<c:if test="${field.condition=='like'}">selected</c:if>>
																		LIKE
																	</option>
																	<option value='likeEnd'
																		<c:if test="${field.condition=='likeEnd'}">selected</c:if>>
																		LIKEEND
																	</option>
																</c:when>
						 ...
						        <div class="tab-layout helpField">
						 			<span>
										例：if(map.get("ACTDEFID")!=null)<br>&nbsp;&nbsp;return " and ACTDEFID like '
																		%"+map.get("ACTDEFID")+"%'" ;<br>
