...
<script type="text/javascript"
	src="${ctx}/jslib/ibms/oa/system/ImageQtip.js"></script>
...
<script type="text/javascript">
	  ...
	  $(document).ready(function() {
		    setTimeout(function(){
		       $('.panel-body').parent().height("500px")
...
<body>
	<div class="panel" height="211px">
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<div class="group" var="attachUpload">
						<a class="link upload" onclick="fileUpload()">上传</a>
					</div>
					<div class="group" var="attachDownload">
						<a class="link download" href="####"
							onclick="javascript:downloadFile()">下载</a>
					</div>
					<div class="group" var="attachDel">
						<a class="link del" name="delFile" onclick="delAttach()">删除</a>
					</div>
					<div var="attachDownloadall" class="group">
						<a class="link download" href="####"
							onclick="javascript:downloadFiles()">打包下载所有附件</a>
					</div>
				</div>
			</div>
			<div class="panel-search">
				<form id="searchForm" method="get" action="tableFile.do">
					<ul class="row">
						...
						<li><span class="label">上传者:</span><input type="text"
							...
							value="${param['Q_creator_SL']}" /></li>
						<li><span class="label">扩展名:</span><input type="text"
							name="Q_ext_SL" class="inputText" value="${param['Q_ext_SL']}" /></li>
						<input type="hidden" id="paramJson" name="paramJson"
							value="${fn:escapeXml(paramJson)}" />
						<div class="row_date">
							<li><span class="label" style="width: 25%">创建时间 从:</span><input
								type="text" name="Q_begincreatetime_DL" class="inputText date"
								value="${param['Q_begincreatetime_DL']}" /></li>
							<li><span class="label">至: </span><input type="text"
								name="Q_endcreatetime_DG" class="inputText date"
								value="${param['Q_endcreatetime_DG']}" /></li>
						</div>
					</ul>
				</form>
			</div>
		...
		<div class="panel-body">
			...
			<display:table name="sysFileList" id="sysFileItem"
				requestURI="tableFile.do" sort="external" cellpadding="1"
				cellspacing="1" export="false" class="table-grid">
				<display:column title="${checkAll}" media="html" style="width:30px;">
					<input type="checkbox" class="pk" name="fileId"
						value="${sysFileItem.fileId}">
				...
				<display:column title="文件名" sortable="true" sortName="filename">
					<a id="fileName"
						href="${ctx}/oa/system/sysFile/download.do?fileId=${sysFileItem.fileId}"
						css="link download">${sysFileItem.filename}.${sysFileItem.ext}</a>
				</display:column>
				<c:if test="${isShowSecurity }">
					<display:column title="密级">
						<c:if
							test="${(sysFileItem.security == null)||(sysFileItem.security eq '')}">
				...
				<display:column title="创建时间" sortable="true" sortName="createtime">
					<fmt:formatDate value="${sysFileItem.createtime}"
						pattern="yyyy-MM-dd HH:mm:ss" />
				</display:column>
				<display:column property="ext" title="扩展名" sortable="true"
					sortName="ext" />
				<display:column property="note" title="大小" sortable="true"
					sortName="note" maxLength="80"></display:column>
				<display:column title="版本" sortable="true" sortName="version">
					<a href="####"
						onclick="javascript:AttachMent.getAttachHistory(${sysFileItem.fileId})">${sysFileItem.version}</a>
				</display:column>
				...
				<display:column title="管理" media="html"
					...
					<a alias="delFile" var="manageDelete" class="link del ops_more"
						name="delFile" href="####"
						onclick="javascript:delAttach({fileId:${sysFileItem.fileId}})"
						css="link del">删除</a>
					<a href="####" var="managePreview"
						onclick="javascript:previewFile(${sysFileItem.fileId})"
						class="link preview">预览</a>
					<a href="####" var="manageUpdate"
						onclick="javascript:fileUpload(${sysFileItem.fileId},1)"
						class="link update">更新附件版本</a>
					<c:choose>
						<c:when test="${sysFile.delFlag eq 1}">
							<font color="red">已删除</font>
						</c:when>
					</c:choose>
					<a var="manageDownload"
						href="${ctx}/oa/system/sysFile/download.do?fileId=${sysFileItem.fileId}"
						class="link download">下载</a>
				</display:column>
			</display:table>
