	...
	<div class="panel" >
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					<div class="group">
						<a class="link ok" onclick="onOk(this)">确定</a>
					</div>
					
					<div class="group">
						<a class="link del" href="javascript:;" onclick="dialog.close()">关闭</a>
					</div>
					<div class="tipbox"><a href="javascript:;" class="tipinfo"><span>该脚本为SQL 脚本 ，为where后的脚本,<br/>例：1.IBMS_BUS_LINK.BUS_CREATOR_ID=:[CUR_USER]<br/>2.table.field  like '%value%'</span></a></div>
				</div>
			</div>
		</div>
		<div class="panel-body">
			<form id="scriptForm" method="post" action="save.do">			
					<table class="table-detail" cellpadding="1" cellspacing="1" border="0"  >
						...
						<tr>
							<td colspan="2">
								...
								<c:forEach items="${relTables}" var="relTable">
									<input type="checkbox" value="${relTable.tableName}" name="tables"  maintable="${bpmFormTable.tableName}" relation="${isRelTable}"  source="1"  >${relTable.tableDesc}
								</c:forEach>
								<c:forEach items="${bpmFormTable.otherTableList}" var="otherTable">
