...
<script type="text/javascript"
	src="${ctx}/jslib/ibms/oa/system/ScriptDialog.js"></script>
...
<script type="text/javascript"
	src="${ctx}/jslib/jquery/plugins/jquery.qtip.js"></script>
<script type="text/javascript">
		...
		$(function() {
				...
				var layoutHeight = $('#defLayout').height();
				$('.l-layout-left').css({
					height:layoutHeight-55,
					overflow:'auto'
				})
				$('.l-layout-center').css({
					height:layoutHeight-55,
					'line-height':(layoutHeight-55)+'px',
					'text-align':'center'
				})
				$('.l-layout-right').css({
					height:layoutHeight-55,
		...
		function getConditionField(fieldName,comment,dbType,isList){
			...
			sb.append("<td>默认值</td>");
			sb.append("<td><select class='plat-select-common' name='changeDefalut' onchange='changeDefault(this)'>");
			if(isList){
			...
			sb.append("<textarea class='plat-textarea' name='defaultValue' cols='40' rows='3' ");
			if(isList){
		...
		function getConditionSelect(dbType,name,comment){
			var sb=new StringBuffer();
			sb.append("<select class='condition plat-select-common' name='"+name+"' dbType='"+dbType+"' comment='"+comment+"'>");
			switch(dbType){
				case "varchar":
					...
					sb.append("<option value='not in'>not in</option>");
					sb.append("<option value='!='>不等于</option>");
					break;
				case "number":
					sb.append("<option value='='>等于</option>");
...
<body>
	<div id="defLayout">
		<div position="left" title="获取字段列表">
			<table cellpadding="1" class="table-grid table-list" cellspacing="1">
				...
				<c:forEach var="col" items="${tableModel.columnList }"
					...
					<c:set var="clsName">
						<c:choose>
							<c:when test="${status.index%2==0}">odd</c:when>
					...
					<tr class="${clsName}">
						...
						<td><input type="text" name="comment" class="inputText"
			...
			<c:choose>
				<c:when test="${style==0 }">
					<ul class="btnContainer">
						<li class="btn"><a href='####' class='button'
							onclick="setField()"><span>=></span></a></li>
					</ul>
				</c:when>
				<c:otherwise>
					<ul class="btnContainer">
						<li class="btnTree"><a href='####' class='button'
							onclick="setField()"><span>=></span></a></li>
					</ul>
				</c:otherwise>
			</c:choose>
		...
		<div id="fieldSetting" position="right" title="字段设置">
			...
			<c:choose>
				...
				<c:otherwise>
					<div class="panel-detail treeContainer">
						<div class="header" style="margin-top: 5px;">显示的字段</div>
						<div class="content">
							<table class="table-detail" cellpadding="0" cellspacing="0"
								...
								<tr>
									...
									<td><input type="text" id="treeId" name="treeId"
										...
										value="${ bpmFormDialog.treeField.id}" /> <!--<input type="checkbox" name="treeReturn" checked="checked" />返回 -->
									</td>
								</tr>
								<tr>
									<th width="20%">父ID:</th>
									<td><input type="text" id="parentId" name="parentId"
										class="inputText treeField"
										value="${ bpmFormDialog.treeField.pid}" /></td>
								</tr>
								<tr>
									<th width="20%">父ID初始值:</th>
									<td><textarea id="parentValue" name="parentValue"
											cols="40" rows="5"></textarea> <input type="checkbox"
										name="isScript" id="isScript" />脚本</td>
								</tr>
								<tr>
									<th width="20%">显示名称:</th>
									<td><input type="text" id="displayName" name="displayName"
										class="inputText treeField"
										value="${bpmFormDialog.treeField.displayName}" /> <!-- 											<input type="checkbox" name="treeReturn" checked="checked" />返回 -->
			...
			<div class="panel-detail conditionContainer">
				<div class="header" style="margin-top: 5px;">条件字段</div>
				<div class="content">
					<div class="tab-box">
						<div class="tagMenu">
							<ul class="tab-menu">
						...
						<div class="tab-content">
							<div class="tab-layout trConditionField">
									...
									<tbody id="trConditionField">
										<c:forEach items="${bpmFormDialog.conditionList}" var="field">
											<tr class='trCondition' id='condition${field.fieldName}' name='${field.fieldName}' comment='${field.comment}'>
												<td>${field.fieldName}</td>
												<td>
													<select class='condition plat-select-common' name='${field.fieldName}' dbType='${field.fieldType}' comment='${field.comment}'>
														<c:choose>
															<c:when test="${field.fieldType=='varchar'}">
																<option value='='
																	<c:if test="${field.condition=='='}">selected</c:if>>等于</option>
																<option value='like'
																	<c:if test="${field.condition=='like'}">selected</c:if>>LIKE</option>
																<option value='likeEnd'
																	<c:if test="${field.condition=='likeEnd'}">selected</c:if>>LIKEEND</option>
																<option value='!='
																	<c:if test="${field.condition=='!='}">selected</c:if>>不等于</option>
															</c:when>
															<c:when test="${field.fieldType=='number'}">
																<option value='='
																	<c:if test="${field.condition=='='}">selected</c:if>>等于</option>
																<option value='>='
																	<c:if test="${field.condition=='>='}">selected</c:if>>大于等于</option>
																<option value='>'
																	<c:if test="${field.condition=='>'}">selected</c:if>>大于</option>
																<option
																	value='<' <c:if test="${field.condition=='<'}">selected</c:if> >小于</option>
	          									...
	          									<c:otherwise>
	          										<option value='=' <c:if test="${field.condition=='='}">selected</c:if>>等于</option>
																...
																<option value='<=' <c:if test="${field.condition=='<='}">selected</c:if> >小于等于</option>
																<option value='!=' <c:if test="${field.condition=='!='}">selected</c:if> >不等于</option>
	          									</c:otherwise>
	          								</c:choose>
	          							</select>
											</td>
	          				...
	          				<tr id='conditionVal${field.fieldName}'>
												<td>默认值</td>
												<td>
													<select name='changeDefalut' class="plat-select-common" onchange='changeDefault(this)'>
												...
												<td colspan="2">
													...
													<textarea class="plat-textarea" name='defaultValue' cols='40' rows='3'>${field.defaultValue}</textarea>
												</td>
										</tr>
										</c:forEach>
									</tbody>
							...
							<div class="tab-layout javaField">
								<textarea id="script" class="plat-textarea" name="defaultValue" codemirror="true"	mirrorheight="200px" name="script" rows="26" cols="70"></textarea>
							</div>
						...
						<tbody id="trRtnField">
							<c:forEach items="${ bpmFormDialog.returnList}" var="field">
								<tr id='rtn${field.fieldName}' name='${field.fieldName}' comment='${field.comment}'>
									<td>${field.fieldName}</td>
									<td>${field.comment}</td>
									<td><a alt='上移' href='####' class='link moveup'
										onclick='sortTr(this,true)'>&nbsp;</a> <a alt='下移' href='####'
										class='link movedown' onclick='sortTr(this,true)'>&nbsp;</a> <a
										href='####' class='link del' onclick='delRow(this)'>删除</a></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
					...
					<div style="overflow: auto;">
						<table cellpadding="1" class="table-grid table-list" cellspacing="1">
							...
							<tbody id="trSortField">
								<c:forEach items="${ bpmFormDialog.sortList}" var="field">
									<tr id="sort${field.fieldName}" name="${field.fieldName}">
										...
										<td id="sort"><input type="checkbox" name="sort" id="ASC"
											<c:if test="${field.comment == 'ASC'}">checked="checked"</c:if> />
											&nbsp;升序（不勾选则为降序）</td>
										<td><a alt='上移' href='####' class='link moveup'
											onclick='sortTr(this,true)'>&nbsp;</a> <a alt='下移' href='####'
											class='link movedown' onclick='sortTr(this,false)'>&nbsp;</a>
											<a href='####' class='link del' onclick='delRow(this)'>删除</a></td>
									</tr>
								</c:forEach>
							</tbody>
							<tr id="sortTrTemplate" style="display: none;">
								<td id="fieldName"></td>
								<td id="sort"><input type="checkbox" name="sort" id="ASC"
									checked="checked" />&nbsp;升序（不勾选则为降序）</td>
								<td><a alt='上移' href='####' class='link moveup'
									onclick='sortTr(this,true)'>&nbsp;</a> <a alt='下移' href='####'
									class='link movedown' onclick='sortTr(this,false)'>&nbsp;</a> <a
									href='####' class='link del' onclick='delRow(this)'>删除</a></td>
							</tr>
						</table>
					</div>
		...
		<div position="bottom" class="bottom" style="padding-top: 8px;">
			<a href='####' class='button' onclick="selectForm()"><span
				class="icon ok"></span><span>确定</span></a> <a href='####' class='button'
				style='margin-left: 10px;' onclick="dialog.close();"><span
				class="icon cancel"></span><span>取消</span></a>
		</div>

		<input type="hidden" name="resultfield" id="resultfield"
			value="${bpmFormDialog.resultfield}" /> <input type="hidden"
			name="id" id="id" value="${bpmFormDialog.id}" />
	</div>
	...
	<div id="templateDiv"
		...
		<input type="checkbox" /> <input type="text" value="" name="comment" class="plat-input-sql"/>
		<span><a href="####" class="link del" onclick="delDiv(this)"></a></span>
	</div>

	<div id="templateSelector" style="display: none;">
		<select class="plat-select-common" name="ct" id="ct">
		...
		<div style="display: none; margin-top: 5px;" id="settingDiv">
			<select id="dialog-type" name="dialog-type"
				onchange="dialogChange(this)" style="width: 120px;">
				<option value="">请选择对话框……</option>
			</select> <select id="dialog-param" name="dialog-param" style="width: 120px;">
