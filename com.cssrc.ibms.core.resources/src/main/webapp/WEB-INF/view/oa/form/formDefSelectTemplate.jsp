...
function openEdit(){
	...
	var url='edit.do?' + params;
	jQuery.openFullWindow(url);
	//var tab = {id:'formEdit',text:'表单设计',iconCls:'',attributes:{url:'${ctx}edit.do?' + params}};
	
	parent.closeWin();//调用 "/WebRoot/WEB-INF/view/oa/form/formDefList.jsp" 中的"function closeWin()"方法关闭当前的ligerDialog窗口。
};
</script>
</head>
<body>
		...
		<div class="panel-body">
			<form id="frmDefInfo">
					...
					<table class="table-detail" cellpadding="0" cellspacing="0" border="0">
						...
						<tr>
							<th width="30%">${relTable.tableDesc }模板:</th>
							<td>
								<select templateId="templateId" tableId="${relTable.tableId }">
									<c:forEach items="${relTableTemplates}" var="bpmFormTemplate">
										<option value="${bpmFormTemplate.alias}">${bpmFormTemplate.templateName}</option>
									</c:forEach>
								</select>
							</td>
