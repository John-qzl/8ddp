...
<script type="text/javascript"src="${ctx}/jslib/ibms/oa/form/BpmDataTemplateFilter.js"></script>
	...
	<div class="panel-body">
		<table style="margin: auto;width:100%;margin-top: 1px;" class="table-detail" cellpadding="0" cellspacing="0" border="0">
			<tr>
				...
				<td><input type="text" class="inputText" name="name" id="name" size="50" onblur="autoGetKey(this)"></td>
				<td  width="10%" nowrap="nowrap">过滤key：</td>
				<td><input type="text" class="inputText" name="key" id="key" size="50" ><font color=red>此处请不用中文，否则在ie下有乱码，过滤无效。</font></td>
			</tr>
			<tr>
				<td>
					脚本类型：
							...
							<td>
								<select id="tableSelect" class="left margin-set" name="tableSelect" onchange="tableChange.apply(this)">
									<option value="">--请选择--</option>
										...
										<c:forEach items="${relTables}" var="relTable">	
											<option value="${relTable.tableName}" source="${source}" relation="${isRelTable}">${relTable.tableDesc}</option>
										</c:forEach>
										<c:forEach items="${bpmFormTable.otherTableList}" var="otherTable">	
