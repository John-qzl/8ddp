...
<head>
    ...
    <link rel="stylesheet" href="${ctx}/stylers/default/css/iconfont.css" type="text/css">
    ...
    <script type="text/javascript">
        ...
        function showMainMenu() {
            ...
            $('.template1 .tab-commonTemplate').click(function () {
                var formCommonTemplateUrl = "${ctx}/template/manage/common.do?moduleId=0&moduleName=全部型号&ofPart=CPYS";//型号  ofpart用于标识当前领域 可能存在的值有CPYS WQSJ  BCSY
                $("#tab-commonTemplate-frame").attr("src", encodeURI(encodeURI(formCommonTemplateUrl)));
            })
            // 数据分析tab
            $('.template1 .tab-dea').click(function () {
                //设定表单模块的IFrame的请求地址
                var formDEAUrl = "${ctx}/dataPackage/dea/getQueryInfo.do?xhId=0&type=" + flagType;
                $(".template1 #tab-dea-frame").attr("src", encodeURI(encodeURI(formDEAUrl)));
            })
            // 工作统计tab
            $('.template1 .tab-analysis').click(function () {
                //设定表单模块的IFrame的请求地址
                var formComAnalysisUrl = "${ctx}/dataPackage/dp/dpCom/analysis.do?xhId=0&type=" + flagType+"&ofPart=YSCH";
            ...
            switch (treeNode.level) {
                case 0:
                    ...
                    $('div[position="center"]').append($('.template .template1').clone(true));
                 	...
                 	// 通用模板tab
                    $('.template1 .tab-commonTemplate').click(function () {
                        var formCommonTemplateUrl = "${ctx}/template/manage/common.do?moduleId=" + treeNode.typeId + "&moduleName=" + treeNode.typeName+"&ofPart=CPYS";//型号  ofpart用于标识当前领域 可能存在的值有CPYS WQSJ  BCSY
                        $("#tab-commonTemplate-frame").attr("src", encodeURI(encodeURI(formCommonTemplateUrl)));
                    })
                    // 数据分析tab
                    $('.template1 .tab-dea').click(function () {
                    ...
                    $('.template1 .tab-analysis').click(function () {
                        //设定表单模块的IFrame的请求地址
                        var formComAnalysisUrl = "${ctx}/dataPackage/dp/dpCom/analysis.do?xhId=" + treeNode.typeId + "&type=" + flagType+"&ofPart=YSCH";
                        $("#tab-analysis-frame").attr("src", encodeURI(encodeURI(formComAnalysisUrl)));
                    })
                    break;
                case 1:
...
<div class="template" style="display:none;">
    <div class="layui-tab layui-tab-brief template1">
        <ul class="layui-tab-title">
            <li class="layui-this"><img class="Tabimg" src="${ctx}/styles/images/modelManagement/list.png">型号列表</li>
            <li class="tab-commonTemplate"><img class="Tabimg" src="${ctx}/styles/images/modelManagement/template.png">通用模板</li>
            <li class="tab-dea"><img class="Tabimg" src="${ctx}/styles/images/modelManagement/analysis.png">数据分析</li>
            <li class="tab-analysis"><img class="Tabimg" src="${ctx}/styles/images/modelManagement/statistics.png">工作统计</li>
        </ul>
        <div class="layui-tab-content">
            <div class="layui-tab-item layui-show">
                <iframe id="listFrame" src="" frameborder="no" width="100%" height="100%"></iframe>
