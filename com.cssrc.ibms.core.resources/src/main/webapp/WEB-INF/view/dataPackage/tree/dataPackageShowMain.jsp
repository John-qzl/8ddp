...
<script type="text/javascript">
		...
		var projectName = "<%=request.getParameter("projectName")%>";
		 --%>
		var projectId = "<%=request.getParameter("productId")%>";
		var projectName = "<%=request.getParameter("productName")%>";
		
		function addIframe(unionId) {
			// 产品-数据包节点 
			var iframe = '<iframe class="w100" name="baseInfo" id="baseInfo" frameborder="0" src=""></iframe>';
			$(".layui-tab-item.layui-show.baseInfo").append(iframe);
			var url="${ctx}"+"/oa/form/dataTemplate/detailData.do?__displayId__=10000021200549&__pk__="+id
		...
		$(function (){
			...
			addIframe(unionId);
		  	...
		  	$('.layui-tab-title').on('click','li',function(){
				...
				case "baseInfo":
					var currentUrl="${ctx}"+"/oa/form/dataTemplate/detailData.do?__displayId__=10000021200549&__pk__="+id
					+"&__dbomFKName__="+parentName;
					$("#baseInfo").attr("src",currentUrl);
					break;
				case "dataPackage":
					var currentUrl ="${ctx}"+ "/oa/form/dataTemplate/preview.do?__displayId__=10000000680207"
							+"&__dbomFKName__="+currentName
							+"&__dbomFKValue__="+id
							+"&projectId="+projectId
							+"&projectName="+projectName
							+"&__dbomSql__=F_SSSJB="+id;
					$("#dataPackage").attr("src",currentUrl);
					break;
				case "workTeam":
		...
		<div class="layui-tab layui-tab-brief template1">
			...
			<div class="layui-tab-content">
				<div class="layui-tab-item layui-show baseInfo" style="overflow: auto;">
					...
					<!-- <iframe class="w100" name="baseInfo" id="baseInfo" frameborder="0" src="" width="30%"></iframe> -->
				</div>
