...
<head>
    ...
    <script type="text/javascript" src="${ctx}/js/dataPackage/tree/RangeTestDirectotyTree.js"></script>
    ...
    <script type="text/javascript">
        ...
        var mappingUrl = "/mission/dimension/tree/";
        ...
        function showMainMenu() {
            ...
            //加载列表
            var url = "${ctx}" + "/oa/form/dataTemplate/preview.do?__displayId__=10000029830022";
            url = encodeURI(url);
            ...
            $('.template1 .tab-commonTemplate').click(function () {
                var formCommonTemplateUrl = "${ctx}/template/manage/common.do?moduleId=0&moduleName=全部型号&ofPart=WQSJ";//型号  ofpart用于标识当前领域 可能存在的值有CPYS WQSJ  BCSY
                $("#tab-commonTemplate-frame").attr("src", encodeURI(encodeURI(formCommonTemplateUrl)));
            })
            // 工作统计tab
            $('.template1 .tab-analysis').click(function () {
                //设定表单模块的IFrame的请求地址
                var formComAnalysisUrl = "${ctx}/dataPackage/dp/dpCom/analysis.do?xhId=0&type=" + flagType+"&ofPart=WQSJ";
        ...
        function moduleTreeOnClick(event, treeId, treeNode) {
            ...
            switch (treeNode.level) {
                case 0:
                    ...
                    // 通用模板tab
                    $('.template1 .tab-commonTemplate').click(function () {
                        var formCommonTemplateUrl = "${ctx}/template/manage/common.do?moduleId=" + treeNode.typeId + "&moduleName=" + treeNode.typeName+"&ofPart=WQSJ";//型号  ofpart用于标识当前领域 可能存在的值有CPYS WQSJ  BCSY
                        $("#tab-commonTemplate-frame").attr("src", encodeURI(encodeURI(formCommonTemplateUrl)));
                    })
                     // 数据分析tab
                /*     $('.template1 .tab-dea').click(function () {
                    ...
                    $('.template1 .tab-analysis').click(function () {
                        //设定表单模块的IFrame的请求地址
                        var formComAnalysisUrl = "${ctx}/dataPackage/dp/dpCom/analysis.do?xhId=" + treeNode.typeId + "&type=" + flagType+"&ofPart=WQSJ";
                        $("#tab-analysis-frame").attr("src", encodeURI(encodeURI(formComAnalysisUrl)));
                    })
                    break;
                case 1:
                    ...
                    $('.template2 .tab-dp').click(function () {
                        //设定表单模块的IFrame的请求地址
                        // var formDPUrl = "${ctx}/dataPackage/tree/dataPackage/show.do?xhId=" + treeNode.typeId + "&xhName=" + treeNode.typeName + "&xhSSMK="+treeNode.typeOfPart;
                        //由于任务树页面必然是靶场试验,所以从型号树到任务树时不需要携带ssmk

                        var formDPUrl = "${ctx}/dataPackage/tree/weaponCheckDataPackage/show.do?xhId=" + treeNode.typeId + "&xhName=" + treeNode.typeName ;//型号
                        $("#tab-dp-frame").attr("src", encodeURI(encodeURI(formDPUrl)));
                    })
                    //查看型号团队
                    $('.template2 .tab-dp-team').click(function () {

                        var formDPUrl = "${ctx}/oa/form/dataTemplate/preview.do?__displayId__=10000030730178&__xhId__="+treeNode.typeId+"&__dbomSql__=F_SSXHID="+treeNode.typeId;//型号团队预览
                        $("#tab-dp-team-frame").attr("src", encodeURI(encodeURI(formDPUrl)));
                    })
                    /*
                    // 通用模板tab
        ...
        function handler(item) {
            ...
            switch (txt) {
                case "新增":
                    if(item.roleNames.curUser.roles.indexOf("sjgly")>=0||item.roleNames.addRole=="1"){
                        sTree.editNode(true);
                    }
                    else{
                    ...
                    break;
                case "编辑":
                    if(item.roleNames.curUser.roles.indexOf("sjgly")>=0||item.roleNames.editRole=="1"){
...
<div class="template" style="display:none;">
    ...
    <div class="layui-tab real-layui-tab layui-tab-brief template2" >
        <ul class="layui-tab-title real-layui-tab-title" >
            <li class="tab-dp">数据查看</li>
            <li class="tab-dp-team">型号团队</li>
        </ul>
        <!-- 因为内嵌网页的高度一直有问题,所以给下面div加了height:100%属性 -->
        <div class="layui-tab-content" style="height:100%">
            ...
            <%--型号团队iframe--%>
            <div class="layui-tab-item">
                <iframe id="tab-dp-team-frame" src="" frameborder="no" width="100%" height="100%"></iframe>
            </div>
        </div>
