    ...
    <script type="text/javascript">
        ...
        function showMainMenu() {
            ...
            $('.template1 .tab-commonTemplate').click(function () {
                var formCommonTemplateUrl = "${ctx}/template/manage/common.do?moduleId=0&moduleName=全部型号&ofPart=BCSY";//型号  ofpart用于标识当前领域 可能存在的值有CPYS WQSJ  BCSY
                $("#tab-commonTemplate-frame").attr("src", encodeURI(encodeURI(formCommonTemplateUrl)));
            })
            // 工作统计tab
            $('.template1 .tab-analysis').click(function () {
                //设定表单模块的IFrame的请求地址
                var formComAnalysisUrl = "${ctx}/dataPackage/dp/dpCom/analysis.do?xhId=0&type=" + flagType+"&ofPart=BCSY";
            ...
            switch (treeNode.level) {
                case 0:
                    ...
                    $('div[position="center"]').append($('.template .template1').clone(true));
                 	...
                 	// 通用模板tab
                    $('.template1 .tab-commonTemplate').click(function () {
                        var formCommonTemplateUrl = "${ctx}/template/manage/common.do?moduleId=" + treeNode.typeId + "&moduleName=" + treeNode.typeName+"&ofPart=BCSY";//型号  ofpart用于标识当前领域 可能存在的值有CPYS WQSJ  BCSY
                        $("#tab-commonTemplate-frame").attr("src", encodeURI(encodeURI(formCommonTemplateUrl)));
                    })
                  /*   // 数据分析tab
                    ...
                    $('.template1 .tab-analysis').click(function () {
                        //设定表单模块的IFrame的请求地址
                        var formComAnalysisUrl = "${ctx}/dataPackage/dp/dpCom/analysis.do?xhId=" + treeNode.typeId + "&type=" + flagType+"&ofPart=BCSY";
                        $("#tab-analysis-frame").attr("src", encodeURI(encodeURI(formComAnalysisUrl)));
                    })
                    break;
                case 1:
                    ...
                    $('.template2 .tab-dp').click(function () {
                       ...
                       //由于任务树页面必然是靶场试验,所以从型号树到任务树时不需要携带ssmk
                        ...
                        var formDPUrl = "${ctx}/dataPackage/tree/rangeDataPackage/show.do?xhId=" + treeNode.typeId + "&xhName=" + treeNode.typeName ;//型号
                        $("#tab-dp-frame").attr("src", encodeURI(encodeURI(formDPUrl)));
                    })
                    //查看型号团队
