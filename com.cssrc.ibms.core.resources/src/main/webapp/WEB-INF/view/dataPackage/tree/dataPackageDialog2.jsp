	...
	<script type="text/javascript">
		...
		$(function() {
	  	...
	  	if(isSingle){
			...
			//chkboxType 中Y 属性定义 checkbox 被勾选后的情况；属性定义 checkbox 取消勾选后的情况；"p" 表示操作会影响父级节点；"s" 表示操作会影响子级节点。请注意大小写，不要改变
...
//						onCheck : onCheck
			...
			$.post(url,function(result){
				var rootNode = {ID:projectId,F_SSCPLB:"-1",F_PCH:"所有节点"};
...
//					return (node.level > 0);
		...
		function beforeCheck(treeId,treeNode){
			if(treeNode.F_PCH =="所有节点"){
				return true ;
			}
		/* 	else if((treeNode.isParent||treeNode.F_JDLX =="普通分类节点")){
				debugger;
				$.ligerDialog.warn("禁止选择普通分类节点");
				return false ;
			} */
			return !treeNode.isParent ; //当是父节点时 返回false不让选取
		}
