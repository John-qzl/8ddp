...
<script type="text/javascript">
	...
	$(function() {
		//window.top.document.getElementById('10000000480046').contentWindow.reFresh();
		...
		//失去焦点校验
		//$(".validError").blur(function(){
			//var lablename = this.getAttribute("lablename");
			//有关lablename为null，请正确设计表单，表单字段都会自带lablename属性
			//lablename='<span style="color:rgb(136, 10, 10);font-weight: bolder;">'+lablename+'</span>';
			//$.ligerDialog.warn(lablename+"<span style='color:red;'>未正确输入！</span>", "提示信息");
		//})
					...
					$.ligerDialog.confirm(message + ",是否继续操作", "提示信息",
							function(rtn) {
								if (!rtn) {
									...
									TableFile.refresh();
									//$("#dataFormSave").attr("disabled",false);
								}
							});
