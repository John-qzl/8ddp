		...
		if (rtn) {		
			for(var i=deletes.length-1;i>=0;i--){
				...
				if(conditionId!='' && conditionId!=undefined){
					$.post(__ctx+'/oa/flow/userCondition/del.do',{'id':conditionId});
				}
...
function showOtherNodeDlg(conf){
	...
	url=__ctx + "/oa/flow/definition/taskNodes.do?actDefId=${bpmDefinition.actDefId}&nodeId="+conf.nodeId;
	url=url.getNewUrl();
...
function conditionDialog(tableId,edit)
{
	...
	var url;
	if(conditionId){
		url=__ctx + '/oa/flow/definition/conditionEdit.do?defId='+defId+'&nodeId='+nodeId+'&conditionId='+conditionId;
	}	
	else if(edit && !conditionId){
		$.ligerDialog.warn("请先选择要修改的规则条件！",'提示');
		return;
	}else{
		url=__ctx + '/oa/flow/definition/conditionEdit.do?defId='+defId+'&nodeId='+nodeId;
	}
	if(parentActDefId){
