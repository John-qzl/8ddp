...
<div class="table-top">
			<div class="table-top-right">	
								<div class="toolBar">
									...
									<div class="group"><a class="link preview " id="btnPreview"  
									onclick="previewUserSetting(${defId})" >预览</a></div>
									
									<div class="group">
										<a href="javascript:;" class="link tipbox" title="注意的是人员设置里第一行的运算类型是不起任何作用！当人员设置有多行并进行运算时，
										人员设置列表的将按照顺序执行下去，如第一行会找第二行的运算类型与第二行做运算，
										做完运算后作为一个集合，再找第三行的运算类型与第三行做运算，依次类推，直到执行最后一行。">
										帮助
										</a>
									</div>
									<br> 
	...
	<c:choose>
		<c:when test="${fn:length(userList)==0}">
			<tr>
				...
				<td>
					...
					<textarea name="cmpNames" rows="2" class="textarea" readonly="readonly"></textarea>
		...
		<c:otherwise>
			<c:forEach items="${userList}" var="userNode" varStatus="cnt">
				<tr id="${userNode.nodeUserId}_${cnt.count}">
					<td  nowrap="nowrap" height="28">
						<input type='checkbox' name='nodeUserCk'/>
