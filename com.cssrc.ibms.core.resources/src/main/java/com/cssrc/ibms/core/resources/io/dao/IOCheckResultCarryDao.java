...
@Repository
public class IOCheckResultCarryDao {
	@Resource
	...
	public CheckResult getById(String id) {
		String sql = " select * from W_CK_RESULT_CARRY where id='"+id+"'";
	...
	public void update(CheckResult checkResult) {
		...
		sql.append(" update W_CK_RESULT_CARRY set ");
	...
	public Long insert(CheckResult checkResult) {
		StringBuffer sql = new StringBuffer();
		sql.append(" insert into W_CK_RESULT_CARRY (ID,F_RESULT,F_VALUE,F_IFNUMM,F_SKETCHMAP,F_ITEMDEF_ID,F_TB_INSTAN)");
	...
	public Long insert(CheckResult checkResult,String floderPath) {
		if(checkResult.getImages()!=null) {
			for(DpFile file : checkResult.getImages()) {
				this.dpFileDao.insertOpratorPhoto(file, floderPath, checkResult.getId());
			}
		}
		DpFile sfile = checkResult.getSketchImage();
		if(sfile!=null) {
			this.dpFileDao.insertSignPhoto(sfile, floderPath, checkResult.getId());
		}
	...
	public List<Map<String,Object>> query(Map<String,Object> keyValueMap){
		StringBuffer sql = new StringBuffer();
		sql.append(" select * from W_CK_RESULT_CARRY where 1=1 ");
	...
	public List<DpFile> getImages(String id){
		...
		map.put("tableId", id+"&&W_CK_RESULT_CARRY");
