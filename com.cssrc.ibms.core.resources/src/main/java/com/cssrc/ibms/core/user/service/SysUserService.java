...
public class SysUserService extends BaseService<SysUser> implements ISysUserService{
	...
	{
		return this.dao.getByIds(queryFilter);
	}
	...
	{
		return this.dao.getByIdsFilter(queryFilter);
	}
	...
	throws Exception
	{
		...
		if (sysUser.getUserId() == null) {
			...
			if ("D41D8CD98F00B204E9800998ECF8427E".equals(sysUser.getPassword())){
				//如果传来的密码是空,空经过md5加密之后是D41D..
				//就更新除了密码之外的字段
				this.dao.updateWithoutPassword(sysUser);
			}else {
				//正常更新全字段
	...
	public JSONArray getFuzzySysUserList(String userFuzzyName,String fieldName,String relvalue,String type,String typeVal){
		...
		if(!relvalue.isEmpty()){
			...
			while(it.hasNext()){
				...
				Object fieldValue = userObj.get(fieldName);
				Object org = userObj.get("org");
				List<String> moduleManages=moduleManageService.getByModuleIdDis(typeVal);
				//当前组织
				if(typeVal.equals("self")){
					...
					//定制 人员选择器过滤
				}else if(StringUtil.isNotEmpty(typeVal)&&type.equals("acceptanceFile")) {
					if(!userFuzzyName.equals("")) {
						if(moduleManages.contains(userObj.get("userId").toString())&&userObj.get("fullname").toString().indexOf(userFuzzyName)>=0) {
							userArr.add(userObj);
							if(userArr.size()>20)
								break;
						}
					}
					else if(moduleManages.contains(userObj.get("userId").toString())) {
						userArr.add(userObj);
						if(userArr.size()>20)
							break;
					}
