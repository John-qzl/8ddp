...
@Repository
public class IOCheckResultJGJGDao {
	@Resource
	...
	public CheckResult getById(String id) {
		String sql = " select * from W_CK_RESULT_JGJG where id='"+id+"'";
	...
	public void update(CheckResult checkResult) {
		...
		sql.append(" update W_CK_RESULT_JGJG set ");
	...
	public Long insert(CheckResult checkResult) {
		StringBuffer sql = new StringBuffer();
		sql.append(" insert into W_CK_RESULT_JGJG (ID,F_RESULT,F_VALUE,F_IFNUMM,F_SKETCHMAP,F_ITEMDEF_ID,F_TB_INSTAN)");
	...
	public List<Map<String,Object>> query(Map<String,Object> keyValueMap){
		StringBuffer sql = new StringBuffer();
		sql.append(" select * from W_CK_RESULT_JGJG where 1=1 ");
	...
	public List<DpFile> getImages(String id){
		...
		map.put("tableId", id+"&&W_CK_RESULT_JGJG");
