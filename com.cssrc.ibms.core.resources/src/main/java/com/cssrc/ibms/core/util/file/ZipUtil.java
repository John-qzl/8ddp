...
public class ZipUtil {
	 ...
	 *            压缩后是否删除原文件\文件夹
	 */
	public static void zip(String path, Boolean isDelete,String encoding) {
		...
		try {
			...
			if (file.isDirectory()) {
				zipFile = new ZipFile(new File(path + ".rar"));
				zipFile.setFileNameCharset(encoding);
				zipFile.addFolder(path, parameters);
			} else {
				zipFile = new ZipFile(new File(path.split(".")[0] + ".rar"));
				zipFile.setFileNameCharset(encoding);
	 ...
	 *            压缩后是否删除原文件\文件夹
	 */
	public static void zipByFiles(String path, Boolean isDelete,String encoding) {
		ZipFile zipFile = null;
		try {
			ZipParameters parameters = new ZipParameters();
			parameters.setCompressionMethod(Zip4jConstants.COMP_DEFLATE);
			...
			if (file.isDirectory()) {
				...
				for (int i = 0; i < files.length; i++) {
					flieList.add(files[i]);
				}
				zipFile = new ZipFile(new File(path + ".zip"));
				zipFile.setFileNameCharset(encoding);
				zipFile.addFiles(flieList, parameters);
			} else {
	...
	public static void zipSetPass(String path, Boolean isDelete, String password) {
		...
		try {
			...
			if (isDelete) {
				FileOperator.deleteDir(new File(path));
			}
		} catch (ZipException e) {
			e.printStackTrace();
		}
	...
	/**
	 * 在路径tempPath下新建一个fileName的压缩文件
	 * @param tempPath
	 * @param fileName
	 */
	public static void zip(String tempPath, String fileName) {
		zipFile(tempPath,false,fileName);
	}
	/**
	 * 压缩指定的文件和文件夹
	 * @param path 待压缩的路径（文件或文件夹）
	 * @param isDelete 是否删除原文件（夹）
	 * @param filePathAndName 压缩后的文件路径和名称
	 */
	public static void zipFile(String path, Boolean isDelete,String filePathAndName) {
		ZipFile zipFile = null;
		try {
			ZipParameters parameters = new ZipParameters();
			parameters.setCompressionMethod(Zip4jConstants.COMP_DEFLATE);
			...
			zipFile = new ZipFile(new File(filePathAndName));
			zipFile.setFileNameCharset("utf-8");
			zipFile.addFolder(path, parameters);
